...
namespace AmpGen
{
  ...
  public:
    ...
    Store(const size_t &nEntries = 0, const size_t &nFields = 0)
        : m_nEntries(nEntries), m_nBlocks(utils::aligned_size<stored_type>(nEntries) / utils::size<stored_type>::value), m_nFields(nFields),
          m_store(m_nBlocks * m_nFields)
    {
      DEBUG("Calling basic constructor: " << m_nEntries << " nBlocks = " << m_nBlocks << " " << m_nFields << " " << nEntries);
    }

    template <typename functor_type> void addFunctor(const functor_type &functor)
    {
      ...
      for(unsigned i = 0; i != vsize; ++i)
        offsets[i] = m_nFields + i;
      m_index[functor.name()] = offsets;
      m_nFields += vsize;
    }

    template <typename functor_type> Store(const size_t &nEntries, const std::vector<functor_type> &functors) { allocate(nEntries, functors); }
    ...
    {
      if constexpr(align == Alignment::SoA)
        return m_store[field * m_nBlocks + index];
      else
        return m_store[index * m_nFields + field];
    }
    template <typename return_type> inline const return_type get(const size_t &index, const size_t &field) const
    {
  ...
  public:
    ...
    FunctionCache(const input_type *input, const std::size_t &nFields = 0) : Store<stored_type, align>(m_input->size(), nFields), m_input(input) {}

    template <typename functor_type> FunctionCache(const input_type *input, const std::vector<functor_type> &functors) { allocate(input, functors); }
    template <typename functor_type> void allocate(const input_type *input, const std::vector<functor_type> &functors)
    {
      ...
      for(const auto &f : functors)
        Store<stored_type, align>::addFunctor(f);
    ...
    template <typename functor_type, typename = typename std::enable_if<!std::is_integral<functor_type>::value>::type>
    FunctionCache(const input_type *input, const functor_type &functors) : FunctionCache(input, 1)
    {
          ...
          if constexpr(std::is_same<typename functor_type::return_type, void>::value)
            {
              ...
              fcn.batch(Store<stored_type, align>::nBlocks(), m_input->nFields(), fieldStagger, nullptr, Store<stored_type, align>::data(), offsets.data(),
                        fcn.externBuffer().data(), m_input->data());
              DEBUG("Returning: " << real_v(Store<stored_type, align>::operator()(0, 0).real())); //  << std::endl;
            }
          else
            {
              DEBUG("Evaluating bulk function on [no-rto] : stagger:" << stagger << " Input Fields: " << m_input->nFields()
                                                                      << " field stagger: " << fieldStagger << " input: " << m_input->data()[0]);

              fcn.batch(Store<stored_type, align>::nBlocks(), m_input->nFields(), fieldStagger, Store<stored_type, align>::data() + f[0] * stagger,
                        fcn.externBuffer().data(), m_input->data());
            }
        }
      else
