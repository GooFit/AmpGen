...
/**
  ...
  Generally, the relativistic Breit-Wigner a sufficient description for many purposes for nonscalar resonances that decay to a single two-body final state or
  are relatively far from relevant thresholds.More complex systems require more specialised descriptions.
  */
...
#define DECLARE_LINESHAPE(X)                                                                                                                                   \
  ...
  public:                                                                                                                                                      \
    ...
    AmpGen::Expression get(const AmpGen::Expression &s, const AmpGen::Expression &s1, const AmpGen::Expression &s2, const std::string &particleName,           \
                           const unsigned int &L, const std::string &lineshapeModifier, AmpGen::DebugSymbols *dbexpressions = 0) const override;               \
...
namespace AmpGen {
  ...
  /** @ingroup Lineshapes namespace Lineshape
      Namespace that contains all lineshapes, i.e. propagators for describing amplitudes and phases for resonances (and nonresonant) contributions to a total
     amplitude.
   */
  ...
  namespace Lineshape {
    ...
    /** @ingroup Lineshapes class BW
        @brief Simple two-body Breit-Wigner lineshape that describes relatively narrow, isolated resonances that couple to a single channel / orbital
       configuration.
        ...
        @f]
        where @f$q@f$ is the linear momentum of either decay product in the rest frame of the parent, while @f$q_0@f$ the same quantity evaluated at the
       resonance mass. The Blatt-Weisskopf functions, @f$B_l(q,q_0)@f$ control the behaviour of functions at large momentum transfers. The width is normalised
       such that @f$\Gamma(m^2) = \Gamma_0 @f$. The parameters of the lineshape are tabulated below Parameter              | User name | Description
         -----------------------|--------------------------------------|------------------------------------------------------------------------
         @f$m@f$                | <EM>particleName_</EM>mass           | Breit-Wigner mass, defined as energy at which the self-energy of the resonance is
       purely imaginary (defaults to value in PDG)
         @f$\Gamma_0@f$         | <EM>particleName_</EM>width          | Breit-Wigner width, defined as the width of resonance at the Breit-Wigner mass
         @f$r@f$                | <EM>particleName_</EM>radius         | Hadronic radius for Blatt-Weisskopf form-factor (defaults to 1.5GeV for light
       resonances, 3.5GeV for charm)

        <EM> BL </EM> : Use Blatt-Weisskopf factors normalised at @f$ \sqrt{s}=m @f$ (by default, normalised at @f$\sqrt{s}=0@f$)
        \image html BW_combined.png "Modulus and phase of the Relativistic Breit-Wigner propagator, for orbital momentum up-to four, using the mass and nominal
       width of the rho meson."
    */
    ...
    */
    ...
    /** @ingroup Lineshapes class GounarisSakurai
        ...
        The Gounaris-Sakurai lineshape models dispersive corrections to the I=1,J=1 @f$\pi\pi@f$ propagator (G.J. Gounaris and J.J. Sakurai, Phys. Rev. Lett.21,
       244 (1968))
        @f[
        ...
        Parameter              | User name                            | Description
        -----------------------|--------------------------------------|------------------------------------------------------------------------
        @f$m@f$                | <EM>particleName_</EM>mass           | Breit-Wigner mass, defined as energy at which the self-energy of the resonance is purely
       imaginary (defaults to value in PDG)  <br>
        @f$\Gamma_0@f$         | <EM>particleName_</EM>width          | Breit-Wigner width, defined as the width of resonance at the Breit-Wigner mass <br>
        @f$r@f$                | <EM>particleName_</EM>radius         | Hadronic radius for Blatt-Weisskopf form-factor (defaults to 1.5GeV for light
       resonances, 3.5GeV for charm) <br>

         \image html figs/GS_combined.png "Gounaris-Sakurai lineshape for the ϱ(770) meson, with the equivalent relativistic Breit Wigner lineshape shown for
       comparison.
    */
    ...
    /** @ingroup Lineshapes class LASS
        ...
        The LASS parameterisation of the @f$ K\pi @f$ S-wave is derived from fits to ~ elastic @f$ K \pi @f$ scattering data, which is approximately up to the
       @f$ K \eta^\prime @f$ threshold. In this regime, unitarity implies that phases, rather than amplitudes should be summed. In this context, a slow varying
       nonresonant phase,
         @f[
         ...
         @f[
          \mathcal{A}(s)  = \frac{2 a \sqrt{s} }{ 2 + arq^2 - 2iaq} + \frac{2+arq^2 + 2iaq}{2+arq^2 - 2iaq }\mathcal{A}_{BW}(s) = \mathcal{A}_{NR}(s) +
       \mathcal{A}^{\prime}_{BW}(s).
         @f]
         As this expression somewhat resembles the sum of a Breit-Wigner with a slowly varying nonresonant component, the two parts are sometimes split apart
       with an additional production amplitude placed on one or the other. These can be accessed separately using the modifiers LASS.BW and LASS.NR, and given
       independent coupling constants. From the user interface: \code{.cpp} K*(1430)bar0[LASS.NR]{K-,pi+}  0 2.0 0.1 0 -90  0.1 K*(1430)bar0[LASS.BW]{K-,pi+}
       0 1.5 0.1 0  20  0.1 \endcode Corresponds to the overall lineshape:
         @f[
           \mathcal{A}(s)
         ...
         @f]
         An alternative way to introduce a different coupling between the two components is to fix one coupling and to introduce an additional production
       coupling. In the user interface, this might for example in @f$ D^{0} \to K^{-} \pi^{+} \pi^{0} @f$ decays: \code{.cpp} D0{K*(1430)bar0,pi0} 0 1.3 0.0 0
       15  0.1 K*(1430)bar0[LASS.NR]{K-,pi+}  0 2.0 0.1 0 -90  0.1 K*(1430)bar0[LASS.BW]{K-,pi+}  2 1.0 0.0 0   0  0.0 \endcode Then the lineshape with
       production couplings will be:
         @f[
           \mathcal{A}(s)
        ...
        Parameter              | User name                            | Description
        -----------------------|--------------------------------------|------------------------------------------------------------------------
        @f$m@f$                | <EM>particleName_</EM>mass           | Breit-Wigner mass of the resonant component, defined as energy at which the self-energy
       of the resonance is purely imaginary (defaults to value in PDG)  <br>
        @f$\Gamma_0@f$         | <EM>particleName_</EM>width          | Breit-Wigner width of the resonant component, defined as the width of resonance at the
       Breit-Wigner mass <br>
        @f$a@f$                | LASS::a                              | Scattering length of the nonresonant component, defaults to @f$2.07\mathrm{G\kern -0.1em
       eV}^{-1}@f$
        @f$r@f$                | LASS::r                              | Effective interaction length of the nonresonant component, defaults to
       @f$3.32\mathrm{G\kern -0.1em eV}^{-1}@f$
    */
    DECLARE_LINESHAPE(LASS);
    /** @ingroup Lineshapes class GLASS
        @brief Generalised description of the @f$ K\pi@f$ S-wave, taken from BABAR / BELLE analysis of @f$ D^{0} \to K_{S}^{0} \pi^{+} \pi^{-} @f$
       https://journals.aps.org/prd/pdf/10.1103/PhysRevD.98.112012

        Alternative parameterisation of the @f$ K \pi @f$ S-wave taken from the BABAR/BELLE analysis of @f$ D^{0} \to K_{S}^{0} \pi^{+} \pi^{-} @f$
       https://journals.aps.org/prd/pdf/10.1103/PhysRevD.98.112012. It introduces four additional parameters, two amplitudes and two phases, and is written
        @f[
        ...
        @f]
        that is, the 'resonant' and 'nonresonant' phases of the LASS formulation with additional 'proudction' phases @f$ \phi_R @f$ and @f$\phi_F @f$, and
       production couplings @f$R, F@f$. One of the production couplings can typically be fixed to 1, normally the resonant coupling, @f$R @f$.
       ...
       of the resonance is purely imaginary (defaults to value in PDG)  <br>
        @f$\Gamma_0@f$         | <EM>particleName_</EM>width          | Breit-Wigner width of the resonant component, defined as the width of resonance at the
       Breit-Wigner mass <br>
        @f$a@f$                | <EM>particleName</EM>::GLASS::a      | Scattering length of the nonresonant component, defaults to @f$2.07\mathrm{G\kern -0.1em
       eV}^{-1}@f$
        @f$r@f$                | <EM>particleName</EM>::GLASS::r      | Effective interaction length of the nonresonant component, defaults to
       @f$3.32\mathrm{G\kern -0.1em eV}^{-1}@f$
        @f$\phi_R@f$           | <EM>particleName</EM>::GLASS::phiR   | Additional production phase of the resonant component, defaults to @f$0^{\mathrm{o}}@f$,
       measured in degrees.
        @f$\phi_F@f$           | <EM>particleName</EM>::GLASS::phiF   | Additional production phase of the nonresonant component, defaults to
       @f$0^{\mathrm{o}}@f$, measured in degrees.
    ...
    */
    ...
    /** @ingroup Lineshapes class Flatte
        ...
        the @f$f_{0}(980)^{0}@f$ and @f$a_{0}(980)^{0}@f$ resonances, which couple to the channels @f$\pi\pi@f$ and @f$KK@f$ for the @f$f_{0}(980)^{0}@f$ and
       @f$\pi\eta@f$ and @f$KK@f$ for the @f$a_{0}(980)^{0}@f$. In particular, the opening of the @f$KK@f$ threshold close to the resonance mass strongly
       distorts the lineshape. For a generic implementation of a coupled channel, see the CoupledChannel lineshape.
        @f[
     ...
     *  @brief Polynominal shape \f$ \mathcal{A}(s) = \sum^n_i c_i s^{i} \f$ where the sum is to lineshapeModifier::Degree, and the free parameters of the shape
     * are lineshapeModifier_ci
     */
    DECLARE_LINESHAPE(PolyNR);

    /** @ingroup Lineshapes class PolyNR
     *  @brief Polynominal shape \f$ \mathcal{A}(s) = \sqrt{ \sum_{ij} c_{ij} s^{i} (s^\prime)^{j} } \f$ where the sum is to lineshapeModifier::Degree, and the
     * free parameters of the shape are lineshapeModifier_cij
     */
    DECLARE_LINESHAPE(Poly);

    /** @ingroup Lineshapes class FOCUS
     *  @brief K matrix amplitudes used for I=1/2 and I=3/2 in the description of the \f$ K\pi \f$ S-wave in the analysis of @f$ D^{+}\rightarrow
     * K^{-}\pi^{+}\pi^{+}@f$ https://arxiv.org/abs/0705.2248
     */
    DECLARE_LINESHAPE(FOCUS);

    /// I=1/2 and I=3/2 K Matrices used in the description of the \f$ K\pi\f$ S-wave in the analysis of @f$\eta_{c}\rightarrow K^{+} K^{-} \pi^{0}@f$
    /// https://arxiv.org/abs/1701.04881
    ...
    /** @ingroup Lineshapes class MIPWA
        ...
        The real and imaginary (or amplitude and phase) of the lineshape are fixed at \f$ N \f$ discrete positions to independent pairs of free parameters to be
       determined in a fit, and the value of the lineshape determined elsewhere by interpolating between these values using cubic splines. Based on techniques
       first used by the E791 collaboration in studying @f$ K \pi @f$ scalars, see https://arxiv.org/abs/hep-ex/0510045. <br> The amplitude is given by
         @f[
            \mathcal{A}(s) = \sum_n ( a_n + b_n \tilde{s} + c_n \tilde{s}^2 + d_n \tilde{s}^3 ) \mathrm{rect}\left( \frac{s - nL}{nL} \right),
         @f]
         where @f$a_n@f$ is the value of the function evaluated at the @f$ n @f$ knot and the other polynomial coefficients are determined by imposing
       continuity and differentiability. Parameter              | Interface parameter name             | Description
         -----------------------|--------------------------------------|------------------------------------------------------------------------
  ...
  Expression width(const Expression &s, const Expression &s1, const Expression &s2, const Expression &mass, const Expression &width, const Expression &radius,
                   unsigned int L, DebugSymbols *dbexpressions = nullptr);
} // namespace AmpGen
#endif
