...
namespace AmpGen
{
  ...
  template <std::size_t I = 0, typename FuncT, typename... Tp> typename std::enable_if_t<I == sizeof...(Tp), void> for_each(std::tuple<Tp...> &, FuncT) {}

  template <std::size_t I = 0, typename FuncT, typename... Tp> inline typename std::enable_if_t < I<sizeof...(Tp), void> for_each(std::tuple<Tp...> &t, FuncT f)
  {
    f(std::get<I>(t));
    for_each<I + 1, FuncT, Tp...>(t, f);
  }
  template <typename iterator, typename... transform_types> void for_each_sequence(iterator begin, iterator end, transform_types... transforms)
  {
    for_each(std::tuple<transform_types...>(transforms...), [&](auto &transform) { std::for_each(begin, end, transform); });
  }

  template <std::size_t I = 0, typename FuncT, typename... Tp> typename std::enable_if_t<I == sizeof...(Tp), void> for_each(const std::tuple<Tp...> &, FuncT) {}
  ...
  template <std::size_t I = 0, typename FuncT, typename... Tp>
  typename std::enable_if_t<I == sizeof...(Tp), void> for_each_with_counter(const std::tuple<Tp...> &, FuncT)
  {}
  ...
  template <typename R, typename RT, typename ARGS, bool result = std::is_same<decltype(((*(R *)nullptr))(ARGS())), RT>::value> constexpr bool typeHelper(int)
  {
    return result;
  }

  template <typename R, typename RT, typename ARGS> constexpr bool typeHelper(...) { return false; }

  template <typename R, typename RT, typename ARGS> constexpr bool hasReturnType() { return typeHelper<R, RT, ARGS>(0); }
...
#define def_has_function(function_name)                                                                                                                        \
  template <typename T> struct has_##function_name                                                                                                             \
  {                                                                                                                                                            \
  ...
  template <typename T, typename rt, typename... args> struct is_functor<T, rt(args...)>
  {
    template <typename U> static constexpr auto test(U *) -> decltype(std::declval<U>().operator()(std::declval<args>()...), std::true_type());
