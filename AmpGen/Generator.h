...
namespace AmpGen {
  template <typename phaseSpace_t = PhaseSpace> class Generator {
#if ENABLE_AVX
  ...
  public:
    ...
    template <typename pdf_t> double getMax(const eventlist_t &events, pdf_t &pdf) const {
      double max = 0.;
...
#pragma omp parallel for
#endif
        for(size_t block = 0; block < mc.nBlocks(); ++block) { mc.setGenPDF(block, pdf(mc.block(block), block) / mc.genPDF(block)); }
        ...
        for(const auto &event : mc) {
          ...
          if(event.genPdf() > maxProb * m_rnd->Rndm()) {
            ...
            efficiencyReport[event.index()] = false;
        ...
        pb.print(double(list.size()) / double(N),
                 " ε[gen] = " + mysprintf("%.4f", efficiency) + "% , " + std::to_string(int(t_total.count() / 1000.)) + " seconds");
    ...
    template <typename pdf_t, typename = typename std::enable_if<!std::is_integral<pdf_t>::value>::type> EventList generate(pdf_t &pdf, const size_t &nEvents) {
