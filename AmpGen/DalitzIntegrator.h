...
namespace AmpGen
{
  ...
  public:
    ...
    template <typename FCN> double integrateDP(FCN &&fcn) const { return integrateDP(fcn, m_s0); }
...
#pragma message("WARNING: DalitzIntegrator only supports scalar or AVX2(d) instruction sets")
    ...
    auto i1 = integrate<2>(
                [&](const std::array<real_v, 2> &x) {
                  ...
                  return J(*reinterpret_cast<const sqCo *>(&x), s) * real(fcn(event));
                },
                std::array<double, 2>{0., 0.}, std::array<double, 2>{1., 1.})
              / s;
    return i1;
#endif
    return 0;
