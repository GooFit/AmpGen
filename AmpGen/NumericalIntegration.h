...
namespace AmpGen
{
  template <typename function_type> double integrate_1d(const function_type &fcn, const double &min, const double &max, gsl_integration_workspace *ws = nullptr)
  {
    ...
    if constexpr(is_functor<function_type, double(const double &)>::value)
      ...
      static_assert(true, "function matches no signature!");
  ...
  template <unsigned dim, typename fcn> double integrate(const fcn &F, const std::array<double, dim> xmin, const std::array<double, dim> &xmax)
  {
    ...
    if constexpr(dim == 1)
      {
        if constexpr(is_functor<fcn, double(const double &)>::value)
          {
          ...
          static_assert(true, "1D function doesn't have recognised signature");
        return 0;
      }
    else
      {
        ...
        do
          {
                ...
                while(true)
                  {
                    ...
                    if(isbtmp > isbrgs)
                      break;
                    if(isbtmp < isbrgs && partial_integrals[isbtmp].var < partial_integrals[isbtmp + 1].var)
                      isbtmp++;
                    if(rgnerr >= partial_integrals[isbtmp].var)
                      break;
            ...
            if((relerr < epsrel && aresult < epsabs) or ((relerr < epsrel || abserr < epsabs) && ifncls > minpts))
              {
                status = 0;
                break;
              }
            if((isbrgs >= partial_integrals.size() - 1) or (ifncls + 2 * irlcls > maxpts))
              {
                status = 2;
                break;
              }
