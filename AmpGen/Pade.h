...
namespace AmpGen
{
  ...
  namespace detail
  {
    std::vector<double> solve_pade(const std::function<double(const double &)> &fcn, const double &min, const double &max, const unsigned &N,
  ...
  {
  public:
    Pade(const std::vector<double> &r, const double &min, const double &max) : min(min), max(max)
    {
    ...
    template <typename T2> T2 operator()(const T2 &s) const
    {
      T2 x = (s - min) * range;
      T2 f = 0.;
      T2 g = 1.;
    ...
    {
      for(int i = 0; i != N + 1; ++i)
        std::cout << co_f[i] << std::endl;
      for(int i = 0; i != N; ++i)
        std::cout << co_g[i] << std::endl;
    }
