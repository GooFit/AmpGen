...
namespace AmpGen {
  namespace integrate_fp_constants {
    ...
    static const double wn1[14] = {-0.193872885230909911, -0.555606360818980835, -0.876695625666819078, -1.15714067977442459, -1.39694152314179743,
    ...
    static const double wn3[14] = {0.0518213686937966768,  0.0314992633236803330,  0.0111771579535639891,  -0.00914494741655235473, -0.0294670527866686986,
                                   -0.0497891581567850424, -0.0701112635269013768, -0.0904333688970177241, -0.110755474267134071,   -0.131077579637250419,
                                   -0.151399685007366752,  -0.171721790377483099,  -0.192043895747599447,  -0.212366001117715794};

    static const double wn5[14]
      = {0.871183254585174982e-01, 0.435591627292587508e-01, 0.217795813646293754e-01, 0.108897906823146873e-01, 0.544489534115734364e-02,
         0.272244767057867193e-02, 0.136122383528933596e-02, 0.680611917644667955e-03, 0.340305958822333977e-03, 0.170152979411166995e-03,
         0.850764897055834977e-04, 0.425382448527917472e-04, 0.212691224263958736e-04, 0.106345612131979372e-04};

    static const double wpn1[14] = {-1.33196159122085045, -2.29218106995884763, -3.11522633744855959, -3.80109739368998611, -4.34979423868312742,
    ...
    static const double wpn3[14] = {0.0445816186556927292, -0.0240054869684499309, -0.0925925925925925875, -0.161179698216735251, -0.229766803840877915,
                                    -0.298353909465020564, -0.366941015089163228,  -0.435528120713305891,  -0.504115226337448555, -0.572702331961591218,
                                    -0.641289437585733882, -0.709876543209876532,  -0.778463648834019195,  -0.847050754458161859};
  }
  ...
  std::tuple<double, double, unsigned> integrate_fp(const fcn &F, const std::array<double, dim> &ctr, const std::array<double, dim> &wth) {
    using namespace integrate_fp_constants;
    ...
    sum1 = F(z).at(0);
    double difmax = 0;
    const real_v xl(-xl4, -xl2, +xl2, +xl4);
    const real_v dxp(-xl4, +xl4, -xl4, +xl4);
    const real_v dxp2(-xl4, -xl4, +xl4, +xl4);
    ...
    for(unsigned j = 0; j != dim; j++) {
      ...
      idvaxn = dif >= difmax ? j : idvaxn;
      difmax = dif >= difmax ? dif : difmax;
      for(unsigned k = j + 1; k < dim; ++k) sum4 += utils::sum_elements(eval_2(z, dxp * wth[j], j, dxp2 * wth[k], k));
    }
    for(int j = 0; j < get_power<2, dim>::value; j += 4) {
      // auto loop_mask =
      for(int k = 0; k != dim; ++k)
        z[k] = real_v(ctr[k] + (0x1 & ((j + 0) >> k) ? +1 : -1) * xl5 * wth[k], ctr[k] + (0x1 & ((j + 1) >> k) ? +1 : -1) * xl5 * wth[k],
                      ctr[k] + (0x1 & ((j + 2) >> k) ? +1 : -1) * xl5 * wth[k], ctr[k] + (0x1 & ((j + 3) >> k) ? +1 : -1) * xl5 * wth[k]);
