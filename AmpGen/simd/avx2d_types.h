...
#define libmvec_alias(F)                                                                                                                                       \
  inline real_v F(const real_v &v) {                                                                                                                           \
    auto arr = v.to_ptr();                                                                                                                                     \
    return real_v(std::F(arr[0]), std::F(arr[1]), std::F(arr[2]), std::F(arr[3]));                                                                             \
  }
#endif
...
namespace AmpGen {
  namespace AVX2d {
    struct real_v {
      ...
      real_v(const double &x0, const double &x1, const double &x2, const double &x3) { data = _mm256_set_pd(x3, x2, x1, x0); }
    ...
    libmvec_alias(sin) libmvec_alias(cos) libmvec_alias(exp)
      // libmvec_alias( log )
      inline real_v log(const real_v &v) {
    ...
    inline real_v gather(const double *base_addr, const real_v &offsets) { return _mm256_i64gather_pd(base_addr, offsets.to_int(), sizeof(double)); }
    inline real_v fmadd(const real_v &a, const real_v &b, const real_v &c) { return _mm256_fmadd_pd(a, b, c); }
    ...
    inline complex_v select(const real_v &mask, const real_v &a, const complex_v &b) {
      return complex_v(select(mask, a, b.real()), select(mask, real_v(0.), b.imag()));
    }
    inline complex_v select(const real_v &mask, const complex_v &a, const real_v &b) {
      return complex_v(select(mask, a.real(), b), select(mask, a.imag(), real_v(0.)));
    }
