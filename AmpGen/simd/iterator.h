...
namespace AmpGen {
  ...
  public:
    ...
    scatter_iterator(const size_t &pos, store_type *store) : m_store(store), m_pos(pos) {
    ...
    ~scatter_iterator() {
      if constexpr(modifiable == true) {
        if(m_store != nullptr && m_pos % simd_size != 0) { m_store->gather(m_buffer, m_pos / simd_size); }
      }
    }
  ...
  template <unsigned simd_size, bool modifiable = false, class store_type> auto make_scatter_iterator(const unsigned &pos, store_type *store) {
