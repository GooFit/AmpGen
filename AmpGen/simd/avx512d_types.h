...
#define stl_fallback(x)                                                                                                                                        \
  inline real_v x(const real_v &v) {                                                                                                                           \
    auto a = v.to_array();                                                                                                                                     \
    return real_v(std::x(a[0]), std::x(a[1]), std::x(a[2]), std::x(a[3]), std::x(a[4]), std::x(a[5]), std::x(a[6]), std::x(a[7]));                             \
  }
    ...
    inline real_v operator+=(real_v &lhs, const real_v &rhs) {
      lhs = lhs + rhs;
      return lhs;
    }
    inline real_v operator-=(real_v &lhs, const real_v &rhs) {
    ...
    inline real_v atan2(const real_v &y, const real_v &x) {
      ...
      return real_v(std::atan2(by[0], bx[0]), std::atan2(by[1], bx[1]), std::atan2(by[2], bx[2]), std::atan2(by[3], bx[3]), std::atan2(by[4], bx[4]),
                    std::atan2(by[5], bx[5]), std::atan2(by[6], bx[6]), std::atan2(by[7], bx[7]));
    }
    ...
    inline real_v gather(const double *base_addr, const real_v &offsets) { return _mm512_i64gather_pd(double_to_int(offsets), base_addr, sizeof(double)); }
    ...
    inline void frexp(const real_v &value, real_v &mant, real_v &exponent) {
      ...
      mant
        = _mm512_castsi512_pd(_mm512_or_si512(_mm512_and_si512(arg_as_int, _mm512_set1_epi64(0x000FFFFFFFFFFFFFll)), _mm512_set1_epi64(0x3FE0000000000000ll)));
    }

    inline real_v fmadd(const real_v &a, const real_v &b, const real_v &c) { return _mm512_fmadd_pd(a, b, c); }
    ...
    stl_fallback(exp) stl_fallback(tan) stl_fallback(sin) stl_fallback(cos) inline real_v remainder(const real_v &a, const real_v &b) {
      return a - real_v(_mm512_roundscale_pd(a / b, _MM_FROUND_TO_NEG_INF)) * b;
    }
    ...
    inline complex_v operator/(const real_v &lhs, const complex_v &rhs) {
      return complex_v(lhs * rhs.real(), -lhs * rhs.imag()) / (rhs.real() * rhs.real() + rhs.imag() * rhs.imag());
    }
    ...
    inline complex_v select(const __mmask8 &mask, const complex_v &a, const complex_v &b) {
      return complex_v(select(mask, a.real(), b.real()), select(mask, a.imag(), b.imag()));
    }
    inline complex_v select(const __mmask8 &mask, const real_v &a, const complex_v &b) {
      return complex_v(select(mask, a, b.real()), select(mask, 0.f, b.imag()));
    }
    inline complex_v select(const __mmask8 &mask, const complex_v &a, const real_v &b) {
      return complex_v(select(mask, a.real(), b), select(mask, a.imag(), 0.f));
    }
    ...
    inline complex_v log(const complex_v &v) { return complex_v(0.5 * log(norm(v)), atan2(v.imag(), v.real())); }

    inline std::ostream &operator<<(std::ostream &os, const complex_v &obj) { return os << "( " << obj.real() << ") (" << obj.imag() << ")"; }
#pragma omp declare reduction(+ : real_v : omp_out = omp_out + omp_in)
#pragma omp declare reduction(+ : complex_v : omp_out = omp_out + omp_in)
