...
namespace AmpGen {
  ...
  namespace utils {
    ...
    simd_type gather(const container_type &container, const functor_type &functor, unsigned offset = 0, typename simd_type::scalar_type df = 0.) {
      ...
      if(df == 0.)
        for(unsigned k = 0; k != simd_type::size; ++k)
          rv[k] = offset + k < container.size() ? functor(container[offset + k]) : functor(container[container.size() - 1]);
      else
    ...
    template <typename simd_type, typename value_type> bool all_of(const simd_type &obj, const value_type &v) { return all_of(obj == v); }
    ...
    template <unsigned p = 0, typename vtype> auto get(vtype v) {
      if constexpr(is_complex_type<vtype>::value)
        return std::complex(get<p>(v.real()), get<p>(v.imag()));
      else if constexpr(is_vector_type<vtype>::value)
        return v.at(p);
      else if constexpr(std::is_same<vtype, complex_v>::value)
        return std::complex(get<p>(v.real()), get<p>(v.imag()));
      else if constexpr(!is_vector_type<vtype>::value)
        return v;
    }
    ...
    template <typename T> inline auto norm(T &&value) {
      if constexpr(is_std__complex<std::remove_reference_t<T>>::value) {
