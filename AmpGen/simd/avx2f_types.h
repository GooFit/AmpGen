...
#else
#define libmvec_alias(F, O)                                                                                                                                    \
  inline real_v O(const real_v &v) {                                                                                                                           \
    auto arr = v.to_ptr();                                                                                                                                     \
    return real_v(std::F(arr[0]), std::F(arr[1]), std::F(arr[2]), std::F(arr[3]), std::F(arr[4]), std::F(arr[5]), std::F(arr[6]), std::F(arr[7]));             \
  }
#endif
...
namespace AmpGen {
  namespace AVX2f {
    struct real_v {
      ...
      real_v(const scalar_type &x0, const scalar_type &x1, const scalar_type &x2, const scalar_type &x3, const scalar_type &x4, const scalar_type &x5,
             const scalar_type &x6, const scalar_type &x7) {
    ...
    inline real_v real_v::operator+=(const real_v &rhs) {
      *this = *this + rhs;
      return *this;
    }
    inline real_v real_v::operator-=(const real_v &rhs) {
    ...
    libmvec_alias(sinf, sin) libmvec_alias(cosf, cos) libmvec_alias(expf, exp)
      libmvec_alias(logf, log) inline std::array<int32_t, real_v::size> store(const __m256i &v) {
      alignas(32) std::array<int32_t, real_v::size> rt;
      _mm256_store_si256((__m256i *)&rt[0], v);
      return rt;
    }

    inline void sincos(const real_v &v, real_v &s, real_v &c) { /// todo - add back support for mvec
      s = sin(v);
      c = cos(v);
    }
    ...
    inline complex_v select(const real_v &mask, const complex_v &a, const complex_v &b) {
      return complex_v(select(mask, a.real(), b.real()), select(mask, a.imag(), b.imag()));
    }
    inline complex_v select(const real_v &mask, const real_v &a, const complex_v &b) {
      return complex_v(select(mask, a, b.real()), select(mask, 0.f, b.imag()));
    }
    inline complex_v select(const real_v &mask, const complex_v &a, const real_v &b) {
