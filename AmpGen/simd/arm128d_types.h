...
#define libmvec_alias(F)                                                                                                                                       \
  inline real_v F(const real_v &v) {                                                                                                                           \
    auto arr = v.to_ptr();                                                                                                                                     \
    return real_v(std::F(arr[0]), std::F(arr[1]));                                                                                                             \
  }
#endif
...
namespace AmpGen {
  namespace ARM128d {
    ...
    libmvec_alias(sin) libmvec_alias(cos) libmvec_alias(exp) libmvec_alias(log) inline void sincos(const real_v &v, real_v &s, real_v &c) {
      s = sin(v);
      c = cos(v);
    }
    ...
    inline real_v fmadd(const real_v &a, const real_v &b, const real_v &c) { return vmlaq_f64(a, b, c); }
