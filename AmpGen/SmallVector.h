...
namespace AmpGen {
  template <typename type, unsigned max_size> struct SmallVector {
    ...
    SmallVector() = default;
    SmallVector(std::initializer_list<value_type> &&values) { insert(begin(), values.begin(), values.end()); }
    bool operator==(const SmallVector &other) const { return other.store == this->store; }
  };
}
