...
/** @defgroup ExpressionEngine Expressions
    ...
    There is limited functionality for expressions to be evaluated in-situ to the default values associated with their parameters, which can useful for
   debugging purposes. The more typical usage is to use an expression to generate a CompiledExpression that compiles the expression and wraps the corresponding
   shared library with a functor.
    ...
    Use of the unary classes directly can cause unexpected behaviour if used in conjunction with the auto keyword, and does not include some optimisations as
   readily such as constant folding.
    */
...
namespace AmpGen {
  ...
  */
  ...
  template <class T, typename std::enable_if_t<hasConstructor<Constant, T>()>> Expression operator+(const Expression &A, const T &B) { return A + Constant(B); }
  template <class T, typename std::enable_if_t<hasConstructor<Constant, T>()>> Expression operator-(const Expression &A, const T &B) { return A - Constant(B); }
  template <class T, typename std::enable_if_t<hasConstructor<Constant, T>()>> Expression operator*(const Expression &A, const T &B) { return A * Constant(B); }
  template <class T, typename std::enable_if_t<hasConstructor<Constant, T>()>> Expression operator/(const Expression &A, const T &B) { return A / Constant(B); }

  template <class T, typename std::enable_if_t<hasConstructor<Constant, T>()>> Expression operator+(const T &A, const Expression &B) { return Constant(A) + B; }
  template <class T, typename std::enable_if_t<hasConstructor<Constant, T>()>> Expression operator-(const T &A, const Expression &B) { return Constant(A) - B; }
  template <class T, typename std::enable_if_t<hasConstructor<Constant, T>()>> Expression operator*(const T &A, const Expression &B) { return Constant(A) * B; }
  template <class T, typename std::enable_if_t<hasConstructor<Constant, T>()>> Expression operator/(const T &A, const Expression &B) { return Constant(A) / B; }
  ...
  template <class T> bool is(const Expression &expression) { return dynamic_cast<const T *>(expression.get()) != nullptr; }
  template <class T> T cast(const Expression &expression) { return *static_cast<const T *>(expression.get()); }
