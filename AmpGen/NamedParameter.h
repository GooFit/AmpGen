...
namespace AmpGen
{
  /** @class NamedParameter
    ...
    */
  template <typename T> class NamedParameter
  {
  protected:
    ...
    bool setFromOptionsParser()
    {
      ...
      if(vsl.size() < 2)
        return false; // first element is parameter name
      m_valueArray.clear();
  ...
  public:
    NamedParameter(const std::string &name, const T &def = T(), const std::string &helpString = "") : m_name(name), m_helpString(helpString)
    {
      setVal(def);
    ...
    NamedParameter(const std::string &name, const std::vector<T> &defVec, const std::string &helpString = "") : m_name(name), m_helpString(helpString)
    {
      setVal(defVec);
      setFromOptionsParser();
      if(OptionsParser::printHelp())
        help(defVec.size() > 0 ? defVec[0] : T());
    }
    void help(const T &def)
    {
      std::string type = type_string<T>();
      type = replaceAll(type, "AmpGen::", "");
    ...
    const T getVal(int i = 0) const
    {
      if(i < 0 || i >= (int)m_valueArray.size())
        {
          ERROR("Parameter name \"" << name() << "\". Index: " << i << " out of range [ " << 0 << " , " << m_valueArray.size());
    ...
    void setVal(const std::vector<T> &valList) { m_valueArray = valList; }
      ...
      do
        {
          ...
          if(obj != T())
            return_container.push_back(obj);
        }
