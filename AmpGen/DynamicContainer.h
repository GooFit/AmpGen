...
namespace AmpGen {
   ...
   */
  template <class TYPE, class CONTAINER_TYPE = std::vector<TYPE>> class DynamicContainer {
  public:
    class Iterator {
    private:
    ...
    public:
      ...
      Iterator operator++() const {
        if(m_pos + 1 % m_container->m_buffer.size() == 0 && m_pos + 1 != m_container->m_totalSize) { m_container->refresh(); }
        return Iterator(m_pos + 1, m_container);
      }
      Iterator &operator++() {
        m_pos++;
        if(m_pos % m_container->m_buffer.size() == 0 && m_pos % m_container->m_totalSize) { m_container->refresh(); }
        return *this;
      }
  ...
  public:
    template <class GENERATOR> DynamicContainer(const size_t &totalSize, const GENERATOR &generator) : m_totalSize(totalSize), m_generator(generator) {
      refresh();
    }
