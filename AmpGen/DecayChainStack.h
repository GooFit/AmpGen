...
namespace AmpGen
{
  ...
  template <unsigned N> class DecayChainStack : public DecayChainStackBase
  {
    struct Node
    {
      ...
      template <typename T> T operator()(const T &s) const
      {
        switch(type)
          {
          case Type::BW:
            return (bwMass * bwWidth)
                   / ((phi_range.second - phi_range.first) * ((s - bwMass * bwMass) * (s - bwMass * bwMass) + bwMass * bwMass * bwWidth * bwWidth));
      ...
      double operator()(TRandom3 *random) const
      {
        switch(type)
          ...
          case Type::Flat: return (range.second - range.first) * random->Rndm() + range.first;
          case Type::Stable: return range.first;
          case Type::QuasiStable: return range.first;
          }
        return 0;
      ...
      void set(const Particle *particle)
      {
        ...
        bwWidth = props->width();
        bwMass = props->mass();
        INFO("Setting particle: " << particle->name() << " " << particle->isStable() << " " << particle->isQuasiStable() << " "
                                  << particle->lineshapeContains({"FormFactor", "None", "TD"}));
        if(particle->isStable())
          type = Type::Stable;
        else if(particle->isQuasiStable())
          type = Type::QuasiStable;
        else if(particle->lineshapeContains({"FormFactor", "None", "TD"}))
          type == Type::Flat;
        else
          {
            type = Type::BW;
      ...
      {
        return type == other.type && range == other.range && phi_range == other.phi_range && bwMass == other.bwMass && bwWidth == other.bwWidth && l == other.l
               && r == other.r && lfs == other.lfs && rfs == other.rfs;
      }
  ...
  public:
      ...
      while(toDo.size() != 0)
        {
          ...
          if(!current->daughter(0)->isStable())
            {
              d1_index = ++counter + N;
              toDo.emplace(current->daughter(0), d1_index - N);
            }
          if(!current->daughter(1)->isStable())
            {
              d2_index = ++counter + N;
              toDo.emplace(current->daughter(1), d2_index - N);
            }
          m_nodes[index] = Node(Node::Type::Flat, d1_index, d2_index);
          if(index == 0 or current->isQuasiStable())
            m_nodes[index].range = std::make_pair(s0 - G, s0 + G);
          else
          ...
          for(auto &n : m_nodes)
            {
              if(n.l > N && n.r > N)
                {
                  m_nodes[n.l - N].setRange(m_nodes[n.l - N].range.first,
                                            n.range.second + m_nodes[n.r - N].range.first - 2 * std::sqrt(n.range.second * m_nodes[n.r - N].range.first));
                  m_nodes[n.r - N].setRange(m_nodes[n.r - N].range.first,
                                            n.range.second + m_nodes[n.l - N].range.first - 2 * std::sqrt(n.range.second * m_nodes[n.l - N].range.first));
                }
            }
        }
      auto concat = [](auto v1, const auto &v2) {
      ...
      for(auto n = m_nodes.rbegin(); n != m_nodes.rend(); ++n)
        {
          ...
          // INFO(m_nodes.size() - ( n-m_nodes.rbegin()) + N -1  << " " << rho << " " << n->l << " " << n->r << " " << n->range.first << " " << n->range.second
          // << " " << n->type <<
    ...
    */
    template <typename T> T genPdf(const T *event) const
    {
      T g = 1;
      for(const auto &node : m_nodes)
        {
    ...
    template <typename T> T s(const T *event, const SmallVector<unsigned, N> &lfs, const SmallVector<unsigned, N> &rfs) const
    {
      std::array<T, 4> P = {0.};
    ...
    double rho_2(const double &s0, const double &s1, const double &s2) const { return 1 - 2 * (s1 + s2) / s0 + (s1 - s2) * (s1 - s2) / (s0 * s0); }
