...
namespace AmpGen {
  ...
  template <typename ret_type, typename... arg_types> class CompiledExpression<ret_type(arg_types...)> : public CompiledExpressionBase {
  private:
  ...
  public:
    ...
    template <typename... namedArgs> void set(const Expression &expression, const std::string &name, const namedArgs &... args) {
      ...
      auto process_argument = [this, &mps](const auto &arg) mutable {
        ...
        if constexpr(std::is_convertible<decltype(arg), DebugSymbols>::value) {
          this->m_db = arg;
        } else if constexpr(std::is_convertible<decltype(arg), std::map<std::string, unsigned>>::value)
          this->m_evtMap = arg;
        else if constexpr(std::is_convertible<decltype(arg), const MinuitParameterSet *>::value
                          or std::is_convertible<decltype(arg), const AmpGen::MinuitParameterSet *>::value
                          or std::is_convertible<decltype(arg), MinuitParameterSet *>::value) {
          mps = arg;
        } else if constexpr(std::is_convertible<decltype(arg), MinuitParameterSet>::value)
          mps = &arg;
          ...
          ERROR("Unrecognised argument: " << type_string(arg));
      };
    ...
    template <typename T> void debug(const T *event) const {
      if(!m_fcn.isLinked()) { FATAL("Function " << name() << " not linked"); }
      if(!m_fdb.isLinked()) { FATAL("Function" << name() << " debugging symbols not linked"); }
      std::vector<std::pair<std::string, complex_v>> debug_results;
      if constexpr(std::is_same<void, ret_type>::value)
        debug_results = m_fdb(nullptr, 0, &(m_externals[0]), event);
      else
        debug_results = m_fdb(&(m_externals[0]), event);
      for(auto &debug_result : debug_results) {
        auto val = debug_result.second;
        auto label = debug_result.first;
        if(utils::all_of(val.real(), -999.))
          std::cout << bold_on << std::setw(50) << std::left << label << bold_off << std::endl;
        else if(utils::all_of(val.imag(), 0.))
          std::cout << "  " << std::setw(50) << std::left << label << " = " << val.real() << std::endl;
        else
    ...
    bool link(const std::string &handle) override { return link(dlopen(handle.c_str(), RTLD_NOW)); };
    std::string arg_type(const unsigned &i) const override { return typelist<arg_types...>()[i]; }
  };
