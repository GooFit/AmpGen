...
/** @defgroup Vertices Spin
    ...
    These objects are combined using a vertex to form a generalised current that can be used to compute the angular momentum components of the transition
   matrix. In general, these objects are only described for decays of the type \f$ 1\to 2 \f$, with more complex structures constructed from such quasi two-body
   decays (see Isobar model), but more general amplitudes can also be defined.
  */
...
namespace AmpGen {
  ...
  namespace Vertex {
    ...
    struct Base {
      ...
      */
      virtual AmpGen::Tensor
      operator()(const AmpGen::Tensor &P, const AmpGen::Tensor &Q, const AmpGen::Tensor &V1, const AmpGen::Tensor &V2, AmpGen::DebugSymbols *db = nullptr)
        = 0;
