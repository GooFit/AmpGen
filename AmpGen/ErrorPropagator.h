...
namespace AmpGen {
  ...
  */
  ...
  public:
    ...
    ///< Constructor for LinearErrorPropagator, taking a MinuitParameterSet as argument, assumes a diagonal coviarance matrix using the uncertainties on
    ///< parameters.
    ...
    template <class FCN> double operator()(FCN fcn) const { return getError(std::function<double(void)>(fcn)); }
