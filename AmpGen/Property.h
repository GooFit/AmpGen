...
namespace AmpGen {
      ...
      if constexpr(isVector<value_t>::value) {
        ...
        for_each_with_counter(m_value, [this, vsl](auto &f, unsigned i) {
          ...
          *const_cast<basic_t *>(&f) = lexical_cast<basic_t>(vsl[i], status);
          if(!status) { ERROR("Failed to parse token: " << vsl[i] << " for parameter: " << this->m_name); }
        });
      } else {
        if(vsl.size() != 1) {
  ...
  private:
    void help(const value_t &def) {
      ...
      if(std::is_same_v<value_t, std::string>) type = "string";
      if(std::is_same_v<value_t, std::vector<std::string>>) type = "strings";
