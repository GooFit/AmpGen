...
namespace AmpGen
{
  ...
  declare_enum(coordinateType, cartesian, polar) declare_enum(angType, deg, rad)
    ...
    void set_tuple_from_expression(arg_type &arg, const Expression &expr)
  {
    if constexpr(std::is_same<arg_type, Expression>::value)
      ...
      {
        arg = double(std::real(expr()));
      }
    else if constexpr(std::is_same<arg_type, std::complex<double>>::value)
      {
        arg = expr();
      }
    else
      FATAL("Unrecognised type: " << type_string<arg_type>());
  }
  ...
  template <typename... types> std::tuple<types...> unpack(const Expression &expression)
  {
    ...
    if(!is<ExpressionPack>(expression))
      FATAL("Not a packed expression");
    auto as_pack = cast<ExpressionPack>(expression).expressions();
    if(as_pack.size() != std::tuple_size<std::tuple<types...>>::value)
      FATAL("Wrong number of expressions");
  ...
  public:
    ...
    static Expression
    parse(std::vector<std::string>::const_iterator begin, std::vector<std::string>::const_iterator end, const MinuitParameterSet *mps = nullptr);
    static ExpressionParser *gExpressionParser;
    static ExpressionParser *getMe()
    {
      if(!gExpressionParser)
        gExpressionParser = new ExpressionParser();
  ...
  private:
    ...
    Expression
    parseTokens(std::vector<std::string>::const_iterator begin, std::vector<std::string>::const_iterator end, const MinuitParameterSet *mps = nullptr);
