...
namespace AmpGen {
  ...
  public:
    ...
    void setGenPDF(const unsigned int &pos, const double &g) { m_data[pos].setGenPdf(g); }
    ...
    template <typename functor> unsigned count(functor &&fcn) const { return std::count_if(std::begin(*this), std::end(*this), fcn); }
  };
  ...
  DECLARE_ARGUMENT(IdBranches,
                   std::vector<std::string>); /// Branches containing PID information, used if the names of particles are incorrect (looking at you, DTF)
