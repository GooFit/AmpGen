...
template <typename PDF> FitResult *doFit(PDF &&pdf, EventList_type &data, EventList_type &mc, MinuitParameterSet &MPS)
{
  ...
  for_each(pdf.pdfs(), [&](auto &f) {
    mc.transform([&f](auto &mcevt) { mcevt.setWeight(f.getValNoCache(mcevt) * mcevt.weight() / mcevt.genPdf()); })
      .tree(counter > 1 ? "MCt" + std::to_string(counter) : "MCt")
      ->Write();
