...
MinuitParameterSet CombinedFit(EventType eventType, std::vector<EventList> TagData,
...
void CombinedFitAndWrite(EventType eventType, 
              std::vector<EventList> BSigData, std::vector<int> BgammaSigns, std::vector<bool> BuseXYs, std::vector<bool> BConj, 
...
//    sig.setMC(Int);
  ...
  */
  ...
  bool useCache = NamedParameter<bool>("useCache", false);
  if (useCache){
...
//                            MPS, seed, NInt);
 ...
 //   INFO("Done two fit!");
    ...
    CombinedFitAndWrite(eventType, BSigData, BgammaSigns, BuseXYs,  BConj, MPS,  seed,  NInt_Final,  plotFile,  logFile,
             BTags, fBins, doProjections, doScan);
