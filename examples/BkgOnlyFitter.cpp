...
int main(int argc, char *argv[])
{
  ...
  [[maybe_unused]] size_t nThreads = NamedParameter<size_t>("nCores", 8, "Number of threads to use");
  ...
  */
  ...
  /* Generate events to normalise the PDF with. This can also be loaded from a file,
     ...
     is 5 million. */
  EventList_type eventsMC = intFile == "" ? Generator<>(evtType, &rndm).generate(1.0e6) : EventList_type(intFile, evtType, GetGenPdf(true));
...
template <typename likelihoodType> FitResult *doFit(likelihoodType &&likelihood, EventList_type &data, EventList_type &mc, MinuitParameterSet &MPS)
{
  ...
  INFO("chi2/ndf = " << chi2.chi2() / (chi2.nBins() - 1.));
