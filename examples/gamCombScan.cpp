...
//    INFO("totalLL = "<<totalLL.getVal());
   ...
   // combMini.prepare();
    ...
    auto p1 = MPS[NamedParameter<std::string>("scan_x","pCorrrelatedSum::C00")];
    auto p2 = MPS[NamedParameter<std::string>("scan_y","pCorrrelatedSum::C01")];
    int nsigma = NamedParameter<int>("nSigma", 3);
    int N = NamedParameter<int>("nPoints", 10);
    std::ofstream scanOut;
    scanOut.open(NamedParameter<std::string>("scanOutput", "gamCombScan.txt"));
    for (int i =0; i < N; i++){
      for (int j =0; j<N;j++){
        double x = p1->mean() - p1->err()*nsigma + 2*p1->err()*nsigma*i/N;
        double y = p2->mean() - p2->err()*nsigma + 2*p2->err()*nsigma*j/N;
        MPS[p1->name()]->setCurrentFitVal(x);
        MPS[p2->name()]->setCurrentFitVal(y);
        double z = combMini.FCN();
        scanOut<<x<<"\t"<<y<<"\t"<<z<<"\n";
      }
    }
    scanOut.close();
