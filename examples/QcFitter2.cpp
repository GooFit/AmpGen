...
template <typename PDF>
FitResult* Fit( PDF&& ll, CorrelatedSum& cs, EventList data1, EventList data2, EventList mc1, EventList mc2, MinuitParameterSet& MPS , std::string logFile,   std::map<std::string, std::vector<double> > inits );
...
CorrelatedSum makeCs(std::map<std::string, EventList> events, MinuitParameterSet& MPS);
CorrelatedLL<EventList, CorrelatedSum&> tagLL(std::map<std::string, EventList> events, MinuitParameterSet& MPS);
...
MinuitParameterSet * MPS_tag = new MinuitParameterSet();
  ...
  if (makeCPConj){
    ...
    if (doTagFit) { 
      auto cs_tag = CorrelatedSum(sigevents_tag.eventType(), tagevents_tag.eventType(), *MPS_tag);
...
//      totalLL.add(LL_tag2);
  ...
  if (doCombFit){
    ...
    auto combLL2 = SumLL<CorrelatedLL<EventList, CorrelatedSum&>>(totalLL);
...
INFO("Test function");
    ...
    CorrelatedLL<EventList, CorrelatedSum&> csLL =  make_likelihood(sigEvents, tagEvents, cs);
