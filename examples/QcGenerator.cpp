...
public:
  FixedLibPdf() = default;
  FixedLibPdf(const EventType &type, MinuitParameterSet &) : FixedLibPdf(Property<std::string>(this, type.decayDescriptor() + "::lib"))
  {
...
{
public:
  DTYieldCalculator(const double &productionCrossSection = 3260) : productionCrossSection(productionCrossSection) {}

  double operator()(const double &lumi, const AmpGen::EventType &t_signal, const AmpGen::EventType &t_tag, const bool &print = false);
...
private:
  ...
  bool m_ignoreQc = {Property<bool>(this, "IgnoreQC", false)};
  size_t m_blockSize = {1000000};

public:
  template <class T> std::tuple<double, double, double> z(T &t1, T &t2, const EventType &type) const
  {
...
#pragma omp parallel for reduction(+ : zR, zI, n1, n2)
  ...
  Psi3770(ModelStore &models, const EventType &signalType, const EventType &tagType)
      : m_signalType(signalType), m_tagType(tagType), m_signal(models.find<T1>(m_signalType)), m_signalBar(models.find<T1>(m_signalType.conj(true))),
        m_tag(models.find<T2>(m_tagType)), m_tagBar(models.find<T2>(m_tagType.conj(true))), m_signalPhsp(m_signalType), m_tagPhsp(m_tagType),
  ...
  double prob_noQC(const DTEvent &event) const
  {
    return std::norm(m_signal(event.signal) * m_tagBar(event.tag)) + std::norm(m_signalBar(event.signal) * m_tag(event.tag));
  }
  ...
  DTEventList generate(const size_t &N)
  {
    ...
    while(output.size() < N)
      {
        ...
        for(auto &event : events)
          {
            if(event.prob > norm * gRandom->Uniform())
    ...
    auto beta = [](const Event &event, const size_t &j) {
      return sqrt(event[4 * j + 0] * event[4 * j + 0] + event[4 * j + 1] * event[4 * j + 1] + event[4 * j + 2] * event[4 * j + 2]) / event[4 * j + 3];
    };
  ...
  DTEventList generatePHSP()
  {
    ...
    for(size_t x = 0; x < m_blockSize; ++x)
      output.emplace_back(generatePhaseSpace());
#pragma omp parallel for
    for(size_t i = 0; i < m_blockSize; ++i)
      output[i].prob = P(output[i]);
    return output;
  }
  double rho()
  {
...
int main(int argc, char **argv)
{
  ...
  size_t nThreads = Property<size_t>(nullptr, "nCores", hwt, "Number of threads to use");
  double luminosity = Property<double>(nullptr, "Luminosity", 818.3, "Luminosity to generate. Defaults to CLEO-c integrated luminosity.");
  size_t nEvents
    = Property<size_t>(nullptr, "nEvents", 0, "Can also generate a fixed number of events per tag, if unspecified use the CLEO-c integrated luminosity.");
  size_t seed = Property<size_t>(nullptr, "Seed", 0, "Random seed to use.");
  bool poissonYield = Property<bool>(nullptr, "PoissonYield", true, "Flag to include Poisson fluctuations in expected yields (only if nEvents is not specified)");
  double crossSection = Property<double>(nullptr, "CrossSection", 3.26 * 1000, "Cross section for e⁺e⁻ → Ψ(3770) → DD'");
  std::string output = Property<std::string>(nullptr, "Output", "ToyMC.root", "File containing output events");
  auto pNames = Property<strings>(nullptr, "EventType", {}, "EventType to generate, in the format: \033[3m parent daughter1 daughter2 ... \033[0m"); 
  auto tags   = Property<strings>(nullptr, "TagTypes", {}, "Vector of opposite side tags to generate, in the format \033[3m outputTreeName decayDescriptor \033[0m."); 
  ...
  for(auto &tag : tags.value())
    {
      ...
      std::string flib = Property<std::string>(nullptr, type.decayDescriptor() + "::lib", "");
      if(flib == "")
        {
          auto generator = Psi3770<CoherentSum, CoherentSum>(models, signalType, type);
          double rho = generator.rho();
          double yield = nEvents;
          if(nEvents == 0 && poissonYield)
            yield = gRandom->Poisson(yield_noQC * rho);
          if(nEvents == 0 && !poissonYield)
            yield = yield_noQC * rho;
      ...
      if(name.find("::") != std::string::npos)
        {
          ...
          if(props != 0)
            new_name = props->anti().name() + name.substr(pos);
        }
      else
        {
          ...
          if(reOrIm == "Re" || reOrIm == "Im")
          ...
          else if(tokens.size() == 2)
            {
              ...
              if(props != 0)
                new_name = props->anti().name() + "_" + tokens[1];
            }
        }
...
{
  std::vector<std::string> things = AmpGen::Property<strings>(nullptr, name); 
...
double DTYieldCalculator::operator()(const double &lumi, const AmpGen::EventType &t_signal, const AmpGen::EventType &t_tag, const bool &print)
{
  auto statisticalFactor = 2;
  if(t_signal == t_tag || t_signal == t_tag.conj(false, true))
    statisticalFactor = 1;
  ...
  auto eff = [this](const std::string &tag) -> double {
    auto it = efficiencies.find(tag);
  ...
  if(print)
    {
      ...
      INFO("BR          = " << branchingRatios[signal] << " x " << branchingRatios[tagBar] << " + " << branchingRatios[signalBar] << " x "
                            << branchingRatios[tag] << " = " << br);
...
std::string DTEventList::particleName(const AmpGen::EventType &type, const size_t &j)
{
  ...
  if(count.second == 1)
    return programatic_name(type[j]);
...
TTree *DTEventList::tree(const std::string &name)
{
  ...
  std::vector<int> id_sig(m_sigType.size()), ids_sig(m_sigType.size()), id_tag(m_tagType.size()), ids_tag(m_tagType.size());
  ...
  bool sym = Property<bool>(nullptr, "symmetrise", true);
  for(auto &evt : *this)
    {
