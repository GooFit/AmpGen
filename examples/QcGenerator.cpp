    ...
    while(output.size() < N) {
      ...
      if(norm == -1) {
        for(auto &event : events)
          if(event.prob > norm) norm = event.prob;
        norm *= 1.5;
...
int main(int argc, char **argv) {
  ...
  bool poissonYield
    = Property<bool>(nullptr, "PoissonYield", true, "Flag to include Poisson fluctuations in expected yields (only if nEvents is not specified)");
  ...
  auto tags
    = Property<strings>(nullptr, "TagTypes", {}, "Vector of opposite side tags to generate, in the format \033[3m outputTreeName decayDescriptor \033[0m.");
...
double DTYieldCalculator::operator()(const double &lumi, const AmpGen::EventType &t_signal, const AmpGen::EventType &t_tag, const bool &print) {
  ...
  if(print) {
    ...
    INFO("BR          = " << branchingRatios[signal] << " x " << branchingRatios[tagBar] << " + " << branchingRatios[signalBar] << " x " << branchingRatios[tag]
                          << " = " << br);
...
TTree *DTEventList::tree(const std::string &name) {
  ...
  for(auto &evt : *this) {
    ...
    for(size_t i = 0; i != m_sigType.size(); ++i) id_sig[i] = swap ? -ids_sig[i] : ids_sig[i];
    for(size_t i = 0; i != m_tagType.size(); ++i) id_tag[i] = swap ? -ids_tag[i] : ids_tag[i];
    outputTree->Fill();
  }
