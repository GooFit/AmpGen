...
int main(int argc, char *argv[]) {
  ...
  Property<std::string> motherID{nullptr, "MotherIDBranch", "",
                                 "Name of branch that contains the ID of the parent, i.e. > 0 for particles, < 0 for antiparticles."};
  ...
  for(auto const &particle : particles.value())
    for(auto const &bf : branchFormat.value()) branches.push_back(mysprintf(bf, particle.c_str()));
  ...
  if(f == nullptr) FATAL(inputFilename.value() + " not found");
  if(in_tree == nullptr) FATAL(treeName.value() + " not found");

  INFO("Got tree " << inputFilename << ":" << treeName);
  std::string cut = "";
  for(auto const &c : cuts.value()) cut += c;
  INFO("Using cut = " << cut);
  ...
  if(rejectMultipleCandidates) {
    ...
    for(int i = 0; i < elist->GetN(); ++i) { eventsToTake.push_back(elist->GetEntry(i)); }
  }
  ...
  if(usePIDCalib) {
    ...
    for(unsigned int i = 0; i < eventsToTake.size(); ++i) {
      ...
      for(auto &t : trees) {
        if(evt > t->GetEntries()) { ERROR("Accessing out of bounds data - something has gone HORRIBLY wrong"); }
        t->GetEntry(evt);
      }
      for(auto &w : weights) {
        if(w < 0) {
