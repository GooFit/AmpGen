...
int main(int argc, char **argv)
{
  ...
  Property<bool> conj           {nullptr, "Conj", false, "Flag to generate the CP conjugate amplitude under the assumption of CP conservation"};
  Property<bool> addCPConjugate {nullptr, "AddConj", false, "Flag to add all of the CP conjugate amplitudes, under the assumption of CP conservation"};
  ...
  Property<unsigned> nCores {nullptr, "nCores", concurentThreadsSupported, "Number of cores to use (OpenMP only)"};
  ...
  if(poissonVaryYield)
    nEvents.set( rand.Poisson(nEvents) );
  ...
  if(decay != "")
    ...
    eventType = EventType(eventType_s, generateTD ); 
  ...
  if(accepted.size() == 0)
    return -1;
  TFile *f = TFile::Open(outfile.value().c_str(), "RECREATE");
