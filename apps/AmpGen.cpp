      ...
      for(auto &evt : normEvents)
        {
          ...
          double n = 0;
          if constexpr(std::is_same<T, CoherentSum>::value)
            n = std::norm(pdf.getValNoCache(evt));
          if constexpr(std::is_same<T, PolarisedSum>::value)
            n = pdf.getValNoCache(evt);
          if(n > pMax)
            pMax = n;
        }
...
int main(int argc, char **argv)
{
  ...
  size_t nEvents = Property<size_t>(nullptr, "nEvents", 1, "Total number of events to generate");
  size_t blockSize = Property<size_t>(nullptr, "BlockSize", 5000000, "Number of events to generate per block");
  int seed = Property<int>(nullptr, "Seed", 0, "Random seed used in event Generation. Should always be set for pseudoexperiment generation.");
  std::string outfile = Property<std::string>(nullptr, "Output", "Generate_Output.root", "Name of output file");
  auto pdfType = Property<pdfTypes>(nullptr, 
    "Type", pdfTypes::CoherentSum,
  ...
  auto phspType = Property<phspTypes>(nullptr, 
    "PhaseSpace", phspTypes::PhaseSpace,
    helpStringOptions(
  ...
  std::string lib = Property<std::string>(nullptr, "Library", "", "Name of library to use for a fixed library generation");
  size_t nBins = Property<size_t>(nullptr, "nBins", 100, "Number of bins for monitoring plots.");
  auto ext = *split(outfile, '.').rbegin();
  bool sourceOnly = Property<bool>(nullptr, "SourceOnly", ext == "root" ? false : true, "Flag to only generate the source code, but not produce any events");
  bool poissonVaryYield = Property<bool>(nullptr, "PoissonYield", false, "Vary the number of events generated by a poisson distribution");
  bool conj = Property<bool>(nullptr, "Conj", false, "Flag to generate the CP conjugate amplitude under the assumption of CP conservation");
  bool add_cp_conjugate = Property<bool>(nullptr, "AddConj", false, "Flag to add all of the CP conjugate amplitudes, under the assumption of CP conservation");

#ifdef _OPENMP
  unsigned int concurentThreadsSupported = std::thread::hardware_concurrency();
  unsigned int nCores = Property<unsigned int>(nullptr, "nCores", concurentThreadsSupported, "Number of cores to use (OpenMP only)");
  ...
  if(poissonVaryYield)
    nEvents = rand.Poisson(nEvents);
  MinuitParameterSet MPS;
  MPS.loadFromStream();

  EventType eventType;
  std::string decay = Property<std::string>(nullptr, "Decay", "", "Single decay written on the command line");
  if(decay != "")
    {
      Particle p(decay);
  ...
  */
  ...
  for(auto &plot : plots)
    plot->Write();
  if(Property<bool>(nullptr, "plots_2d", true) == true)
    {
