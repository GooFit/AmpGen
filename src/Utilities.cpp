...
std::vector<std::string> AmpGen::split(const std::string &s, const std::vector<char> &delims, bool keepDelims) {
  ...
  std::vector<std::string> elems;
  std::stringstream ss( s );
  std::string strDelim = std::accumulate( delims.begin(), delims.end(),
  std::string("") ); std::string line; while ( std::getline( ss, line ) ) {
    std::size_t prev = 0, pos;
    while ( ( pos = line.find_first_of( strDelim, prev ) ) != std::string::npos
  ) { if ( pos > prev ) elems.push_back( line.substr( prev, pos - prev ) ); prev
  = pos + 1;
    }
    if ( prev < line.length() ) elems.push_back( line.substr( prev,
  std::string::npos ) );
  }
  return elems;
  */
...
unsigned int AmpGen::editDistance(const std::string &s1, const std::string &s2) {
  ...
  for(unsigned int i = 1; i <= len1; ++i)
    for(unsigned int j = 1; j <= len2; ++j) d[i][j] = std::min({d[i - 1][j] + 1, d[i][j - 1] + 1, d[i - 1][j - 1] + (s1[i - 1] == s2[j - 1] ? 0 : 1)});
  return d[len1][len2];
}

std::string AmpGen::round(const double &number, const unsigned int &nsf) {
  double value = std::round(number * pow(10, nsf)) / pow(10, nsf);
  return nsf != 0 ? mysprintf(("%." + std::to_string(nsf) + "f").c_str(), value) : std::to_string((int)value);
}
...
bool AmpGen::stringMatchesWildcard(const std::string &input, const std::string &wildcard_string, const char wildcard_character) {
  ...
  if(wildcard_string.size() == 1 && wildcard_string[0] == wildcard_character) { return true; }
  if(pos == std::string::npos) { return input == wildcard_string; }
...
void AmpGen::printSplash() {
  ...
  std::cout << "   ╚═╝  ╚═╝╚═╝     ╚═╝╚═╝      ╚═════╝ ╚══════╝╚═╝  ╚═══╝" << std::endl;
  std::cout << "\033[0m\n";
  std::cout << bold_on << "                      version " << AMPGEN_MAJOR_VERSION << "." << AMPGEN_MINOR_VERSION << std::endl;
  std::cout << "        build: ";
  ...
  for(auto &top_path : top_paths) {
    ...
    if((dir = opendir(top_path.c_str())) != nullptr) {
      ...
      while((ent = readdir(dir)) != nullptr) {
        ...
        if(patternString == "" || stringMatchesWildcard(name, patternString)) files.push_back(top_path + "/" + name);
      }
      closedir(dir);
    } else {
...
template <> double AmpGen::lexical_cast<double>(const std::string &word, bool &status) {
  char *p;
...
template <> int AmpGen::lexical_cast<int>(const std::string &word, bool &status) {
  char *p;
  auto number = strtol(word.c_str(), &p, 10);
  status &= *p == 0;
  return number;
}
template <> unsigned int AmpGen::lexical_cast<unsigned int>(const std::string &word, bool &status) {
  char *p;
...
template <> std::string AmpGen::lexical_cast<std::string>(const std::string &word, bool &status) { return word; }

template <> float AmpGen::lexical_cast<float>(const std::string &word, bool &status) {
  char *p;
...
template <> bool AmpGen::lexical_cast<bool>(const std::string &word, bool &status) {
  bool value = false;
  if(word == "1" || word == "true") {
    value = true;
...
template <> long int AmpGen::lexical_cast<long int>(const std::string &word, bool &status) {
  char *p;
  auto number = strtol(word.c_str(), &p, 10);
  status &= *p == 0;
  return number;
...
template <> unsigned long int AmpGen::lexical_cast<unsigned long int>(const std::string &word, bool &status) {
  char *p;
  auto number = strtoul(word.c_str(), &p, 10);
  status &= *p == 0;
  return number;
