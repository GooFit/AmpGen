...
MinuitParameter *
MinuitParameterSet::add(const std::string &name, const Flag &flag, const double &mean, const double &sigma, const double &min, const double &max)
{
...
void MinuitParameterSet::printVariable(std::ostream &os) const
{
  for(size_t i = 0; i < size(); i++)
    {
      if(m_parameters[i]->flag() == Flag::Hide)
        continue; /// hide parameter
      os << '\n' << *m_parameters[i];
    }
}
  ...
  if(line.size() == 7 || line.size() == 11)
    {
      ...
      if(flag_re == Flag::Invalid || flag_im == Flag::Invalid)
        return;
      if(OptionsParser::printHelp())
        {
          INFO("MINUIT: Complex " << line[0] << "_Re ( " << to_string<Flag>(flag_re) << ") = " << mean_re << ", step=" << step_re << " (" << min_re << ","
                                  << max_re << ")");
          INFO("MINUIT: Complex " << line[0] << "_Im ( " << to_string<Flag>(flag_im) << ") = " << mean_im << ", step=" << step_im << " (" << min_im << ","
                                  << max_im << ")");
        }
  ...
  for(const auto &tokens : ppfl)
    {
      ...
      if(tokens.size() >= 3 && tokens[1] == "=")
        protoAliases.push_back(tokens);
      else if(tokens[0].find("=") != std::string::npos && !Particle::isValidDecayDescriptor(tokens[0]))
        {
          auto expanded = split(tokens[0], '=');
          WARNING(tokens[0] << " could be an expression, but not separated "
                               "with white space. Did you mean: "
                            << expanded[0] << " = " << expanded[1] << " ... ?");
        }
    }
  for(const auto &alias : protoAliases)
    tryAlias(alias);

  /// adds default CP conjugates for masses, widths and radii if they have not
  ...
  for(auto &param : *this)
    {
      ...
      if(otherValue != nullptr)
        param->setCurrentFitVal(otherValue->mean());
    }
}
...
void MinuitParameterSet::setFromMinuit(const double *xx)
{
  for(unsigned i = 0; i < m_mapping.size(); ++i)
    at(m_mapping[i])->setCurrentFitVal(xx[i]);
}
...
void MinuitParameterSet::setMapping(const std::vector<unsigned> &m)
{
  m_mapping = m;
  for(unsigned i = 0; i < m_mapping.size(); ++i)
    at(m_mapping[i])->setMinuitIndex(i);
}
