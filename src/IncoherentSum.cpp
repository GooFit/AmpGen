...
IncoherentSum::IncoherentSum(const EventType &finalStates, const MinuitParameterSet &mps, const std::string &prefix) : CoherentSum(finalStates, mps, prefix) {
  // m_normalisations.resize( size(), 1 );
}

double IncoherentSum::norm() const { return norm(m_normalisations); }
...
std::vector<FitFraction> IncoherentSum::fitFractions(const LinearErrorPropagator &linProp) {
  ...
  for(unsigned int i = 0; i < m_matrixElements.size(); ++i) calc.emplace_back(m_matrixElements[i].decayDescriptor(), std::vector<size_t>({i}));
...
double IncoherentSum::prob(const Event &evt) const { return m_weight * prob_unnormalised(evt) / m_norm; }
...
real_t IncoherentSum::prob_unnormalisedNoCache(const Event &evt) const {
  return utils::get<0>(real_t(std::accumulate(m_matrixElements.begin(), m_matrixElements.end(), real_t(0), [&evt](const auto &a, const auto &b) {
    auto amp = utils::at(b(evt)[0], 0);
...
#pragma omp parallel for
  ...
  for(unsigned int block = 0; block < events->nBlocks(); ++block) {
    real_v amp(0.);
    for(unsigned j = 0; j != m_matrixElements.size(); ++j) amp += utils::norm(complex_v(m_matrixElements[j].coefficient) * store(block, j));
