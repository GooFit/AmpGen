...
CompiledExpressionBase::CompiledExpressionBase(const std::string &name) : m_name(name), m_progName(programatic_name(name)) {}

CompiledExpressionBase::CompiledExpressionBase(const Expression &expression, const std::string &name, const DebugSymbols &db,
                                               const std::map<std::string, unsigned> &evtMapping)
    : m_obj(expression), m_name(name), m_progName(programatic_name(name)), m_db(db), m_evtMap(evtMapping)
{}
...
void CompiledExpressionBase::resolve(const MinuitParameterSet *mps)
{
  if(m_resolver == nullptr)
    m_resolver = std::make_shared<ASTResolver>(m_evtMap, mps);
      ...
      for(auto &it : expressions_for_this)
        {
          ...
          if(!std::any_of(m_debugSubexpressions.begin(), m_debugSubexpressions.end(), is_same))
            m_debugSubexpressions.push_back(it);
        }
    }
  ...
  if(m_includePythonBindings)
    {
      ...
      std::string src = "&tmp";
      if(rt.find("vector") != std::string::npos)
        src = "tmp.data()";
      if(m_includeParameters)
        {
          std::string fwd_args = "x0";
          for(unsigned i = 1; i < argList.size() - 1; ++i)
            fwd_args += ", x" + std::to_string(i);
...
void CompiledExpressionBase::compile(const std::string &fname) { CompilerWrapper().compile(*this, fname); }

void CompiledExpressionBase::addDebug(std::ostream &stream) const
{
  stream << "#include<string>\n";
  stream << "extern \"C\" std::vector<std::pair< std::string, " << type_string<complex_v>() << " >> " << m_progName << "_DB(" << fcnSignature() << "){\n";
  ...
  for(unsigned int i = 0; i < m_db.size(); ++i)
    {
      ...
      if(expression.to_string(m_resolver.get()) != "NULL")
        ...
        stream << type_string<complex_v>() << "(-999.,0.)}" << comma;
    }
}
...
std::string CompiledExpressionBase::fcnSignature(const std::vector<std::string> &argList) const
{
  ...
  auto fcn = [counter](const auto &str) mutable { return str + " x" + std::to_string(counter++); };
  if(rto)
    return argList[0] + " r, " + argList[1] + " s, " + vectorToString(argList.begin() + 2, argList.end(), ", ", fcn);
...
std::vector<const CacheTransfer *> CompiledExpressionBase::orderedCacheFunctors() const
{
  ...
  for(const auto &c : m_cacheTransfers)
    ordered_cache_functors.push_back(c.get());
  std::sort(ordered_cache_functors.begin(), ordered_cache_functors.end(), [](auto &c1, auto &c2) { return c1->address() < c2->address(); });
...
void CompiledExpressionBase::compileBatch(std::ostream &stream) const
{
...
#endif
  stream << "extern \"C\" void " << progName() << "_batch(";
  ...
  if(use_rto())
    stream << progName() + "( r + cacheSize * i, s, x0, x1 +  i * eventSize);";
  else
