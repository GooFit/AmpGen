  ...
  if(m_cxx == "")
    {
      ...
      if(m_cxx == "")
        ERROR("No configured compiler; set global variable AMPGEN_CXX");
#endif
    }
  else
    {
...
bool CompilerWrapper::compile(CompiledExpressionBase &expression, const std::string &fname)
{
  ...
  if(!m_forceRebuild && fileSize(oname) != -1 && expression.link(oname))
    return true;
  if(print_all)
    INFO("Generating source: " << cname);
                              ...
                              const std::map<std::string, std::string> &metadata_functions)
{
  ...
  for(const auto &expression : expressions)
    output << *expression << "\n";
  for(const auto &[k, v] : metadata_functions)
    output << "extern \"C\" const char* " << k << "(){ return \"" << v << "\";}\n";
  ...
  double tWall = std::chrono::duration<double, std::milli>(twall_end - twall_begin).count();
  if(print_all)
    INFO(cname << " Compile time = " << tWall / 1000. << " [size = " << fileSize(cname) / 1024 << ", " << fileSize(oname) / 1024 << "] kB");
  return true;
}
