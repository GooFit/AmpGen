...
CompilerWrapper::CompilerWrapper() : m_cxx(getenv("AMPGEN_CXX") != nullptr ? std::string(getenv("AMPGEN_CXX")) : "") {
  if(m_cxx == "") {
#ifdef AMPGEN_CXX
    if(m_verbose)
      INFO("Global variable AMPGEN_CXX is undefined, using compiler "
           "variable AMPGEN_CXX: "
           << AMPGEN_CXX);
    m_cxx = AMPGEN_CXX;
#else
    ...
    if(m_verbose) INFO("Global variable CXX is defined: " << m_cxx << "; AMPGEN_CXX is ignored"); // to use AMPGEN_CXX, unset CXX
  }
}
...
void CompilerWrapper::generateSource(const CompiledExpressionBase &expression, const std::string &filename) {
  ...
  if(signature.find("Complex") != std::string::npos) {
    output << "#include \"AmpGen/Complex.h\"\n";
    output << "using namespace AmpGen;\n";
  }
  if(signature.find("AVX2d") != std::string::npos)
    output << "#include \"AmpGen/simd/avx2d_types.h\"\n; using namespace "
              "AmpGen::AVX2d;\n";
  else if(signature.find("AVX2f") != std::string::npos)
    output << "#include \"AmpGen/simd/avx2f_types.h\"\n; using namespace "
              "AmpGen::AVX2f;\n;";
  else if(signature.find("AVX512d") != std::string::npos)
    output << "#include \"AmpGen/simd/avx512d_types.h\"\n; using namespace "
              "AmpGen::AVX512d;\n;";
  else if(signature.find("AVX512") != std::string::npos)
    output << "#include \"AmpGen/simd/avx512_types.h\"\n; using namespace "
              "AmpGen::AVX512;\n;";
  else if(signature.find("ARM128d") != std::string::npos)
    output << "#include \"AmpGen/simd/arm128d_types.h\"\n; using namespace "
              "AmpGen::ARM128d;\n;";
...
std::string CompilerWrapper::generateFilename() {
  ...
  if(status == -1) { ERROR("Failed to generate temporary filename " << status); }
  std::remove(buffer);
  return buffer;
}
...
bool CompilerWrapper::compile(CompiledExpressionBase &expression, const std::string &fname) {
  ...
  if(m_verbose) INFO("Generating source: " << cname);
  ...
  if(m_verbose)
    INFO(expression.name() << " " << cname << " Compile time = " << tWall / 1000. << " [size = " << fileSize(cname) / 1024 << ", " << fileSize(oname) / 1024
                           << "] kB");
  return true;
}
...
bool CompilerWrapper::compile(std::vector<CompiledExpressionBase *> &expressions, const std::string &fname,
  ...
  std::string cname = fname == "" ? generateFilename() : fname;
  std::string oname = std::filesystem::path(cname).replace_extension(m_extension);
  if(m_verbose) INFO("Generating source: " << cname);
  auto twall_begin = std::chrono::high_resolution_clock::now();
  ...
  auto twall_end = std::chrono::high_resolution_clock::now();
  double tWall = std::chrono::duration<double, std::milli>(twall_end - twall_begin).count();
  if(m_verbose) INFO(cname << " Compile time = " << tWall / 1000. << " [size = " << fileSize(cname) / 1024 << ", " << fileSize(oname) / 1024 << "] kB");
  return true;
}

bool CompilerWrapper::isClang() const { return m_cxx.find("clang") != std::string::npos || m_cxx.find("llvm-g++") != std::string::npos; }
...
void CompilerWrapper::compileSource(const std::string &fname, const std::string &oname) {
  ...
  if(m_verbose) {
    std::string result = std::accumulate(std::begin(argp), std::end(argp), std::string(), [](const std::string &a, const char *b) { return a + " " + b; });
  ...
  if(childPID == 0) {
    ...
    perror("execv() in ../src/CompilerWrapper.cpp"); // add
                                                     // "CompilerWrapper::Verbose
                                                     // true" to .opt file to
                                                     // diagnose
    exit(0);
  }
