...
Coupling::Coupling(MinuitParameter *re, MinuitParameter *im) : m_re(re), m_im(im) {
  ...
  if(m_coord == coordinateType::Invalid) FATAL("Coordinates for coupling constants must be either cartesian or polar");
  if(m_angUnit == angType::Invalid) FATAL("TotalCoupling::AngularUnits must be either rad or deg");
