      ...
      if(name.find("_Re") != std::string::npos)
        {
          ...
          if(!Particle::isValidDecayDescriptor(name.substr(0, name.find("_Re"))))
            continue;
...
{
  if(!hasDecay(head))
    return std::vector<Coupling>();
  if(prefix == "")
    return m_rules.find(head)->second;
...
Expression TotalCoupling::to_expression() const
{
  return std::accumulate(couplings.begin(), couplings.end(), Expression(1),
                         [](const auto &prod, const auto &coupling) { return prod * coupling.to_expression(); });
}

void TotalCoupling::print() const
{
  INFO(this->operator()() << " -> ");
  for(const auto &coupling : *this)
    INFO(coupling.to_expression() << " = " << coupling());
}
      ...
      for(const auto &[particle, coupling] : expanded)
        {
          ...
          if(p.isStateGood() && std::find_if(rt.begin(), rt.end(), [p](const auto &x) { return x.first.decayDescriptor() == p.decayDescriptor(); }) == rt.end())
  ...
  do
    {
      ...
      for(const auto &[particle, coupling] : rt)
        {
          if(!canExpand(particle))
            {
              newTmpParticles.emplace_back(particle, coupling);
              continue;
            }
