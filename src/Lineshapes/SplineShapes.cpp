...
DEFINE_LINESHAPE(GSpline)
{
  ...
  if(useDispersiveTerm)
    real_part = real_part + getSpline(particleName, sInGeV, "dm2", dbexpressions, true) - getSpline(particleName, mass * mass, "dm2", dbexpressions, true);
...
DEFINE_LINESHAPE(FormFactorSpline)
{
  ...
  const Expression BW = GeV * getSpline(particleName, s, "FF") / (-s + mass * mass - Constant(0, 1) * mass * runningWidth);
  return BW;
}
