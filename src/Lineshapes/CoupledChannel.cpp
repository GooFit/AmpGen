...
template <typename T> T rho_threeBody(const T &s, const Particle &resonance, const Particle &bachelor)
{
  ...
  return Ternary(s23_max > sT, Imag(z * log((s + s1 - HsT - sT) / (2 * sqrt(s * s1))) - Hz * log(K / (2. * sqrt(s * s1) * (sT - z)))) / (16. * M_PI * M_PI * s),
                 0);
}
...
namespace
{
  ...
  template <unsigned L, typename expression_type> expression_type blatt_weisskopf_sq(const expression_type &z)
  {
    if constexpr(L == 0)
      ...
      return expression_type(2 * z / (1 + z));
  ...
  template <unsigned N> Expression rho_pade(const Expression &s, const Particle &p)
  {
    ...
    auto CM_num = analytic_continuation(
      [=](const double &s) { return blatt_weisskopf_sq<N>(q2(s, m1 * m1, m2 * m2) * R * R) * sqrt(q2(s, m1 * m1, m2 * m2) / s) / (8 * M_PI); }, s_thresh);
    Pade<4, double> cm_pade_lo(real(CM_num), 0.0, s_thresh);
    Pade<4, double> cm_pade_hi(real(CM_num), s_thresh, 5);
    return Ternary(s >= s_thresh, cm_pade_hi(s) + 1i * blatt_weisskopf_sq<N>(q2(s, m1 * m1, m2 * m2) * R * R) * 2 * sqrt(Q2(s, m1 * m1, m2 * m2) / s),
                   cm_pade_lo(s))
           / 1i;
  }
}
