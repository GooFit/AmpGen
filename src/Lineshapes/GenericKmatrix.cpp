...
namespace AmpGen
{
  make_enum(PA_TYPE, PVec, QVec, ZVec);
}

namespace
{
  unsigned find_channel_index(const Particle &particle, const std::vector<Particle> &channels)
  {
    for(unsigned int i = 0; i != channels.size(); ++i)
      {
        auto match_state = particle.matches(channels[i]);
        if(match_state == Particle::MatchState::Exact)
          return i;
      }
...
DEFINE_GENERIC_SHAPE(GenericKmatrix)
{
  ...
  for(unsigned i = 0; i < channels.size(); i += 1)
    {
      ...
      if(dbexpressions != nullptr)
        dbexpressions->emplace_back("phsp_" + p.decayDescriptor(), *phsps.rbegin()); // ADD_DEBUG( *phsps.rbegin(), dbexpressions);
  ...
  for(unsigned pole = 1; pole <= nPoles; ++pole)
    {
      ...
      if(dbexpressions != nullptr)
        dbexpressions->emplace_back(stub + "::mass", mass);
  ...
  // follow https://doi.org/10.1007/s1010502a0002 eqns (9)-(13) modulo the Adler zero term (which is argued away in a fuzzy manner by citing a private
  // communication)
  if(pa_type == PA_TYPE::PVec)
    {
      ...
      for(unsigned k = 0; k < nChannels; ++k)
        a[k] = Parameter(particleName + "::a::" + std::to_string(k + 1));
  ...
  else if(pa_type == PA_TYPE::ZVec)
    {
      ...
      // if( tokens[0] == "pole")
      {
        ...
        if(tokens.size() == 3)
          channel = stoi(tokens[2]) - 1;
        else
          channel = find_channel_index(p, pchannels);
  ...
  else if(pa_type == PA_TYPE::QVec)
    {
      ...
      for(unsigned i = 0; i < nChannels; ++i)
        M = M + kMatrix[{i, 0}] * propagator[{0, i}];
      return M; // * phsps[0];
    }
  else
    throw std::runtime_error("This shouldn't happen. Currently supported types: P-vector approach (PA_TYPE::PVec) and Q-vector approach (PA_TYPE::QVec)");
}
