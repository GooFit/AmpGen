...
Expression AmpGen::gFromGamma(const Expression &m, const Expression &gamma, const Expression &rho) { return fcn::sqrt(m * gamma / rho); }
...
Tensor AmpGen::getPropagator(const Tensor &kMatrix, const std::vector<Expression> &phaseSpace, DebugSymbols *db) {
  ...
  for(unsigned i = 0; i < nChannels; ++i) {
    for(unsigned j = 0; j < nChannels; ++j) { T[{i, j}] = SubTree((i == j ? 1 : 0) - 1i * kMatrix[{i, j}] * phaseSpace[j]); }
  }
...
DEFINE_LINESHAPE(kMatrix) {
  ...
  std::vector<Expression> phaseSpace = {phsp_twoBody(sInGeV, mPiPlus, mPiPlus), phsp_twoBody(sInGeV, mKPlus, mKPlus), phsp_fourPi(sInGeV),
  ...
  if(dbexpressions != nullptr) {
    ...
    for(unsigned int i = 0; i < 5; ++i) dbexpressions->emplace_back("phi[" + std::to_string(i) + "]", phaseSpace[i]);
  ...
  if(tokens[0] == "scatt") {
    ...
    Expression M;
    for(unsigned int x = 0; x < nChannels; ++x) { M = M + kMatrix[{x, i}] * F[{j, x}]; }
    return M;
  } else if(tokens[0] == "pole") {
    Expression M;
    DEBUG("Returning pole term");
    auto pole = poleConfigs[pTerm];
    for(unsigned int i = 0; i < pole.couplings.size(); ++i) { M = M + F[{0, i}] * pole.couplings[i]; }
    return SubTree(M / (pole.s - sInGeV));
  } else if(tokens[0] == "poleKK") {
    Expression M;
    DEBUG("Returning pole term");
    auto pole = poleConfigs[pTerm];
    for(unsigned int i = 0; i < pole.couplings.size(); ++i) { M = M + F[{1, i}] * pole.couplings[i]; }
    return SubTree(M / (pole.s - sInGeV));
  }
