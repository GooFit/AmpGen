...
Expression rho_2(const Expression &s, const Expression &s0) { return fcn::complex_sqrt(1.0 - (4 * s0) / s); }
...
DEFINE_LINESHAPE(Bugg)
{
  ...
  Expression gamma_2K = g_2K * g1sg * s / (M * M) * Exp(-alpha * q(s, mKPlus * mKPlus)) * rho_2(s, mKPlus * mKPlus);

  Expression gamma_2eta = g_2eta * g1sg * s / (M * M) * Exp(-alpha * q(s, mEta * mEta)) * rho_2(s, mEta * mEta);
