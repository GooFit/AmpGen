...
DEFINE_LINESHAPE(FormFactor)
{
  ...
  int Lp = L;
  if(lineshapeModifier == "L0")
    Lp = 0;
  if(lineshapeModifier == "L1")
    Lp = 1;
  if(lineshapeModifier == "L2")
    Lp = 2;
  if(lineshapeModifier == "L3")
    Lp = 3;

  Expression FormFactor = sqrt(BlattWeisskopf_Norm(q2 * radius * radius, 0, Lp));
  if(lineshapeModifier == "BL")
    FormFactor = sqrt(BlattWeisskopf(q2 * radius * radius, Lp));
  if(lineshapeModifier == "NFF")
    FormFactor = 1;
  if(lineshapeModifier == "BELLE2018")
    FormFactor = sqrt(BlattWeisskopf_Norm(q2 * radius * radius, q20 * radius * radius, Lp));
...
DEFINE_LINESHAPE(BW)
{
  ...
  if(lineshapeModifier == "BL")
    FormFactor = sqrt(BlattWeisskopf(q2 * radius * radius, L));
  if(lineshapeModifier == "BELLE2018")
    FormFactor = sqrt(BlattWeisskopf_Norm(q2 * radius * radius, q20 * radius * radius, L));
  if(lineshapeModifier == "NFF")
    FormFactor = 1;
