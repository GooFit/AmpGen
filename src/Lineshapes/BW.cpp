...
DEFINE_LINESHAPE( FormFactor )
{
  ...
  const Expression FormFactor = fcn::sqrt( BlattWeisskopf_Norm( q2 * radius * radius, 0, Lp ) );
  if( L != 0 ){
    ADD_DEBUG( fcn::sqrt(s), dbexpressions );
    ADD_DEBUG( fcn::sqrt(s1), dbexpressions );
    ADD_DEBUG( fcn::sqrt(s2), dbexpressions );
...
DEFINE_LINESHAPE( NonRes )
{
  ...
  const Expression q20  = fcn::abs( Q2( mass * mass, s1, s2 ) );
...
Expression aSqrtTerm( const Expression& s, const Expression& m0 )
{
  ...
  return Ternary( a2 > Constant( 0 ), fcn::sqrt( a2 ), Constant( 0 ) );
}

Expression fSqrtTerm( const Expression& s, const Expression& m0 )
{
  return fcn::complex_sqrt( 1.0 - ( 4 * m0 * m0 ) / s );
}
