...
GaussErrorPropagator::GaussErrorPropagator(const TMatrixD &reducedCovariance, const std::vector<MinuitParameter *> &params, TRandom3 *rnd)
  ...
  for(int i = 0; i < m_decomposedCholesky.GetNrows(); ++i) {
    for(int j = i + 1; j < m_decomposedCholesky.GetNrows(); ++j) { std::swap(m_decomposedCholesky(i, j), m_decomposedCholesky(j, i)); }
  }
}
...
LinearErrorPropagator::LinearErrorPropagator(const TMatrixD &reducedCovarianceMatrix, const std::vector<MinuitParameter *> &params)
...
LinearErrorPropagator::LinearErrorPropagator(const std::vector<MinuitParameter *> &params) {
  ...
  for(size_t i = 0; i < m_parameters.size(); ++i) m_cov(i, i) = m_parameters[i]->err() * m_parameters[i]->err();
}
...
double LinearErrorPropagator::getError(const std::function<double(void)> &fcn) const {
  ...
  for(unsigned int i = 0; i < N; ++i) {
    // CHECK_BLINDING
    DEBUG("Perturbing parameter: [" << m_parameters[i]->name() << "] " << m_parameters[i]->mean() << " by " << sqrt(m_cov(i, i)) << " " << m_parameters[i]);
...
std::vector<double> LinearErrorPropagator::getVectorError(const std::function<std::vector<double>(void)> &fcn, size_t RANK) const {
  ...
  for(unsigned int i = 0; i < N; ++i) {
    ...
    for(size_t j = 0; j < RANK; ++j) { errorVec[j](i) = (plus_variation[j] - minus_variation[j]) / (2 * error); }
  }
  fcn();
...
void LinearErrorPropagator::reset() {
  for(int x = 0; x < m_cov.GetNrows(); ++x) {
    for(int y = 0; y < m_cov.GetNrows(); ++y) { m_cov(x, y) = 0; }
  }
  m_parameters.clear();
}
...
TMatrixD LinearErrorPropagator::covarianceMatrix(const std::vector<std::function<double(void)>> &functions) {
  ...
  for(size_t k = 0; k < M; ++k)
    for(size_t i = 0; i < N; ++i) A(k, i) = derivative(functions[k], i);
  ...
  for(size_t i = 0; i < M; ++i) {
    for(size_t j = 0; j < M; ++j) {
      for(size_t k = 0; k < N; ++k) {
        for(size_t l = 0; l < N; ++l) { vci(i, j) += A(i, k) * m_cov(k, l) * A(j, l); }
      }
    }
  }
  return vci;
...
std::pair<double, double> LinearErrorPropagator::combinationCovWeighted(const std::vector<std::function<double(void)>> &functions) {
  ...
  for(size_t i = 0; i < functions.size(); ++i) {
    for(size_t j = 0; j < functions.size(); ++j) { wt += covInverse(i, j); }
  }
  ...
  for(size_t i = 0; i < functions.size(); ++i) {
    for(size_t j = 0; j < functions.size(); ++j) { sigma2 += weights[i] * weights[j] * cov(i, j) / (wt * wt); }
  }
...
std::vector<double> LinearErrorPropagator::combinationWeights(const std::vector<std::function<double(void)>> &estimators) {
  ...
  for(size_t i = 0; i < estimators.size(); ++i) {
    for(size_t j = 0; j < estimators.size(); ++j) { wt += covInverse(i, j); }
  }
...
const std::map<std::string, size_t> LinearErrorPropagator::posMap() const {
  ...
  for(size_t pos = 0; pos != m_parameters.size(); ++pos) { pMap[m_parameters[pos]->name()] = pos; }
  return pMap;
}
...
TMatrixD LinearErrorPropagator::correlationMatrix(const std::vector<std::function<double(void)>> &functions) {
  ...
  for(size_t i = 0; i < functions.size(); ++i) {
    for(size_t j = 0; j < functions.size(); ++j) { cov_matrix(i, j) = cov_matrix(i, j) / sqrt(diag[i] * diag[j]); }
  }
  return cov_matrix;
}
...
NonlinearErrorPropagator::NonlinearErrorPropagator(const std::function<double(void)> &fcn, const TMatrixD &cov,
                                                   const std::vector<MinuitParameter *> &parameters)
    : m_fcn(fcn), m_cov(cov), m_parameters(parameters) {}

template <typename target_t, typename proposal_t> class MetropolisHastings {
public:
  MetropolisHastings(const target_t &target, const proposal_t &proposal, const TVectorD &errors) : target(target), proposal(proposal), errors(errors) {}
...
MetropolisHastings<target_t, proposal_t> make_sampler(const target_t &target, const proposal_t &proposal, const TVectorD &err) {
...
TMatrixD NonlinearErrorPropagator::correlationMatrix(const std::vector<std::function<double(void)>> &functions, TRandom3 *rnd, const unsigned &nSamples) {
  ...
  for(int i = 0; i < decomposedCholesky.GetNrows(); ++i) {
    for(int j = i + 1; j < decomposedCholesky.GetNrows(); ++j) { std::swap(decomposedCholesky(i, j), decomposedCholesky(j, i)); }
  }
