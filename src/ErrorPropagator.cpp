...
void GaussErrorPropagator::reset()
{
  for(unsigned int j = 0; j < m_parameters.size(); ++j)
    m_parameters[j]->setCurrentFitVal(m_startingValues[j]);
}
...
void LinearErrorPropagator::add(const LinearErrorPropagator &p2)
{
  ...
  if(superSet != oldSize)
    m_cov.ResizeTo(superSet, superSet);
  for(size_t x = 0; x != oldSize; ++x)
    {
      for(size_t y = 0; y != oldSize; ++y)
        m_cov(x, y) = old_cov(x, y);
    }
  auto p2_cov = p2.cov();
  for(size_t x = 0; x < p2.size(); ++x)
    {
...
std::vector<double> LinearErrorPropagator::getVectorError(const std::function<std::vector<double>(void)> &fcn, size_t RANK) const
{
  ...
  for(unsigned int j = 0; j < RANK; ++j)
    rt[j] = sqrt(Dot(errorVec[j], m_cov * errorVec[j]));
  return rt;
}
    ...
    for(int i = 0; i != 500; ++i)
      {
        ...
        for(int i = 0; i != N; ++i)
          x_prime(i) = x(i) + errors(i) * gRandom->Gaus();
...
TMatrixD NonlinearErrorPropagator::correlationMatrix(const std::vector<std::function<double(void)>> &functions, TRandom3 *rnd, const unsigned &nSamples)
{
  ...
  for(auto &function : functions)
    function_averages.push_back(function());

  for(auto &param : m_parameters)
    {
      if(param->isFree())
        parameters.emplace_back(param, param->mean());
    }
  double L0 = m_fcn();
  ...
  for(size_t i = 0; i != functions.size(); ++i)
    tree->Branch(("f_" + std::to_string(i)).c_str(), &f_prime[i]);

  for(unsigned int i = 0; i != N; ++i)
    tree->Branch(parameters[i].first->name().c_str(), &parameters_v[i]);
  TVectorD p0(N);
  TVectorD err(N);
  ...
  auto target_distribution = [&](TVectorD &state) {
    for(unsigned int i = 0; i != N; ++i)
      parameters[i].first->setCurrentFitVal(state(i));
  ...
  for(unsigned sample = 0; sample != nSamples; ++sample)
    {
      if(sample % 100 == 0)
        INFO(sample << " / " << nSamples << " completed");
      auto p = mh();
      for(unsigned int i = 0; i != N; ++i)
        parameters[i].first->setCurrentFitVal(p(i));
      for(unsigned int i = 0; i != N; ++i)
        parameters_v[i] = p(i);

      for(unsigned i = 0; i != functions.size(); ++i)
        f_prime[i] = (functions[i]());
