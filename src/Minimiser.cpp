...
void Minimiser::operator()(int i, const ROOT::Minuit2::MinimumState &state) {
  ...
  if(i >= 0) {
    ...
    if(state.HasCovariance() && m_printLevel >= PrintLevel::VeryVerbose) INFO("Error matrix change = " << state.Error().Dcovar());
  } else {
...
void Minimiser::prepare() {
  if(m_printLevel == PrintLevel::Invalid) { FATAL("Requested print level is not valid"); }
...
bool Minimiser::doFit() {
  ...
  if(dynamic_cast<Minuit2::Minuit2Minimizer *>(m_minimiser) != nullptr) dynamic_cast<Minuit2::Minuit2Minimizer *>(m_minimiser)->SetTraceObject(*this);
  ...
  for(size_t i = 0; i < m_nParams; ++i) {
    ...
    for(unsigned int j = 0; j < m_nParams; ++j) { m_covMatrix[i + m_nParams * j] = m_minimiser->CovMatrix(i, j); }
  }
  ...
  if(m_printLevel >= PrintLevel::Info) {
    ...
    for(const auto &param : *m_parSet) {
      ...
      if(param->flag() == Flag::Free or param->flag() == Flag::Fix or param->flag() == Flag::Blind) {
        if(m_runMinos)
          INFO(std::setw(longest_parameter_name) << param->name() << "     " << std::setw(5) << to_string<Flag>(param->flag()) << std::right << std::setw(13)
                                                 << mean << " ± " << std::left << std::setw(13) << (param->isFree() ? param->err() : 0)
                                                 << " Pos err:" << std::setw(11) << (param->isFree() ? param->errPos() : 0) << " Neg err:" << std::setw(11)
                                                 << (param->isFree() ? param->errNeg() : 0));
        else
          INFO(std::setw(longest_parameter_name) << param->name() << "     " << std::setw(5) << to_string<Flag>(param->flag()) << std::right << std::setw(13)
                                                 << mean << " ± " << std::left << std::setw(13) << (param->isFree() ? param->err() : 0));
      }
...
TGraph *Minimiser::scan(MinuitParameter *param, const double &min, const double &max, const double &step) {
  ...
  if(param->isBlind()) { secretoffset = m_parSet->at(param->name() + "_blind")->mean(); }
  //   param->fix();
...
TMatrixTSym<double> Minimiser::covMatrix() const {
  ...
  for(unsigned int i = 0; i < internalPars; i++) {
    for(unsigned int j = 0; j < internalPars; j++) { matrix(i, j) = m_covMatrix[i + j * internalPars]; }
  }
  return matrix;
}
...
TMatrixTSym<double> Minimiser::covMatrixFull() const {
  ...
  for(unsigned int i = 0; i < m_mapping.size(); ++i) {
    for(unsigned int j = 0; j < m_mapping.size(); j++) { matrix(m_mapping[i], m_mapping[j]) = m_covMatrix[i + j * m_mapping.size()]; }
  }
  return matrix;
}
    ...
    public:
      ...
      void set(ROOT::Math::Minimizer *mini, int status, double fcn) {
        ...
        for(unsigned i = 0; i < fNdf; ++i)
          for(unsigned j = 0; j <= i; ++j) fCovMatrix[it++] = mini->CovMatrix(i, j);
