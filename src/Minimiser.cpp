...
void Minimiser::operator()(int i, const ROOT::Minuit2::MinimumState &state)
{
  ...
  if(i >= 0)
    {
      INFO("Iteration  #  " << std::setw(3) << i << " - FCN = " << std::setw(16) << state.Fval() << " Edm = " << std::setw(12) << state.Edm()
                            << " NCalls = " << std::setw(6) << state.NFcn());
      if(state.HasCovariance() && m_printLevel >= PrintLevel::VeryVerbose)
        INFO("Error matrix change = " << state.Error().Dcovar());
    }
  else
    {
      INFO("Iteration  #  " << std::setprecision(13) << " - FCN = " << std::setw(16) << state.Fval() << " Edm = " << std::setw(12) << state.Edm()
                            << " NCalls = " << std::setw(6) << state.NFcn());
      if(state.HasCovariance() && m_printLevel >= PrintLevel::VeryVerbose)
        INFO("Error matrix change = " << state.Error().Dcovar());
    }
  // auto gradient = state.Gradient().Vec().Data();
  // Print parameter values during the minimisation (blind params are shifted
  // by the blinding offset)
          ...
          if(par->isBlind())
            {
              ...
              if(!secretoffset)
                {
                  INFO("Attempting to print a blind parameter without "
                       "secretoffset. Check Minimiser.cpp");
                  continue;
                }
          ...
          else
            {
              ...
              if(par->minInit() != 0 || par->maxInit() != 0)
                v = par->minInit() + 0.5 * (par->maxInit() - par->minInit()) * (std::sin(v) + 1);
...
void Minimiser::prepare()
{
  ...
  if(m_printLevel >= PrintLevel::VeryVerbose && std::any_of(m_parSet->begin(), m_parSet->end(), [](const auto &p) { return p->isBlind(); }))
    {
      FATAL("Minimiser::PrintLevel is !=0, this is incompatible with having "
            "blind parameters.");
    }
...
{
  if(mini->CovMatrixStatus() == -1)
    return "not available (inversion failed or Hesse failed)";
  if(mini->CovMatrixStatus() == 0)
    return "available but not positive defined status";
  if(mini->CovMatrixStatus() == 1)
    return "only approximate";
  if(mini->CovMatrixStatus() == 2)
    return "is full matrix but forced pos def";
  if(mini->CovMatrixStatus() == 3)
    return "is full and accurate";
  return "unknown";
}
...
std::string minuitStatusString(ROOT::Minuit2::Minuit2Minimizer *mini)
{
  if(mini->Status() == 0)
    return "converged";
  if(mini->Status() == 1)
    return "forced positive definite";
  if(mini->Status() == 2)
    return "Hesse is invalid";
  if(mini->Status() == 3)
    return "Edm is above maximum";
  if(mini->Status() == 4)
    return "Reached max calls";
  if(mini->Status() == 5)
    return "unknown";
  return "unknown";
}
...
bool Minimiser::doFit()
{
  ...
  if(m_status != 0 && m_printLevel != PrintLevel::VeryVerbose)
    {
      WARNING("Fit has not converged, some clues from Minuit2 may not be "
              "printed due to low verbosity level.");
  ...
  if(m_printLevel >= PrintLevel::Info)
    {
      ...
      for(const auto &param : *m_parSet)
        {
          if(param->name().size() > longest_parameter_name)
            longest_parameter_name = param->name().size() + 3;
        }
      for(const auto &param : *m_parSet)
        {
              ...
              if(secretoffset == 0.)
                {
                  INFO("\n\n\n Attempting to print a blind result!!! \n\n\n "
                       "Skipping parameter for now, change this in "
                       "Minimiser.cpp");
                  continue;
                }
              mean += secretoffset;
            }
          if(param->flag() == Flag::Free or param->flag() == Flag::Fix or param->flag() == Flag::Blind)
            {
              if(m_runMinos)
                INFO(std::setw(longest_parameter_name)
                     << param->name() << "     " << std::setw(5) << to_string<Flag>(param->flag()) << std::right << std::setw(13) << mean << " ± " << std::left
                     << std::setw(13) << (param->isFree() ? param->err() : 0) << " Pos err:" << std::setw(11) << (param->isFree() ? param->errPos() : 0)
                     << " Neg err:" << std::setw(11) << (param->isFree() ? param->errNeg() : 0));
              else
                INFO(std::setw(longest_parameter_name) << param->name() << "     " << std::setw(5) << to_string<Flag>(param->flag()) << std::right
                                                       << std::setw(13) << mean << " ± " << std::left << std::setw(13) << (param->isFree() ? param->err() : 0));
            }
        }
    }
  return 1;
...
void Minimiser::addExtendedTerm(ExtendLikelihoodBase *m_term) { m_extendedTerms.push_back(m_term); }
...
void Minimiser::minos(MinuitParameter *parameter)
{
  ...
  for(; index != m_nParams; ++index)
    {
      if(parameter->name() == m_parSet->at(m_mapping[index])->name())
        break;
    }
  double v0 = parameter->mean();
  if(index == m_nParams)
    ERROR(parameter->name() << " not amongst free parameters for fit");
  double low{0}, high{0};
  int status = 0;
  ...
  for(int i = 0; i != m_nParams; ++i)
    m_parSet->at(m_mapping[i])->setCurrentFitVal(init_values[i]);
  unsigned longest_parameter_name = 10;
  for(const auto &param : *m_parSet)
    {
      if(param->name().size() > longest_parameter_name)
  ...
  if(parameter->isBlind())
    {
      ...
      INFO(parameter->name() << " (BLIND) " << parameter->mean() + secretoffset << "  - " << parameter->errNeg() << " + " << parameter->errPos());
    }
  else
    {
      INFO(std::setw(longest_parameter_name) << parameter->name() << "     " << std::right << std::setw(13) << parameter->mean() << " + " << std::left
                                             << std::setw(13) << parameter->errPos() << " - " << std::left << std::setw(13) << parameter->errNeg());
    }
}
  ...
  if(m_printLevel == PrintLevel::VeryVerbose)
    {
      for(const auto &param : *m_parSet)
        {
          if(param->isBlind())
            FATAL("Minimiser::PrintLevel is == VeryVerbose, incompatible with "
                  "having any Blind parameter");
        }
...
namespace AmpGen
{
  namespace detail
  {
    ...
    public:
      ...
      void setName(const unsigned int &index, const std::string &name) { fParNames[index] = name; }
      void setAsymmError(const unsigned int &index, const double &low, const double &high) { fMinosErrors[index] = std::make_pair(low, high); }
      void blind(const unsigned int &index, const double &offset) { fParams[index] += offset; }
    };
  }
}
  ...
  for(int i = 0; i != m_mapping.size(); ++i)
    {
      ...
      fr.setName(i, p->name());
      if(p->errPos() != p->errNeg())
        fr.setAsymmError(i, p->errNeg(), p->errPos());
      if(p->isBlind())
        fr.blind(i, m_parSet->at(p->name() + "_blind")->mean());
    }
