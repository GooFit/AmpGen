...
FitResult::FitResult(const FitResult &other)
    : m_mps(other.mps()), m_chi2(other.chi2()), m_LL(other.LL()), m_Edm(other.Edm()), m_NCalls(other.NCalls()), m_nBins(other.nBins()),
      m_nParam(other.nParam()), m_status(other.status()), m_observables(other.observables()), m_fitFractions(other.fitFractions()),
      m_covarianceMatrix(other.cov())
{}

FitResult::FitResult(const std::string &filename) : m_mps(new MinuitParameterSet()) { m_fitted = readFile(filename); }

FitResult::FitResult(const Minimiser &mini)
    : m_mps(mini.parSet()), m_LL(mini.FCN()), m_Edm(mini.Edm()), m_NCalls(mini.NCalls()), m_nParam(0), m_status(mini.status()),
      m_covarianceMatrix(mini.covMatrixFull())
{
  for(size_t i = 0; i < m_mps->size(); ++i)
    {
      if(m_mps->at(i)->isFree() || m_mps->at(i)->isBlind())
        m_nParam++;
...
bool FitResult::readFile(const std::string &fname)
{
    ...
    if(name == "Parameter")
      {
        ...
        if(param != nullptr)
          {
            this->m_mps->add(param);
            this->m_covMapping[tokens[1]] = j++;
          }
      }
    else if(name == "FitQuality")
      this->setFitQuality(line);
    else if(name == "FitFraction")
      this->m_fitFractions.emplace_back(line);
    else if(name == "Observable")
      this->addToObservables(line);
    else if(name == "ParamsOrder")
      {
        isInCovariance = false;
      }
    else if(name == "CovarianceMatrix")
      {
        isInCovariance = true;
        return;
      }
  ...
  for(size_t i = 0; i < m_covarianceMatrix.GetNcols(); ++i)
    {
      for(size_t j = 0; j < m_covarianceMatrix.GetNrows(); ++j)
        m_covarianceMatrix(i, j) = covarianceLines[i][j];
    }
  return true;
}
  ...
  for(size_t i = 0; i < (size_t)m_covarianceMatrix.GetNrows(); ++i)
    {
      ...
      if(param->name().find("_blind") != std::string::npos)
        continue;
      double offset = 0;
      if(param->isBlind())
        {
          ...
          if(!offset)
            {
              WARNING("Attempting to print the unblind value of a blind "
                      "parameter. Will skip the parameter.");
              continue;
            }
        }
      outlog << mysprintf("Parameter %-50s %-7s %-12lf %-12lf %-12lf %-12lf %-12lf %-12lf \n", param->name().c_str(), to_string<Flag>(param->flag()).c_str(),
                          param->mean() + offset, param->err(), param->errNeg(), param->errPos(), param->minInit(), param->maxInit());
    }

  outlog << "FitQuality " << m_chi2 << " " << m_nBins << " " << m_nParam << " " << m_LL << " " << m_status << "\n";
  for(auto &f : m_fitFractions)
    outlog << "FitFraction " << f.name() << " " << f.val() << " " << f.err() << "\n";
  for(auto &o : m_observables)
    outlog << "Observable " << o.first << " " << o.second << "\n";

  outlog << "\nCovarianceMatrix\n";
  for(size_t i = 0; i < (size_t)m_covarianceMatrix.GetNrows(); ++i)
    {
      for(size_t j = 0; j < (size_t)m_covarianceMatrix.GetNcols(); ++j)
...
void FitResult::print() const
{
  ...
  {
    auto param = m_mps->at(i);
    if( param->name().size() > longest_parameter_name ) longest_parameter_name
  = param->name().size() + 3;
  }
  ...
  parameter for now, change this in FitResult.cpp"); continue;
      }
      mean += secretoffset;
    }
    ...
    INFO( std::setw(longest_parameter_name)
          << param->name() << "     " << std::setw(5) <<
  to_string<Flag>(param->flag())
          << std::right << std::setw(13) << param->mean() << " ± "
          << std::left  << std::setw(13) << (param->isFree() ?
  m_mps->at(i)->err() : 0)
          << " Pos err:" << std::setw(11) << (param->isFree() ?
  m_mps->at(i)->errPos() : 0)
          << " Neg err:" << std::setw(11) << (param->isFree() ?
  m_mps->at(i)->errNeg() : 0)
          << (param->isBlind() ? "  (BLIND)" : "") );
    }
  std::cout<<"\n"<<std::endl;
...
std::vector<MinuitParameter *> FitResult::floating(const bool &extended) const
{
  ...
  for(auto &param : *m_mps)
    {
      if((param->isFree() || param->isBlind() || extended) && param->err() > 1e-6)
        floating.push_back(param);
    }
  return floating;
}
  ...
  for(size_t i = 0; i < m_mps->size(); ++i)
    {
      auto param = m_mps->at(i);
      if((param->isFree() || param->isBlind() || extended) && param->err() > 1e-6)
        floating_indices.push_back(i);
    }
  TMatrixD reducedCov(floating_indices.size(), floating_indices.size());
  if(int(floating_indices.size()) > m_covarianceMatrix.GetNrows())
    {
      ERROR("Looking for more floating indices than real indices: " << m_covarianceMatrix.GetNrows() << " " << floating_indices.size());
      return reducedCov;
    }
...
void FitResult::addFraction(const std::string &name, const double &frac, const double &err) { m_fitFractions.emplace_back(name, frac, err); }
...
void FitResult::writeOptions(std::ostream &output_stream, const std::string &input)
{
  ...
  auto print_param = [&](auto &param) mutable {
    ...
    if(param->minInit() != 0. && param->maxInit() != 0.)
      rt += mysprintf(" %-12lf %-12lf", param->minInit(), param->maxInit());
    return rt;
  };
  ...
  auto try_add_complex = [&](const auto &key) mutable {
    ...
    if(param_re == nullptr || param_im == nullptr)
      return false;
