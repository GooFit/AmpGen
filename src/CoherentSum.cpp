...
CoherentSum::CoherentSum(const EventType &type, const MinuitParameterSet &mps, const std::string &prefix) : m_eventType(type), m_prefix(prefix), m_mps(&mps)
{
  ...
  for(auto &amp : amplitudes)
    INFO(prefix + amp.first.decayDescriptor());
  ...
  for(auto &t : m_matrixElements)
    {
      ...
      t.prepare();
      if(m_prepareCalls != 0 && !t.hasExternalsChanged())
        continue;
      if(m_events != nullptr)
        m_cache.update(t);
...
std::vector<FitFraction> CoherentSum::fitFractions(const LinearErrorPropagator &linProp, bool recomputeIntegrals)
{
  ...
  for(auto &f : interferenceFractions)
...
void CoherentSum::generateSourceCode(const std::string &fname, const double &normalisation, bool add_mt)
{
  ...
  for(unsigned i = 0; i != size(); ++i)
    rt[i] = m_matrixElements[i].expression();
...
{
  if(m_verbosity)
    INFO("Setting norm. event list with:" << sim.size() << " events for " << this);
  reset();
