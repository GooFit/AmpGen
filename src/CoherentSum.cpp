...
CoherentSum::CoherentSum(const EventType &type, const MinuitParameterSet &mps, const std::string &prefix) : m_eventType(type), m_prefix(prefix), m_mps(&mps) {
  auto rules = AmplitudeRules::create(mps);
  auto amplitudes = rules->getMatchingRules(m_eventType, prefix);
  if(amplitudes.size() == 0) {
    WARNING("The defined amplitudes don't seem to be able to be able to "
            "generate eventType: "
            << type);
  }
  ...
  for(size_t i = 0; i < m_matrixElements.size(); ++i) {
    auto task = [i, this, &mps, &amplitudes]() mutable {
      this->m_matrixElements[i] = MatrixElement(amplitudes[i].first, amplitudes[i].second, mps, this->m_eventType.getEventFormat(), this->m_verbose);
...
void CoherentSum::prepare() {
  ...
  //   INFO( m_matrixElements[i].name() << " " <<  std::setprecision(15) <<
  //   m_cache(0, m_cache.find(m_matrixElements[i].name())[0] ));
  ...
  if(std::any_of(m_matrixElements.begin(), m_matrixElements.end(), [](auto &me) { return me.workToDo; })) {
    for(unsigned i = 0; i != m_matrixElements.size(); ++i) {
      for(size_t j = i; j < size(); ++j) {
        if(m_matrixElements[i].workToDo || m_matrixElements[j].workToDo) m_normalisations.get(i, j, &m_integrator, i, j);
      }
    }
  }
  m_integrator.flush();
...
void CoherentSum::debug(const Event &evt, const std::string &nameMustContain) {
  ...
  if(m_verbose)
    for(auto &me : m_matrixElements) me.debug(evt);
...
std::vector<FitFraction> CoherentSum::fitFractions(const LinearErrorPropagator &linProp, bool recomputeIntegrals) {
  ...
  for(size_t i = 0; i < ffForHead.size(); ++i) {
    for(size_t j = i + 1; j < ffForHead.size(); ++j) {
      iCalc.emplace_back(ffForHead[i].name() + "x" + ffForHead[j].name(), processIndex(m_matrixElements, ffForHead[i].name()),
...
void CoherentSum::generateSourceCode(const std::string &fname, const double &normalisation, bool add_mt) {
  ...
  for(unsigned int i = 0; i < size(); ++i) {
    ...
    functions.push_back(
      new CompiledExpression<complex_t(const real_t *)>(ampTensor[i], p.decayDescriptor() + "_wParams", m_eventType.getEventFormat(), disableBatch()));
  ...
  functions.push_back(
    new CompiledExpression<real_t(const real_t *, const int &)>(fcn::norm(amplitude) / normalisation, "FCN", disableBatch(), includePythonBindings()));
...
complex_t CoherentSum::getValNoCache(const Event &evt) const {
  auto v = complex_v(std::accumulate(m_matrixElements.begin(), m_matrixElements.end(), complex_v(0, 0),
...
real_t CoherentSum::norm() const { return norm(m_normalisations); }

real_t CoherentSum::norm(const Bilinears &norms) const {
  complex_t acc(0, 0);
  for(size_t i = 0; i < size(); ++i) {
    for(size_t j = 0; j < size(); ++j) {
      // INFO( i << " " << j << " " << m_matrixElements[i].coefficient *
      // std::conj(m_matrixElements[j].coefficient) << " " <<  ( i > j ?
      // std::conj(norm(j,i)) : norm(i,j) ) );
...
complex_t CoherentSum::norm(const size_t &x, const size_t &y) const { return m_normalisations.get(x, y); }
...
void CoherentSum::printVal(const Event &evt) {
  /*
     ...
     std::cout << mE.decayTree.decayDescriptor() << " = " << mE.coefficient <<
     " x " << m_cache( evt.index() / utils::size<real_v>::value, address )
...
// double CoherentSum::operator()( const double* /*evt*/, const unsigned block
// ) const
//{
//  return operator()((const real_v*)nullptr, block /
//  utils::size<real_v>::value ).at( block % utils::size<real_v>::value );
//}
#endif
...
#pragma omp parallel for
  ...
  for(unsigned int block = 0; block < events->nBlocks(); ++block) {
    complex_v amp(0., 0.);
    for(unsigned j = 0; j != m_matrixElements.size(); ++j) amp = amp + complex_v(m_matrixElements[j].coefficient) * store(block, j);
...
#pragma omp parallel for
  ...
  for(unsigned int block = 0; block < events->nBlocks(); ++block) {
    complex_v amp(0., 0.);
    for(unsigned j = 0; j != m_matrixElements.size(); ++j) amp = amp + complex_v(m_matrixElements[j].coefficient) * store(block, j);
    for(unsigned k = 0; k != utils::size<complex_v>::value; ++k) { values[block * utils::size<complex_v>::value + k] = utils::at(amp, k); }
  }
...
KeyedFunctors<double(Event)> CoherentSum::componentEvaluator(const EventList_type *ievents) const {
  ...
  /// this little slice of weirdness allows either a new cache to be
  /// instantiated, or one to just get a pointer to the one used for the
  /// integration.
  ...
  for(unsigned i = 0; i != m_matrixElements.size(); ++i) {
    for(unsigned j = i; j != m_matrixElements.size(); ++j) {
      ...
      auto functor = [ci, cj, i, j, s, cache](const Event &event) {
        return s * std::real(ci * cache->get<complex_t>(event.index(), i) * std::conj(cj * cache->get<complex_t>(event.index(), j)));
      };
      rt.add(functor, name, "");
    }
  }
