...
Transform::Transform(const Expression &arg, const Tensor &k, const Type &type) : m_arg(arg), m_k(k), m_type(type) {}

Transform Transform::inverse() const { return Transform(m_arg, -m_k, m_type); }
...
Tensor Transform::sigma_dot_p(const Tensor &p) const { return p[0] * Sigma[0] + p[1] * Sigma[1] + p[2] * Sigma[2]; }

Tensor Transform::J_dot_p(const Tensor &p) const
{
  Expression z(0);
  return Tensor({z, -p[2], p[1], z, p[2], z, -p[0], z, -p[1], p[0], z, z, z, z, z, z}, Tensor::dim(4, 4));
}

Tensor Transform::K_dot_p(const Tensor &p) const
{
  Expression z(0);
  return Tensor({z, z, z, p[0], z, z, z, p[1], z, z, z, p[2], p[0], p[1], p[2], z}, Tensor::dim(4, 4));
}

Tensor Transform::make_bispinor_operator(const Tensor &A, const Tensor &B, const Tensor &C, const Tensor &D) const
{
  return Tensor({A[{0, 0}], A[{0, 1}], B[{0, 0}], B[{0, 1}], A[{1, 0}], A[{1, 1}], B[{1, 0}], B[{1, 1}], C[{0, 0}], C[{0, 1}], D[{0, 0}], D[{0, 1}], C[{1, 0}],
                 C[{1, 1}], D[{1, 0}], D[{1, 1}]},
                Tensor::dim(4, 4));
}
...
Tensor Transform::boost_spinor() const { return Identity(2) * fcn::safe_sqrt(0.5 * (m_arg + 1)) + sigma_dot_p(m_k) * fcn::safe_sqrt(0.5 * (m_arg - 1)); }
...
Tensor Transform::operator()(const Representation &repr) const
{
  ...
  if(m_type == Type::Rotate && repr == Representation::Spinor)
    return rotate_spinor();
  if(m_type == Type::Boost && repr == Representation::Spinor)
    return boost_spinor();
...
Tensor TransformSequence::operator()(const Transform::Representation &repr) const { return m_cache[repr]; }
  ...
  for(auto repr : {Transform::Representation::Spinor, Transform::Representation::Bispinor, Transform::Representation::Vector})
    {
      if(m_transforms.size() == 0)
        {
          if(repr == Transform::Representation::Spinor)
            m_cache[repr] = Identity(2);
...
Tensor TransformSequence::operator()(const Tensor &tensor, const Transform::Representation &repr) const
{
  Tensor::Index a, b, c;
...
Tensor Transform::operator()(const Tensor &tensor, const Transform::Representation &repr) const
{
  Tensor::Index a, b;
  auto seq = this->operator()(repr);
  return seq(a, b) * tensor(b);
...
void TransformSequence::stepThrough(const Tensor &tensor, const Transform::Representation &repr)
{
  Tensor tr = tensor;
  Tensor::Index a, b;
...
TransformSequence::TransformSequence(const Transform &transform) : m_transforms{transform} { buildCache(); }
...
TransformSequence::TransformSequence() { buildCache(); }
