...
template <unsigned N>
void fill_event(Event &event, const std::vector<DecayChainStackBase *> &phsp, TRandom3 *rndm, const DiscreteDistribution &weight_distribution, double wmax) {
  std::pair<double, std::array<double, 2 * N - 1>> rt;
  const DecayChainStack<N> *node = nullptr;
  if(dynamic_cast<const DecayChainStack<N> *>(phsp[0]) == 0) FATAL("Failed to cast: " << phsp[0]->NP() << " " << N);
  ...
  do {
    ...
    nTrial++;
    if(nTrial > 50000) {
      ERROR("Tried " << nTrial << " and still no dice (pun intentional)"
                     << " " << std::get<0>(rt));
      node->debug(rndm);
    }
  ...
  for(unsigned int i = 0; i != phsp.size(); ++i) {
    genPdf += weight_distribution.probabilities()[i] * static_cast<const DecayChainStack<N> *>(phsp[i])->genPdf(event.address());
  }
  event.setGenPdf(genPdf);
  //  event.print();
}

template <unsigned N> real_v get_gen_pdf(const real_v *value, const std::vector<DecayChainStackBase *> &phsp, const DiscreteDistribution &weight_distribution) {
  real_v genPdf = 0;
  for(unsigned j = 0; j != phsp.size(); ++j) genPdf += weight_distribution.probabilities()[j] * static_cast<const DecayChainStack<N> *>(phsp[j])->genPdf(value);
  return genPdf;
}

template <unsigned N>
std::vector<double>
calculate_weights(const EventListSIMD &events, const std::vector<DecayChainStackBase *> &phsp, const DiscreteDistribution &weight_distribution) {
  ...
  for(unsigned i = 0; i != phsp.size(); ++i) return_weights[i] = utils::sum_elements(weights[i]);
...
void TreePhaseSpace::recalculate_weights(const EventListSIMD &events) {
  ...
  for(int j = 0; j != m_gen.size(); ++j) { weights[j] = pow(weights[j] / events.size(), 0.5) * m_dice.probabilities()[j]; }
  m_dice = DiscreteDistribution(weights);
}

void TreePhaseSpace::setRandom(TRandom *rand) { m_rand = (TRandom3 *)rand; }

EventType TreePhaseSpace::eventType() const { return m_type; }

size_t TreePhaseSpace::size() const { return m_gen.size(); }
