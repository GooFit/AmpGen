...
template <class T> bool isEqual(const T &A, const double &B) { return std::abs(A - B) < std::numeric_limits<double>::epsilon(); }

bool isZero(const complex_t &A)
{
  return std::abs(std::real(A)) < std::numeric_limits<double>::epsilon() && std::abs(std::imag(A)) < std::numeric_limits<double>::epsilon();
}
...
std::string Constant::to_string(const ASTResolver *resolver) const
{
  ...
  return std::imag(m_value) == 0
           ? "(" + rounded_string(std::real(m_value)) + literalSuffix + ")"
...
Expression simplify_constant_addition(const Constant &constant, const Expression &expression)
{
  ...
  if(is<Sum>(expression))
    return simplify_constant_addition(constant, cast<Sum>(expression).l()) + cast<Sum>(expression).r();
...
Expression AmpGen::operator+(const Expression &A, const Expression &B)
{
  bool status = true;
  if(is<Constant>(A))
    return simplify_constant_addition(cast<Constant>(A), B);
  if(is<Constant>(B))
    return simplify_constant_addition(cast<Constant>(B), A);
  if(is<Product>(A))
    {
...
Expression simplify_sum_multiplication(const Sum &sum, const Expression &expression) { return expression * sum.l() + expression * sum.r(); }

Expression simplify_sub_multiplication(const Sub &sum, const Expression &expression) { return expression * sum.l() - expression * sum.r(); }

Expression AmpGen::operator*(const Expression &A, const Expression &B)
{
  if(is<Constant>(A))
    return simplify_multiplication(cast<Constant>(A), B);
  if(is<Constant>(B))
    return simplify_multiplication(cast<Constant>(B), A);
  if(is<Divide>(A))
    return (cast<Divide>(A).l() * B) / (cast<Divide>(A).r());
  if(is<Divide>(B))
    return (A * cast<Divide>(B).l()) / (cast<Divide>(B).r());
  if(is<Sum>(A) || is<Sub>(A))
    return Expression(Product(B, A));
  return Expression(Product(A, B));
}

Expression AmpGen::operator/(const Expression &A, const Expression &B)
{
  if(is<Constant>(A) && isEqual(A(), 0))
    return Constant(0);
  if(is<Constant>(B))
    {
      if(isEqual(B(), 1))
  ...
  else if(is<Product>(B))
    {
      ...
      if(is<Constant>(as_prod.l()))
        return (Constant(1. / as_prod.l()()) * A) / as_prod.r();
      if(is<Constant>(as_prod.r()))
        return (Constant(1. / as_prod.r()()) * A) / as_prod.l();
    }
  else if(is<Divide>(B))
    return (A * cast<Divide>(B).r()) / cast<Divide>(B).l();
...
Ternary::Ternary(const Expression &cond, const Expression &v1, const Expression &v2) : m_cond(cond), m_v1(v1), m_v2(v2) {}
std::string Ternary::to_string(const ASTResolver *resolver) const
{
  return resolver != nullptr && resolver->enableAVX()
           ? "select(" + m_cond.to_string(resolver) + ", " + m_v1.to_string(resolver) + ", " + m_v2.to_string(resolver) + ")"
...
SubTree::SubTree(const Expression &other) : m_expression(other), m_key(FNV1a_hash(m_expression.to_string())) {}

Function::Function(const std::string &name, const std::vector<Expression> &args) : m_name(name), m_args(args) {}
...
std::string Function::to_string(const ASTResolver *resolver) const
{
  ...
  if(m_args.size() == 0)
    return rt + ")";
  for(auto &arg : m_args)
    rt += arg.to_string(resolver) + ", ";
  return rt.substr(0, rt.size() - 2) + ")";
}
void Function::resolve(ASTResolver &resolver) const {}

std::string SubTree::to_string(const ASTResolver * /*resolver*/) const { return "v" + std::to_string(key()); }
...
{
  if(is<Constant>(A) || is<Parameter>(A) || is<SubTree>(A))
    return A;
...
void SubTree::setKey(const size_t &new_key) { m_key = new_key; }
template <class TYPE> Expression simplify_constant_unary(const Expression &arg) { return is<Constant>(arg) ? Expression(TYPE(arg)()) : Expression(TYPE(arg)); }
...
Expression AmpGen::fcn::safe_sqrt(const Expression &x) { return Ternary(x > 0, fcn::sqrt(x), 0); }

Expression AmpGen::fcn::pow(const Expression &expression, const Expression &co)
{
  if(!is<Constant>(co))
    return Pow(expression, co);
  if(isEqual(co(), 0))
    return 1;
  if(isEqual(co(), 1))
    return expression;
...
void LambdaExpression::resolve(ASTResolver &resolver) const { resolver.resolve(*this); }
...
ExpressionPack::ExpressionPack(const Expression &A, const Expression &B)
{
  ...
  if(c1 != nullptr)
    {
      for(auto &expr : c1->m_expressions)
  ...
  if(c2 != nullptr)
    {
      for(auto &expr : c2->m_expressions)
        m_expressions.push_back(expr);
    }
  else
