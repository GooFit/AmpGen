...
std::string Expression::to_string(const ASTResolver *resolver) const {
  if(m_expression == nullptr || get() == nullptr) { ERROR("No expression contained in this node!"); }
...
Expression simplify_product_addition(const Product &prod, const Expression &expression, bool &status) {
  if(is<Constant>(prod.l()) && (std::real(prod.l()()) < 0.)) { return expression - Constant(-1. * complex_t(prod.l()())) * prod.r(); }
...
Expression AmpGen::operator+(const Expression &A, const Expression &B) {
  ...
  if(is<Product>(B)) { return simplify_product_addition(cast<Product>(B), A, status); }
...
std::string ExpressionPack::to_string(const ASTResolver *resolver) const {
  std::string rt = "";
  for(auto expr : m_expressions) { rt += expr.to_string(resolver) + ", "; }
