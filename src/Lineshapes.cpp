...
Expression AmpGen::BlattWeisskopf_Norm(const Expression &ze, const Expression &z0e, unsigned int L)
{
  ...
  switch(L)
    ...
    default: ERROR("Barrier factors not for implemented for L>4"); return 0;
    }
}
...
Expression AmpGen::width(const Expression &s, const Expression &s1, const Expression &s2, const Expression &mass, const Expression &width,
                         const Expression &radius, unsigned int L, DebugSymbols *dbexpressions)
{
...
Expression Lineshape::Factory::get(const std::string &lineshape, const Expression &s, const Expression &s1, const Expression &s2,
                                   const std::string &particleName, const unsigned int &L, DebugSymbols *dbexpressions)
{
  size_t pos = lineshape.find(".");
  ...
  if(pos == std::string::npos)
    {
      ...
      if(!it)
        ERROR("Lineshape : " << lineshape << " not found");
...
Expression Lineshape::Factory::get(const std::string &lineshape, const AmpGen::Particle &p, DebugSymbols *dbexpressions)
{
  size_t pos = lineshape.find(".");
