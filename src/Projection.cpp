...
Projection::Projection(const std::function<double(const Event &)> &fcn, const std::string &name, const std::string &xAxisTitle, const size_t &nBins,
                       const double &min, const double &max, const std::string &units)
    : m_func(fcn), m_name(name), m_xAxisTitle(xAxisTitle), m_units(units), m_nBins(nBins), m_min(min), m_max(max)
{
...
TH1D *Projection::plot(const std::string &prefix) const
{
  ...
  DEBUG("Returning plot: [" << m_min << " " << m_max << "] " << m_name << " " << plot->GetXaxis()->GetBinLowEdge(1) << " "
...
TH2D *Projection2D::plot(const std::string &prefix) const
{
  std::string p = (prefix == "" ? "" : prefix + "_");
  TH2D *plot
    = new TH2D((p + xAxis.m_name + "_vs_" + yAxis.m_name).c_str(), "", xAxis.m_nBins, xAxis.m_min, xAxis.m_max, yAxis.m_nBins, yAxis.m_min, yAxis.m_max);
...
std::pair<double, double> Projection2D::operator()(const Event &evt) const { return {xAxis.m_func(evt), yAxis.m_func(evt)}; }
...
template <> TH1D *Projection::projInternal(const EventList &events, const ArgumentPack &args) const
{
  ...
  if(selection != nullptr)
    INFO("Filter efficiency = " << axis->GetEntries() << " / " << events.size());
  if(autowrite)
    axis->Write();
  return axis;
}
...
template <>
std::tuple<std::vector<TH1D *>, THStack *>
Projection::projInternal(const EventList &events, const Projection::keyedFunctors &weightFunction, const ArgumentPack &args) const
{
...
template <>
std::tuple<std::vector<TH1D *>, THStack *>
Projection::projInternal(const EventListSIMD &events, const Projection::keyedFunctors &weightFunction, const ArgumentPack &args) const
{
  std::vector<TH1D *> hists;
  double norm_sum = args.getArg<Norm>(1).val;
  std::string prefix = args.getArg<PlotOptions::Prefix>().val;
