...
PolarisedSum::PolarisedSum(const EventType &type, MinuitParameterSet &mps, const std::vector<MinuitProxy> &pVector)
    : m_mps(&mps), m_pVector(pVector), m_eventType(type), m_dim(m_eventType.dim())
{
    ...
    if(m_spaceType == spaceType::spin)
      {
        ...
        for(unsigned i = 0; i != type.size(); ++i)
          pols.push_back(ParticleProperties::get(type[i])->polarisations());
        auto polStates = all_combinations(pols);
        auto protoAmps = rules->getMatchingRules(m_eventType);
        for(const auto &[p, c] : protoAmps)
          INFO(p.uniqueString());
            ...
            tp.enqueue([i, p = protoAmps[i].first, c = protoAmps[i].second, polStates, &mps, ptr = this]() mutable {
              ...
              ptr->m_matrixElements[i] = MatrixElement(
                ...
                CompiledExpression<void(complex_v *, const size_t *, const real_t *, const real_v *)>(
                  TensorExpression(thisExpression), p.decayDescriptor(), &mps, ptr->m_eventType.getEventFormat(), ptr->m_debug ? syms : DebugSymbols()));
              if(ptr->m_autoCompile)
        ...
        for(unsigned i = 0; i != m_matrixElements.size(); ++i)
          {
            tp.enqueue([i, this, &r1, &r2]() mutable {
              ...
              this->m_matrixElements[i] = MatrixElement(p, coupling,
                                                        CompiledExpression<void(complex_v *, const size_t *, const real_t *, const real_v *)>(
  ...
  for(size_t i = 0; i < m_dim.second * m_dim.first * m_dim.first; ++i)
...
std::vector<complex_t> densityMatrix(const unsigned &dim, const std::vector<MinuitProxy> &pv)
{
  ...
  double pz = pv[2];
  if(dim == 2)
    return {1 + pz, px + 1i * py, px - 1i * py, 1 - pz};
  if(dim == 3)
    {
      double Tyy = pv[3];
      double Tzz = pv[4];
      ...
      double Txz = pv[6];
      double Tyz = pv[7];
      return {1 + 1.5 * pz + sqrt(1.5) * Tzz,
              sqrt(0.375) * (px + 1i * py) + sqrt(3.) * (Txz + 1i * Tyz),
              -sqrt(1.5) * (Tzz + 2. * Tyy - 2. * 1i * Txy),
              sqrt(0.375) * (px - 1i * py) + sqrt(3) * (Txz - 1i * Tyz),
              1 - sqrt(6.) * Tzz,
              sqrt(0.375) * (px + 1i * py) - sqrt(3.) * (Txz + 1i * Tyz),
              -sqrt(1.500) * (Tzz + 2. * Tyy + 2. * 1i * Txy),
              sqrt(0.375) * (px - 1i * py) - sqrt(3) * (Txz - 1i * Tyz),
              1. - 1.5 * pz + sqrt(1.5) * Tzz};
    }
  ERROR("Density matrices not implemented for state with size=" << dim);
  return {1.};
}

std::vector<MatrixElement> PolarisedSum::matrixElements() const { return m_matrixElements; }

void PolarisedSum::prepare()
{
  auto resetFlags = [](auto &t) {
    t.workToDo = false;
    t.resetExternals();
  };
  auto flagUpdate = [this](auto &t) { t.workToDo = this->m_nCalls == 0 || t.hasExternalsChanged(); };
  auto updateData = [this](auto &t) mutable {
    if(t.workToDo && this->m_events != nullptr)
      {
        this->m_cache.update(t);
      };
  };
  auto updateInteg = [this](auto &t) mutable {
    if(t.workToDo)
      this->m_integrator.updateCache(t);
  };
  transferParameters();
...
real_v PolarisedSum::operator()(const real_v *, const unsigned index) const { return (m_weight / m_norm) * m_pdfCache[index]; }
...
size_t PolarisedSum::size() const { return m_dim.first * m_dim.second * m_matrixElements.size(); }
...
double PolarisedSum::norm() const { return m_norm; }
  ...
  for(size_t i = 0; i < m_matrixElements.size(); ++i)
    {
      for(size_t j = 0; j < m_matrixElements.size(); ++j)
        {
          DEBUG(i << " " << j << " " << m_matrixElements[i].coupling() * std::conj(m_matrixElements[j].coupling()) << " "
  ...
  for(const auto &me : m_matrixElements)
    {
      ...
      if(m_debug)
        me.debug(evt);
      for(auto &t : this_cache)
        all_cache.emplace_back(t);
    }
  if(m_debug)
    m_probExpression.debug(all_cache.data());
  INFO("P(x) = " << getValNoCache(evt));
  INFO("P(x) = " << (m_norm / m_weight)
                      * utils::at(operator()((const real_v *)nullptr, evt.index() / utils::size<real_v>::value), evt.index() % utils::size<real_v>::value));
  if(m_pVector.size() != 0)
  ...
  for(int i = 0; i != m_matrixElements.size(); ++i)
    {
      ...
      for(int j = 0; j != size; ++j)
        rt[i * size + j] = cast<TensorExpression>(z).tensor()[j];
    }
  ...
  for(unsigned int i = 0; i != m_matrixElements.size(); ++i)
    {
      ...
      functions.push_back(new CompiledExpression<std::vector<complex_t>(const real_t *)>(
        TensorExpression(this_amp), p.decayDescriptor() + "_wParams", m_eventType.getEventFormat(), disableBatch(), includePythonBindings()));
...
Expression PolarisedSum::probExpression(const Tensor &T_matrix, const std::vector<Expression> &p, const std::vector<Expression> &pf, DebugSymbols *db) const
{
  ...
  if(it == 2)
    rho = rho + Sigma[0] * p[0] + Sigma[1] * p[1] + Sigma[2] * p[2];
  if(it == 3)
    {
      auto px = p[0];
      auto py = p[1];
...
std::vector<FitFraction> PolarisedSum::fitFractions(const LinearErrorPropagator &prop, bool recomputeIntegrals, bool interferenceFractions)
{
  ...
  for(const auto &[head, couplings] : rules->rules())
    {
      ...
      for(const auto &process : couplings)
        {
          if(process.head() == m_eventType.mother() && process.prefix() != m_prefix)
            continue;
          auto numeratorIndices = processIndex(m_matrixElements, process.name());
          if(numeratorIndices.size() == 0 || numeratorIndices == pCalc.normSet)
            continue;
          pCalc.emplace_back(process.name(), numeratorIndices);
        }
      if(pCalc.calculators.size() == 0)
        continue;
  ...
  INFO("Fit fractions: ");
  for(const auto &p : outputFractions)
    INFO(p);
...
{
  if(m_ownEvents && m_events != nullptr)
    delete m_events;
}
  ...
  if(events != m_integrator.events<EventList_type>())
    {
      ...
      for(auto &me : m_matrixElements)
        const_cast<store_t *>(cache.get())->update(me);
    }
  else
    cache = std::shared_ptr<const store_t>(&m_integrator.cache(), [](const store_t *t) {});
      ...
      for(unsigned j = i; j != m_matrixElements.size(); ++j)
        {
          ...
          rt.add(
            [ci, cj, i, j, s, cache, this](const Event &event) {
