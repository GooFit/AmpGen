...
REGISTER_CONFIGURABLE(PolarisedSum);
        ...
        tp.enqueue([i, p = protoAmps[i].first, c = protoAmps[i].second, polStates, &mps, ptr = this]() mutable {
          ...
          ptr->m_matrixElements[i] = MatrixElement(
            ...
            CompiledExpression<void(complex_v *, const size_t *, const real_t *, const real_v *)>(
              TensorExpression(thisExpression), p.decayDescriptor(), &mps, ptr->m_eventType.getEventFormat(), ptr->m_verbose ? syms : DebugSymbols()));
      ...
      for(unsigned i = 0; i != m_matrixElements.size(); ++i) {
        tp.enqueue([i, this, &r1, &r2]() mutable {
          ...
          this->m_matrixElements[i] = MatrixElement(
            p, coupling,
            CompiledExpression<void(complex_v *, const size_t *, const real_t *, const real_v *)>(
              TensorExpression(thisExpression), p.decayDescriptor(), this->m_mps, this->m_eventType.getEventFormat(), this->m_verbose ? syms : DebugSymbols()));
...
std::vector<complex_t> densityMatrix(const unsigned &dim, const std::vector<MinuitProxy> &pv) {
  ...
  double py = pv[1];
  double pz = pv[2];
  if(dim == 2) return {1 + pz, px + 1i * py, px - 1i * py, 1 - pz};
  if(dim == 3) {
    double Tyy = pv[3];
    double Tzz = pv[4];
    double Txy = pv[5];
...
void PolarisedSum::prepare() {
  ...
  auto updateData = [this](auto &t) mutable {
    if(t.workToDo && this->m_events != nullptr) { this->m_cache.update(t); };
  };
  auto updateInteg = [this](auto &t) mutable {
    if(t.workToDo) this->m_integrator.updateCache(t);
  };
  transferParameters();
...
void PolarisedSum::debug_norm() {
  ...
  double norm_slow = 0;
  for(auto &evt : *m_integrator.events<EventList_type>()) norm_slow += evt.weight() * getValNoCache(evt) / evt.genPdf();
...
void PolarisedSum::setMC(PolarisedSum::EventList_type &events) {
  ...
  m_integIndex.clear();
  for(auto &i : m_matrixElements) m_integIndex.push_back(m_integrator.getCacheIndex(i));
}
...
void PolarisedSum::debug(const Event &evt) {
  ...
  for(const auto &me : m_matrixElements) {
    ...
    if(m_verbose) me.debug(evt);
    for(auto &t : this_cache) all_cache.emplace_back(t);
  }
  if(m_verbose) m_probExpression.debug(all_cache.data());
...
void PolarisedSum::generateSourceCode(const std::string &fname, const double &normalisation, bool add_mt) {
  ...
  for(unsigned int i = 0; i != m_matrixElements.size(); ++i) {
    ...
    functions.push_back(new CompiledExpression<std::vector<complex_t>(const real_t *)>(TensorExpression(this_amp), p.decayDescriptor() + "_wParams",
                                                                                       m_eventType.getEventFormat(), disableBatch(), includePythonBindings()));
...
real_t PolarisedSum::getValNoCache(const Event &evt) const {
  ...
  for(unsigned i = 0; i != m_matrixElements.size(); ++i) { std::memmove(cache.data() + tsize * i, m_matrixElements[i](evt).data(), tsize * sizeof(complex_v)); }
