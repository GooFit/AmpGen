...
void NormalOrderedExpression::Term::addExpression(const Expression &expression) {
  ...
  } else if(is<Constant>(expression))
    m_prefactor *= expression();
  else
    m_terms.emplace_back(expression, expression.to_string());
}

NormalOrderedExpression::Term::Term(const Expression &expression) : m_prefactor(1), m_divisor(1), m_markForRemoval(false) {
  addExpression(expression);
  std::sort(m_terms.begin(), m_terms.end(), [](const auto &t1, const auto &t2) { return t1.second > t2.second; });
...
std::vector<Expression> NormalOrderedExpression::ExpandBrackets(const Expression &expression) {
  if(is<IBinaryExpression>(expression)) {
    ...
    if(is<Product>(expression)) {
      ...
      for(auto &l : left) {
        for(auto &r : right) { rt.emplace_back(l * r); }
      }
      return rt;
    }
    if(is<Divide>(expression)) {
  ...
  if(m_expandSubTrees && is<SubTree>(expression)) return ExpandBrackets(cast<SubTree>(expression).m_expression);
  if(is<Conj>(expression)) {
    auto terms = ExpandBrackets(cast<Conj>(expression).arg());
    std::vector<Expression> rt;
    for(auto &it : terms) rt.push_back(fcn::conj(it));
    return rt;
  }
  return {expression};
}
