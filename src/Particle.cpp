...
Particle::Particle(const std::string &name, const Particle &p1, const Particle &p2)
    : m_name{name}, m_props{ParticlePropertiesList::get(name)}, m_daughters{std::make_shared<Particle>(p1), std::make_shared<Particle>(p2)}
{
  pdgLookup(true);
  sortDaughters();
...
Particle::Particle(const int &pdgID, const Particle &p1, const Particle &p2)
    : m_props{ParticlePropertiesList::get(pdgID)}, m_daughters{std::make_shared<Particle>(p1), std::make_shared<Particle>(p2)}
{
  if(m_props != nullptr)
    m_name = m_props->name();
  pdgLookup(true);
...
Particle::Particle(const std::string &name, const std::vector<Particle> &particles) : m_name{name}, m_props{ParticlePropertiesList::get(name)}
{
  for(const auto &p : particles)
    m_daughters.push_back(std::make_shared<Particle>(p));
  pdgLookup(true);
  sortDaughters();
  for(auto &d : m_daughters)
    d->setParent(this);
...
Particle::Particle(const std::string &name, const unsigned int &index) : m_props{ParticlePropertiesList::get(name)}, m_index{index}
{
  pdgLookup();
  m_uniqueString = makeUniqueString();
}

Particle::Particle(const std::string &decayString, const std::vector<std::string> &finalStates, const bool &orderDaughters)
{
  ...
  for(auto d : fs)
    {
      for(unsigned i = 0; i < finalStates.size(); ++i)
        {
          if(hasUsedFinalState[i] || d->name() != finalStates[i])
            continue;
...
bool Particle::isValidDecayDescriptor(const std::string &decayDescriptor)
{
  ...
  if(open == 0 || open != close || first == std::string::npos)
    {
      if(open != 0 && open != close)
        WARNING("Unmatched braces in possible decay descriptor: " << decayDescriptor);
      return false;
    }
  ...
  for(const auto &token : tokens)
    {
      if(token == "_Re" or token == "_Im")
        continue;
      // INFO( token << " " << ( token.find("_") != std::string::npos) << " "
      // << (&token == &*tokens.begin()) << " " << &token << " " <<
      // &*tokens.begin() );
      if(token.find("_") != std::string::npos and &token == &*tokens.begin()) /// name has a prefix
        {
          INFO("Checking: " << token);
  ...
  if(!valid)
    WARNING("Invalid decay descriptor: " << decayDescriptor);
  return valid;
}
...
void Particle::pdgLookup(bool quiet)
{
  ...
  if(m_lineshape == "BW" || m_usesDefaultLineshape)
    {
      if(m_name.find("NonRes") != std::string::npos || m_props->width() < m_stableThreshold)
  ...
  m_parity = m_props->P();
  // if ( !isdigit( m_props->J()[0] ) ) ERROR( "Spin not recognised! : " <<
  // m_name << " J = " << m_props->J() );
  ...
  if(abs(m_props->pdgID()) == 24 || abs(m_props->pdgID()) == 23)
    isStrong = false;
  m_minL = m_daughters.size() == 2 ? orbitalRange(isStrong).first : 0;
  if(m_daughters.size() == 2)
    {
      DEBUG("IsStrong ? " << isStrong << " orbital =  " << m_orbital << " minL   =  " << m_minL << " name   =  " << m_name
                          << " d0     =  " << m_daughters[0]->name() << " d1     =  " << m_daughters[1]->name());
    }
  if(m_orbital == 0)
    m_orbital = m_minL; // define in ground state unless specified
  ...
  if(!quiet && m_minL == 999)
    ERROR("Decay: " << m_name << " does not appear to have an allowed spin-orbit configuration");
  if(!quiet && m_daughters.size() != 0 && m_props->charge() != charge)
    ERROR("Decay: " << m_name << " does not conserve (electric) charge");
}
...
std::string Particle::orbitalString() const
{
  ...
  if(m_spinConfigurationNumber != 0)
    {
      if(m_spinConfigurationNumber < 10)
        rt += std::to_string(m_spinConfigurationNumber);
      else
        rt += char(m_spinConfigurationNumber - 10);
    }
  return rt;
}

bool Particle::hasModifier(const std::string &modifier) const { return std::find(m_modifiers.begin(), m_modifiers.end(), modifier) != m_modifiers.end(); }

std::string Particle::modifierString() const
{
  std::string modString = "[";
  auto append = [&modString](const std::string &toAppend) { modString += (modString == "[" ? toAppend : ";" + toAppend); };
  if(m_orbital != m_minL || m_spinConfigurationNumber != 0)
    append(orbitalString());
  if(!m_usesDefaultLineshape)
    append(m_lineshape);
  ...
  for(auto &d : m_daughters)
    {
      ...
      if(qTree.daughters().size() == 0 || (qTree.isQuasiStable() && m_daughters.size() != 1))
        ...
        for(auto &it : qTree.daughters())
          returnVal.addDaughter(it);
    }
...
Expression Particle::propagator(DebugSymbols *db) const
{
  if(db != nullptr && !isStable())
    db->emplace_back(uniqueString() + " lineshape", Parameter("NULL", 0, true));
  if(m_daughters.size() == 0)
    return 1;
  ...
  Expression prop = 1.;
  if(m_daughters.size() == 2)
    prop = Lineshape::Factory::get(m_lineshape, *this,
                                   db); // m_lineshape, s, daughter(0)->massSq(), daughter(1)->massSq(),
                                        // m_name, m_orbital, db);
  else if(m_daughters.size() >= 3)
    prop = Lineshape::Factory::get(m_lineshape == "BW" ? "SBW" : m_lineshape, *this, db);
  else if(m_daughters.size() == 1 && m_lineshape.find("BW") == std::string::npos && m_lineshape.find("FormFactor") == std::string::npos)
    {
      prop = Lineshape::Factory::get(m_lineshape, *this, db);
    }
  total *= make_cse(prop);
  for(auto &d : m_daughters)
    total *= make_cse(d->propagator(db));
  if(db != nullptr)
    db->emplace_back("A(" + uniqueString() + ")", total);
  return total;
}
...
std::vector<std::vector<size_t>> Particle::identicalDaughterOrderings() const
{
  ...
  std::transform(finalStateParticles.begin(), finalStateParticles.end(), std::back_inserter(zero_perm), [](auto &p) { return p->name(); });
  ...
  do
    {
      ...
      for(size_t i = 0; i < indices.size(); ++i)
        if(zero_perm[i] != finalStateParticles[indices[i]]->name())
          isSameAsP0 = false;
      if(isSameAsP0)
        orderings.push_back(indices);
    }
...
Expression Particle::getExpression(DebugSymbols *db, const std::vector<int> &state)
{
  ...
  std::transform(fsp.begin(), fsp.end(), std::back_inserter(exchangeParities), [](const auto &p) { return p->props()->isFermion() ? -1 : 1; });
  ...
  for(const auto &ordering : orderings)
    {
      ...
      if(sumAmplitudes)
        total = total + exchangeParity.second * ls;
      else
        total = total + fcn::conj(ls) * ls;
      if(!doSymmetrisation)
        break;
    }
  ADD_DEBUG(total, db);
  ...
  if(sumAmplitudes)
    {
      if(is<Constant>(total))
        {
          WARNING("Amplitude is just a constant: " << total
                                                   << " may cause problems for compiler, making a little bit "
                                                      "complex");
          total += 1i * 0.00001;
        }
  ...
  if(m_daughters.size() == 0)
    {
      ...
      if(S.size() != 1)
        ADD_DEBUG_TENSOR_NAMED(S, db, "S(" + name() + ")");
      S.st();
      return S;
    }
  else if(m_daughters.size() == 2)
    {
      ...
      auto rt = Vertex::Factory::getSpinFactor(P(), Q(), daughter(0)->spinTensor(db), daughter(1)->spinTensor(db), vname, db);
      rt.st();
      return rt;
    }
  else if(m_daughters.size() == 3)
    {
      return Vertex::Factory::getSpinFactorNBody(
        {{daughter(0)->P(), daughter(0)->spinTensor()}, {daughter(1)->P(), daughter(1)->spinTensor()}, {daughter(2)->P(), daughter(2)->spinTensor()}}, spin(),
        db);
    }
...
Tensor Particle::externalSpinTensor(const int &polState, DebugSymbols *db) const
{
  ...
  if(m_spinBasis == spinBasis::Dirac && (m_props->isPhoton() || m_props->isNeutrino()))
    {
      WARNING("Use the Weyl (helicity) basis for calculations involving "
              "photons / neutrinos, as they don't have a rest frame. This "
              "will result in ill-defined amplitudes.");
    }
      ...
      if(m_spinBasis == spinBasis::Weyl)
        {
          ...
          if(polState == 1)
            return N * Tensor({-pZoverP + 1i * z * pY * f * invP, -1i * pZoverP - 1i * z * pX * f * invP, z * invP, 0.});
          if(polState == -1)
            return N * Tensor({pZoverP + 1i * zb * pY * f * invP, -1i * pZoverP - 1i * zb * pX * f * invP, -zb * invP, 0.});
          if(polState == 0)
            return Tensor({pX * pE * invP / m, pY * pE * invP / m, pZoverP * pE / m, pP / m});
        }
      if(m_spinBasis == spinBasis::Dirac)
        {
          Expression N = make_cse(1. / (m * (pE + m)));
          if(polState == 1)
            return -Tensor({1. + z * pX * N, 1i + z * pY * N, z * pZ * N, z / m}) / sqrt(2);
          if(polState == -1)
            return Tensor({1. + zb * pX * N, -1i + zb * pY * N, zb * pZ * N, zb / m}) / sqrt(2);
          if(polState == 0)
            return Tensor({pX * pZ * N, pY * pZ * N, 1 + pZ * pZ * N, pZ / m});
        }
    }
  if(m_props->twoSpin() == 1)
    {
      if(m_spinBasis == spinBasis::Weyl)
        {
          ...
          if(id > 0)
            return N * Tensor({xi[ind][0], xi[ind][1], k * xi[ind][0], k * xi[ind][1]});
          if(id < 0)
            return N * 1i * Tensor({k * xi[ind][0], k * xi[ind][1], xi[ind][0], xi[ind][1]});
        }
      if(m_spinBasis == spinBasis::Dirac)
        {
          Expression N = make_cse(fcn::sqrt((pE + m) / (2 * m)));
          if(id > 0 && polState == 1)
            return N * Tensor({1, 0, pZ / (pE + m), z / (pE + m)});
          if(id > 0 && polState == -1)
            return N * Tensor({0, 1, zb / (pE + m), -pZ / (pE + m)});
          if(id < 0 && polState == +1)
            return N * Tensor({pZ / (pE + m), z / (pE + m), 1, 0});
          if(id < 0 && polState == -1)
            return N * Tensor({zb / (pE + m), -pZ / (pE + m), 0, 1});
        }
    }
...
std::pair<size_t, size_t> Particle::orbitalRange(const bool &conserveParity) const
{
  ...
  DEBUG("Name = " << m_name << " Range = " << min << " -> " << max << " conserving parity ? " << conserveParity << " 2J = " << S << " 2s1= " << s1
                  << " 2s2= " << s2);

  if(conserveParity == false)
    return {min, max};
  int l = min;
  for(; l < max + 1; ++l)
    if(conservesParity(l))
      break;
  if(l == max + 1)
    return {999, 999};
...
std::vector<std::pair<double, double>> Particle::spinOrbitCouplings(const bool &conserveParity) const
{
  ...
  for(double L = lRange.first; L <= lRange.second; ++L)
    {
      if(conserveParity && !conservesParity(L))
        continue;
      for(double S = std::abs(j1 - j2); S <= j1 + j2; ++S)
        {
          double z2 = J * J - L * L - S * S;
          if(L != 0 && S != 0 && std::abs(z2 / (2 * L * S)) <= 1)
            lsCouplings.emplace_back(L, S);
          if((L == 0 || S == 0) && z2 == 0)
            lsCouplings.emplace_back(L, S);
        }
    }
...
std::string Particle::texLabel(const bool &printHead, const bool &recurse) const
{
  ...
  if(!isHead())
    val = m_props->label();
  if(printHead && isHead())
    val = m_props->label() + "\\rightarrow ";

  if(isHead() || recurse)
    {
      if(!isHead() || m_orbital != m_minL)
        val += leftBrace;
      for(auto &prod : m_daughters)
        val += prod->texLabel(false, recurse) + " ";
      if(!isHead() || m_orbital != m_minL)
        val += rightBrace;
      if(m_orbital != m_minL)
        val += "^{" + orbitalString() + "}";
...
void Particle::conjThis()
{
  ...
  for(unsigned i = 0; i != m_daughters.size(); ++i)
    m_daughters[i]->conjThis();
}
...
Particle Particle::conj(bool invertHead, bool reorder)
{
  ...
  cp.setName(p->name());
  for(auto &d : m_daughters)
    cp.addDaughter(std::make_shared<Particle>(d->conj(invertHead, reorder)));
  if(reorder)
    cp.sortDaughters();
  cp.pdgLookup();
  return cp;
...
QuarkContent Particle::quarks() const { return m_name.find("NonRes") != std::string::npos ? daughterQuarks() : m_props->quarkContent(); }
...
void Particle::clearDecayProducts() { m_daughters.clear(); }
...
int Particle::finalStateParity() const
{
  if(m_daughters.size() == 0)
    return 1;
  if(m_daughters.size() == 1)
    return m_daughters[0]->finalStateParity(); /// forward through quasiparticles
  if(m_daughters.size() == 2)
    {
      int f = spin() - (m_orbital + daughter(0)->spin() + daughter(1)->spin());
      int lpart = (f % 2 == 0 ? 1 : -1);
      for(auto &d : m_daughters)
        lpart *= d->finalStateParity();
      return lpart;
    }
  WARNING("> 2 body vertices may require special considerations when "
          "conjugating, returning (-1)^J");
  return pow(-1, spin());
}

bool Particle::conservesParity(unsigned int L) const { return parity() == daughter(0)->parity() * daughter(1)->parity() * (L % 2 == 0 ? 1 : -1); }
...
bool Particle::isQuasiStable() const { return props()->width() < m_stableThreshold && name() != "gamma0"; }
...
void Particle::setParent(const Particle *ptr) { m_parent = ptr; }
...
bool Particle::operator<(const Particle &other)
{
  ...
  if(std::abs(props()->pdgID()) == std::abs(other.props()->pdgID()) && props()->pdgID() != other.props()->pdgID())
    return props()->pdgID() > other.props()->pdgID();
  if(props()->charge() != other.props()->charge())
    return props()->charge() > other.props()->charge();
  if(props()->pdgID() != other.props()->pdgID())
    return props()->pdgID() > other.props()->pdgID();
  return index() < other.index();
}

bool Particle::operator>(const Particle &other) { return !(*this < other); }
...
void Particle::setPolarisationState(const std::vector<int> &state)
{
  ...
  for(size_t i = 0; i < fs.size(); ++i)
    fs[i]->setPolarisationState(state[i + 1]);
  m_uniqueString = makeUniqueString();
}
...
unsigned int Particle::matches(const Particle &other) const
{
  ...
  if((other.daughters().size() == 0 && daughters().size() != 0) || (other.daughters().size() != 0 && daughters().size() == 0))
    return MatchState::PartialExpansion;
  if(other.daughters().size() != daughters().size())
    return MatchState::None;
  ...
  if(m_orbital != other.L())
    rt |= MatchState::DifferentOrbital;
  if(m_polState != other.polState())
    rt |= MatchState::DifferentPolarisation;
  if(rt == 0)
    rt = MatchState::Exact;
...
int Particle::CP() const { return C() * finalStateParity(); }

int Particle::C() const
{
  if(m_daughters.size() == 1)
    return m_daughters[0]->C();
  int prod = 1;
  for(auto &d : m_daughters)
    prod *= (d->props()->C() == 0 ? 1 : d->props()->C()) * d->C();
  return prod;
}
...
{
  Particle rt = *this;
  for(unsigned int i = 0; i != rt.daughters().size(); ++i)
    rt.setDaughter(i, this->daughters()[i]->clone());
  return rt;
}

void Particle::setDaughter(const unsigned int &index, const Particle &p) { m_daughters[index] = std::make_shared<Particle>(p); }

bool Particle::expand(const Particle &particle)
{
  for(auto &d : m_daughters)
    {
      if(d->name() == particle.name() && d->isStable())
        {
          d = std::make_shared<Particle>(particle);
          return true;
        }
  ...
  for(auto &modifier : m_modifiers)
    {
      ...
      if(tokens.size() == 2 && tokens[0] == key)
        return stdx::optional<std::string>(tokens[1]);
    }
  return stdx::nullopt;
}

std::ostream &AmpGen::operator<<(std::ostream &os, const Particle &particle) { return os << particle.decayDescriptor(); }

const Particle *Particle::parent() const
{
  WARNING("Pointer to parent can be invalidated by copying / moving, proceed "
          "with care..");
  return m_parent;
}
...
namespace AmpGen
{
  complete_enum(spinFormalism, Covariant, Canonical) complete_enum(spinBasis, Dirac, Weyl)
}
