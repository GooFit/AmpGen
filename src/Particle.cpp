...
Particle::Particle(const std::string &decayString, const std::vector<std::string> &finalStates, const bool &orderDaughters) {
  if(m_spinBasis == spinBasis::Invalid) { FATAL("Unrecognised spin basis!"); }
  ...
  if(!isStateGood()) { ERROR("Amplitude " << decayString << " not configured correctly"); }
...
void Particle::pdgLookup(bool quiet) {
  ...
  // m_name << " J = " << m_props->J() );
  if(m_defaultModifier != "" && m_lineshape.find(".") == std::string::npos) { m_lineshape = m_lineshape + "." + std::string(m_defaultModifier); }
...
std::string Particle::modifierString() const {
  ...
  for(auto &mod : otherModifiers)
    if(hasModifier(mod)) append(mod);
  for(auto &mod : m_modifiers)
    if(mod.find("=") != std::string::npos) append(mod);
  modString += "]";
...
void Particle::setOrdering(const std::vector<size_t> &ordering) {
  auto finalStateParticles = getFinalStateParticles();
  for(unsigned int i = 0; i < ordering.size(); ++i) { finalStateParticles[i]->setIndex(ordering[i]); }
}
...
Expression Particle::getExpression(DebugSymbols *db, const std::vector<int> &state) {
  ...
  for(const auto &ordering : orderings) {
    ...
    if(db != nullptr) {
      ...
      if(finalStateString != "") finalStateString = finalStateString.substr(0, finalStateString.size() - 1);
...
Tensor Particle::externalSpinTensor(const int &polState, DebugSymbols *db) const {
  ...
  if(spin() == 0) return Tensor(std::vector<double>({1.}), std::vector<unsigned>({1}));
...
std::pair<size_t, size_t> Particle::orbitalRange(const bool &conserveParity) const {
  ...
  for(int i = min_s12; i <= max_s12; i++) { min = std::min(min, std::abs(S - i)); }
  min /= 2;
  max /= 2;
...
void Particle::conjThis() {
  if(m_props->hasDistinctAnti()) m_props = ParticlePropertiesList::get(-m_props->pdgID(), false);
...
unsigned int Particle::matches(const Particle &other) const {
  ...
  if(rt & MatchState::Exact && rt != MatchState::Exact) rt &= ~MatchState::Exact;
  return rt;
}
