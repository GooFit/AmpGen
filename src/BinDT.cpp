...
double nearestNeighbourVariance(std::vector<double *> evts, const size_t &index)
{
  ...
  parallel_sort(
    evts.begin(), evts.end(), [index](const double *evt1, const double *evt2) { return *(evt1 + index) > *(evt2 + index); }, std::max(256ul, evts.size() / 4));
...
std::pair<std::vector<double *>, std::vector<double *>> splitEvents(std::vector<double *> evts, const size_t &index, double &midposition)
{
  ...
  midposition = evts.size() % 2 == 0 ? (*(evts[midpoint - 1] + index) + *(evts[midpoint] + index)) / 2
...
std::function<std::vector<double>(const Event &)> BinDT::makeDefaultFunctors()
{
  if(m_dim == 5)
    {
      return [](const Event &evt) -> std::vector<double> { return {evt.s(1, 2, 3), evt.s(0, 1), evt.s(0, 2), evt.s(2, 3), evt.s(0, 1, 2)}; };
    }
  if(m_dim == 2)
    {
...
BinDT::BinDT(const EventList &events, const ArgumentPack &args) : BinDT(args) { makeNodes(events.begin(), events.end()); }
...
void BinDT::readFromStream(std::istream &stream)
{
      ...
      try
        {
          ...
          if(tokens.size() == 5)
          ...
          else
            {
              ...
              if(tokens.size() == 3)
                node->setBinNumber(stoi(tokens[2]));
              nodes[address] = std::make_pair(line, node);
  ...
  DEBUG("Read: " << nodes.size() << " nodes from plain-text file"); // , now re-establishing tree
                                                                    // structure; EndNodes=" << edc );
...
std::shared_ptr<BinDT::INode> BinDT::makeNodes(const std::vector<double *> &evts, std::queue<unsigned> indexQueue, const unsigned &depth)
{
  ...
  DEBUG("Depth = " << depth << " nodes = " << m_endNodes.size() << " maxDepth = " << m_maxDepth << " evts.size() = " << evts.size()
                   << " minEvents = " << m_minEvents);
  ...
  indexQueue.pop();
  if(indexQueue.empty())
    refreshQueue(evts, indexQueue, depth + 1);
...
double bestCut_lineSearch(const std::vector<double *> &source, const std::vector<double *> &target, int index, const size_t &dim, const size_t &minEvents)
{
  ...
  double optPos = 0;
  double s_w1 = 0;
  double s_wt = parallel_accumulate(source.begin(), source.end(), 0.0,
                                    w);                                                  //  sum_weights);
  double t_w1 = parallel_accumulate(target.begin(), target.begin() + minEvents, 0.0, w); //  sum_weights);
  double t_wt = parallel_accumulate(target.begin(), target.end(), 0.0,
                                    w); //  sum_weights);
  unsigned int s_p1 = 0;
      ...
      for(; sourceIt != source.end(); ++sourceIt)
        {
          ...
          if(positionOfThis > pos)
            break;
          s_w1 += w(*sourceIt);
          s_p1++;
        }
      // INFO( sourceLeftWeight << " " << targetLeftWeight << " " <<
  ...
  for(unsigned int i = 0; i < target.size() - m_minEvents; i++)
    {
      ...
      double chi2 = (sourceLeftWeight - targetLeftWeight) * (sourceLeftWeight - targetLeftWeight) / (sourceLeftWeight + targetLeftWeight)
                    + +(sourceRightWeight - targetRightWeight) * (sourceRightWeight - targetRightWeight) / (sourceRightWeight + targetRightWeight);
...
std::shared_ptr<BinDT::INode>
BinDT::makeNodes(std::vector<double *> source, std::vector<double *> target, std::queue<unsigned int> indexQueue, const unsigned int &depth)
{
  unsigned int index = indexQueue.front();
  ...
  if(sourceLeft.size() == 0 || sourceRight.size() == 0)
    {
      ...
      DEBUG("Dividing:  source=(" << sourceLeft.size() << ", " << sourceRight.size() << "); target=(" << targetLeft.size() << ", " << targetRight.size()
  ...
  indexQueue.pop();
  if(indexQueue.empty())
    refreshQueue(source, indexQueue, depth + 1);
...
void BinDT::refreshQueue(const std::vector<double *> &evts, std::queue<unsigned int> &indexQueue, const unsigned int &depth)
{
  if(evts.size() > m_minEvents * pow(2, m_dim) && depth + m_dim < m_maxDepth)
    {
      if(m_queueOrdering.size() == 0)
        for(unsigned int i = 0; i < m_dim; ++i)
          m_queueOrdering.push_back(i);
      for(unsigned int i = 0; i < m_dim; ++i)
        indexQueue.push(m_queueOrdering[i]);
    }
  else
    {
      std::vector<std::pair<unsigned int, double>> indices;
      for(unsigned int i = 0; i < m_dim; ++i)
        indices.emplace_back(i, nearestNeighbourVariance(evts, i));
      std::sort(indices.begin(), indices.end(), [](const auto &it1, const auto &it2) { return it1.second > it2.second; });
      for(const auto &item : indices)
        indexQueue.push(item.first);
    }
}

BinDT::Decision::Decision(const unsigned int &index, const double &value, std::shared_ptr<INode> left, std::shared_ptr<INode> right)
...
BinDT::EndNode::EndNode(const unsigned int &no, const unsigned int &binNumber) : m_voxNumber(no), m_binNumber(binNumber) {}

const BinDT::EndNode *BinDT::EndNode::operator()(const double *evt) const { return this; }

void BinDT::EndNode::serialize(std::ostream &stream) const { stream << this << " " << m_voxNumber << " " << m_binNumber << std::endl; }

const BinDT::EndNode *BinDT::Decision::operator()(const double *evt) const { return *(evt + m_index) < m_value ? (*m_right)(evt) : (*m_left)(evt); }
