...
void EventList::loadFromFile(const std::string &fname, const ArgumentPack &args)
{
  ...
  TTree *tree = nullptr;
  if(fname == "")
    FATAL("Filename must be specified to load data");
  if(tokens.size() == 2)
    {
      gFile = TFile::Open(tokens[0].c_str(), "READ");
      if(gFile == nullptr)
        FATAL("Failed to load file: " << tokens[0]);
  ...
  if(tree == nullptr)
    FATAL("Failed to load tree from file: " << fname);
  loadFromTree(tree, args);
  gFile->Close();
  gFile = current_file;
}

template <typename key_type, typename value_type, typename default_type = key_type>
key_type inv_map(const std::map<key_type, value_type> &mp, const value_type &k, const default_type &df = default_type())
{
...
void EventList::loadFromTree(TTree *tree, const ArgumentPack &args)
{
  ...
  if(m_eventType.size() == 0)
    {
      ...
      for(auto br : *tree->GetListOfBranches())
        {
          if(std::string(br->GetName()).find("_decayTime") != std::string::npos)
            is_TD = true;
        }
      if(tokens.size() != 1)
        m_eventType = EventType(tokens, is_TD);
  ...
  TreeReader tr(tree);
  bool hasEnergy = branches.size() == 0 || branches.size() == 4 * m_eventType.size(); // if the energy of the particle has been
  ...
  if(branches.size() != 0)
    {
      ...
      if(idBranches.size() != 0)
        {
          if(idBranches.size() != m_eventType.size())
            FATAL("Number of ID branches should be number of final state "
                  "particles");
          for(int i = 0; i != ids.size(); ++i)
            tr.setBranch(idBranches[i], ids.data() + i);
        }
    }
  else
  ...
  if(getGenPdf)
    tr.setBranch("genPdf", temp.pGenPdf());
  if(weightBranch != "")
  ...
  if(filter != "")
    {
      if(entryList.size() != 0)
        {
          WARNING("Specified entry list and filter, will overwrite list with "
                  "specified selection");
        }
      tr.prepare();
  ...
  for(const auto &evt : tr)
    {
      if(inputUnits != Units::GeV)
        for(unsigned k = 0; k != eventFormat.size(); ++k)
          temp[k] *= to_double(inputUnits);
      ...
      if(!hasEnergy)
        {
          for(unsigned int k = 0; k != m_eventType.size(); ++k)
            temp[4 * k + 3] = sqrt(m_eventType.mass(k) * m_eventType.mass(k) + temp[4 * k + 0] * temp[4 * k + 0] + temp[4 * k + 1] * temp[4 * k + 1]
                                   + temp[4 * k + 2] * temp[4 * k + 2]);
        }
      push_back(temp);
    }
  read_time.stop();
...
TTree *EventList::tree(const std::string &name, const std::vector<std::string> &extraBranches) const
{
  ...
  for(unsigned i = 0; i != m_eventType.size(); ++i)
    title += " " + m_eventType[i];
...
std::vector<TH1D *> EventList::makeProjections(const std::vector<Projection> &projections, const ArgumentPack &args)
{
  ...
  for(const auto &proj : projections)
    plots.push_back(makeProjection(proj, args));
  return plots;
}
...
void EventList::erase(const std::vector<Event>::iterator &begin, const std::vector<Event>::iterator &end) { m_data.erase(begin, end); }

void EventList::reserve(const size_t &size) { m_data.reserve(size); }
