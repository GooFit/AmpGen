...
ParticleProperties::ParticleProperties(const std::string &pdg_string) {
  ...
  if(spin_status == 0) { DEBUG("Spin of particle: " << name() << " could not be interpretted (J=" << s[8] << ")"); }
  m_radius = 1.5 / GeV;
...
bool ParticleProperties::operator==(const ParticleProperties &rhs) const {
  if(pdgID() == 0 && rhs.pdgID() == 0) { return name() == rhs.name(); }
...
void ParticleProperties::setProperty(const std::string &key, const std::string &value) {
  ...
  if(key == "mass")
    ...
    if(spin_status == 0) { ERROR("Spin of particle: " << name() << " could not be interpretted (J=" << value << ")"); }
  } else if(key == "parity")
