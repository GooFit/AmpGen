...
std::vector<int> ParticleProperties::polarisations() const
{
  ...
  if(twoSpin() == 2)
    return {1, 0, -1}; // vector
  if(twoSpin() == 4)
    return {2, 1, 0, -1, -2}; // tensor
  else
    {
      WARNING("Particle with spin: " << twoSpin() << "/2"
                                     << " not implemented in initial/final state");
      return {0};
    }
}
...
std::string ParticleProperties::J() const
{
  if(m_twoSpin == -1)
    return "?";
  if(m_twoSpin % 2 == 0)
    return std::to_string(int(m_twoSpin / 2));
  if(m_twoSpin % 2 == 1)
    return std::to_string(int(m_twoSpin / 2)) + "/2";
  return "";
}

ParticleProperties::ParticleProperties(const std::string &pdg_string)
{
  auto is_fundamental = [](unsigned pdgID) { return pdgID == 22 or pdgID == 24 or pdgID == 23 or (pdgID >= 11 and pdgID <= 16); };
  m_isValid = false;
  if(pdg_string == "" || pdg_string.empty())
    return;
  if(pdg_string[0] == '*')
    return;
  ...
  if(s[8] == "?")
    ...
    m_twoSpin = 2 * lexical_cast<int>(s[8], spin_status);
  if(spin_status == 0)
    {
      DEBUG("Spin of particle: " << name() << " could not be interpretted (J=" << s[8] << ")");
    }
  m_radius = 1.5 / GeV;
  bool isCharm = (abs(pdgID()) == 421 || abs(pdgID()) == 411 || abs(pdgID()) == 431 || abs(pdgID()) == 4122);
  if(isCharm)
    m_radius = 5.0 / GeV;
  m_isValid = true;
}
...
double ParticleProperties::lifetime() const { return 6.582119514 / (m_width * std::pow(10, 13)); }

bool ParticleProperties::isNonResonant() const { return m_name.find("NonRes") != std::string::npos; }

bool ParticleProperties::isFermion() const { return m_twoSpin % 2 == 1; }

bool ParticleProperties::isPhoton() const { return m_pdgID == 22; }

bool ParticleProperties::isNeutrino() const { return abs(m_pdgID) == 12 || abs(m_pdgID) == 14 || abs(m_pdgID) == 16; }

bool ParticleProperties::isBoson() const { return !isFermion(); }
...
std::string ParticleProperties::spinName() const
{
  if(m_twoSpin % 2 == 0)
    ...
    {
      if(m_twoSpin == 1)
        return m_pdgID > 0 ? "f" : "F";
      if(m_twoSpin == 3)
        return m_pdgID > 0 ? "r" : "R";
      if(m_twoSpin == 5)
        return m_pdgID > 0 ? "e" : "E";
      if(m_twoSpin == 7)
        return m_pdgID > 0 ? "c" : "C";
    }
  WARNING("Spin name not implemented for " << m_twoSpin);
  return "?";
}

const ParticleProperties *ParticleProperties::get(const std::string &name, const bool &quiet) { return ParticlePropertiesList::get(name, quiet); }
...
void ParticleProperties::setProperty(const std::string &key, const std::string &value)
{
  ...
  bool status = true;
  if(key == "mass")
    m_mass = stod(value) / MeV;
  else if(key == "name")
    m_name = value;
  else if(key == "width")
    m_width = stod(value) / MeV;
  else if(key == "spin")
    {
      bool spin_status = 1;
      if(value == "?")
        ...
        m_twoSpin = 2 * lexical_cast<int>(value, spin_status);
      if(spin_status == 0)
        {
    ...
    m_parity = chargeFromString(value, status);
  else if(key == "charge")
