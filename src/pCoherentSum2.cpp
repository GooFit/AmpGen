#include "AmpGen/pCoherentSum2.h"
...
pCoherentSum2::pCoherentSum2() = default; 

pCoherentSum2::pCoherentSum2( const EventType& type, const MinuitParameterSet& mps, const std::string& prefix )
  : m_rules(mps)
  , m_evtType        (type)
  , m_printFreq      (NamedParameter<size_t>(     "pCoherentSum2::PrintFrequency", 100)  )
  , m_dbThis         (NamedParameter<bool>(       "pCoherentSum2::Debug"         , false))
  , m_verbosity      (NamedParameter<bool>(       "pCoherentSum2::Verbosity"     , 0)    )
  , m_objCache       (NamedParameter<std::string>("pCoherentSum2::ObjectCache"   ,"")    )
  , m_prefix         (prefix)
{
...
void pCoherentSum2::updateNorms( const std::vector<size_t>& changedPdfIndices )
{
...
void pCoherentSum2::debug( const Event& evt, const std::string& nameMustContain )
{
  prepare();
...
std::vector<FitFraction> pCoherentSum2::fitFractions(const LinearErrorPropagator& linProp)
{
  prepare();
  bool recomputeIntegrals    = NamedParameter<bool>("pCoherentSum2::RecomputeIntegrals", false );
  std::vector<FitFraction> outputFractions;
  for(auto& rule : m_rules.rules()) 
  {
    FitFractionCalculator<pCoherentSum2> pCalc(this, findIndices(m_matrixElements, rule.first), recomputeIntegrals);
  ...
  FitFractionCalculator<pCoherentSum2> iCalc(this, findIndices(m_matrixElements, m_evtType.mother()), recomputeIntegrals);
...
void pCoherentSum2::generateSourceCode(const std::string& fname, const double& normalisation, bool add_mt)
{
  ...
  bool includePythonBindings = NamedParameter<bool>("pCoherentSum2::IncludePythonBindings",false);
...
complex_t pCoherentSum2::getValNoCache( const Event& evt ) const
{
...
complex_t pCoherentSum2::getValNoCache(const Event& evt, const size_t& offset) const
{
  return std::accumulate( m_matrixElements.begin(), 
      m_matrixElements.end(), 
      complex_t(0,0), 
...
void pCoherentSum2::reset( bool resetEvents )
{
  m_prepareCalls                                     = 0;
  m_lastPrint                                        = 0;
...
void pCoherentSum2::setEvents( EventList& list )
{
...
void pCoherentSum2::setMC( EventList& sim )
{
...
real_t pCoherentSum2::norm() const
{
  return norm(m_normalisations);
}

real_t pCoherentSum2::norm(const Bilinears& norms) const
{
  complex_t acc(0, 0);
  for ( size_t i = 0; i < size(); ++i ) {
    for ( size_t j = 0; j < size(); ++j ) {
...
complex_t pCoherentSum2::norm(const size_t& x, const size_t& y) const
{
  return m_normalisations.get(x, y);
}

void pCoherentSum2::transferParameters()
{
  for ( auto& mE : m_matrixElements ) mE.coefficient = mE.coupling();
  m_weight.update();
}

void pCoherentSum2::printVal(const Event& evt)
{
  for ( auto& mE : m_matrixElements ) {
...
std::vector<size_t> pCoherentSum2::cacheAddresses( const EventList& evts ) const
{
...
complex_t pCoherentSum2::getVal( const Event& evt ) const
{
  complex_t value( 0., 0. );
  for ( auto& mE : m_matrixElements ) {
...
complex_t pCoherentSum2::getVal( const Event& evt, const std::vector<size_t>& cacheAddresses ) const
{
  complex_t value( 0., 0. );
  for ( size_t i = 0; i < m_matrixElements.size(); ++i )
