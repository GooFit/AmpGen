...
Event::Event(const unsigned &N) : m_event(N) {}

Event::Event(const real_t *data, const unsigned &N) : m_event(data, data + N) {}
...
void Event::print() const {
  ...
  for(unsigned i = 4 * nParticles; i != m_event.size(); ++i) { INFO(m_event[i]); }
}
real_t Event::s(const unsigned &index) const {
  return get(4 * index + 3) * get(4 * index + 3) - get(4 * index + 0) * get(4 * index + 0) - get(4 * index + 1) * get(4 * index + 1)
         - get(4 * index + 2) * get(4 * index + 2);
}

real_t Event::s(const unsigned &index1, const unsigned &index2) const {
...
real_t Event::s(const unsigned &index1, const unsigned &index2, const unsigned &index3) const {
...
real_t Event::s(const std::vector<unsigned> &indices) const {
...
void Event::set(const unsigned &i, const real_t *p) {
  for(unsigned j = 0; j < 4; ++j) m_event[4 * i + j] = p[j];
}
void Event::set(const real_t *evt) {
  for(unsigned i = 0; i < m_event.size(); ++i) m_event[i] = *(evt + i);
}
void Event::set(const unsigned &i, const real_t &p) { m_event[i] = p; }

void Event::swap(const unsigned &i, const unsigned &j) {
  double tmp[4];
