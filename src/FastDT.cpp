...
FastDT::FastDT(std::ifstream &stream, const unsigned &n_nodes) { readFromStream(stream, n_nodes); }
...
void FastDT::serialise(std::ofstream &stream)
{
  for(auto &node : m_nodes)
    stream << node.index << " " << node.left << " " << node.right << " " << node.cutValue << std::endl;
}
  ...
  while(m_nodes[address].index != 0)
    {
      ...
      if(address < 0 or address >= m_nodes.size())
        ERROR("Invalid node: " << address);
      // INFO("@ address: " << address << "next node = " <<
      // (event[node.index-1] > node.cutValue ? node.right : node.left)  );
      //  if( node.index -1 < 0 or node.index -1 >= m_dim ) ERROR("Invalid
      //  index: " << node.index -1 );
      address = event[node.index - 1] > node.cutValue ? node.right : node.left;
      if(address < 0)
        return std::abs(address) - 1;
      if(address >= m_nodes.size())
        FATAL("Fallen out of DT, " << address << " out of " << m_nodes.size());
    }
...
void FastDT::refreshQueue(std::vector<double *> &evts, std::queue<unsigned> &indexQueue, const unsigned &depth)
{
  if(evts.size() > m_minEvents * pow(2, m_dim) && depth + m_dim < m_maxDepth)
  ...
  else
    {
      ...
      for(size_t i = 0; i < m_dim; ++i)
        indices.emplace_back(i, nnVariance(evts, i));
...
FastDT::bestCut_ls(const std::vector<double *> &source, const std::vector<double *> &target, int index, const size_t &dim, const size_t &minEvents)
{
...
int FastDT::makeNodes(std::vector<double *> source, std::vector<double *> target) { return makeNodes(source, target, makeQueue(), 0); }
...
int FastDT::makeNodes(std::vector<double *> source, std::vector<double *> target, std::vector<int> indexQueue, const unsigned int &depth)
{
  ...
  double bestChi2 = 0;
  double optPos = 0;
  if(std::all_of(indexQueue.begin(), indexQueue.end(), [](auto &arg) { return arg == -1; }))
    indexQueue = makeQueue();
  ...
  DEBUG("Cutting on " << bestIndex << " @ " << optPos << " chi2 = " << bestChi2 << " p[0] = " << *(source[0] + bestIndex) << " s = [" << sourceLeft.size()
                      << ", " << sourceRight.size() << "]; "
  ...
  if(sourceLeft.size() == 0 || sourceRight.size() == 0)
    {
      ...
      DEBUG("Dividing:  source=(" << sourceLeft.size() << ", " << sourceRight.size() << "); target=(" << targetLeft.size() << ", " << targetRight.size()
                                  << "); index=" << bestIndex << " cutting @ " << optPos << " zeroth source " << *(source[0] + bestIndex)
                                  << " zeroth target: " << *(target[0] + bestIndex) << " depth = " << depth);
