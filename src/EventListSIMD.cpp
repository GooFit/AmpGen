...
EventListSIMD::EventListSIMD(const EventType &type) : m_data(0, type.eventSize()), m_eventType(type) {}
...
void EventListSIMD::loadFromFile(const std::string &fname, const ArgumentPack &args)
{
  ...
  if(tokens.size() == 2)
  ...
  else
    {
      ...
      if(tree == nullptr)
        tree = (TTree *)gFile->Get("EventList");
    }
  if(tree == nullptr)
  ...
  if(m_eventType.size() == 0)
    {
      ...
      if(tokens.size() != 1)
        setEventType(EventType(tokens, is_TD));
...
TTree *EventListSIMD::tree(const std::string &name, const std::vector<std::string> &extraBranches) const
{
  ...
  // for ( const auto& f : m_extensions ) outputTree->Branch( f.first.c_str(),
  // tmp.address( f.second ) );
...
std::vector<TH1D *> EventListSIMD::makeProjections(const std::vector<Projection> &projections, const ArgumentPack &args)
{
  ...
  for(const auto &proj : projections)
    plots.push_back(proj(*this, args));
  return plots;
}

TH1D *EventListSIMD::makeProjection(const Projection &projection, const ArgumentPack &args) const { return projection(*this, args); }
...
void EventListSIMD::clear() { m_data.clear(); }
