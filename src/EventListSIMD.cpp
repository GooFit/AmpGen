...
const Event EventListSIMD::operator[](const size_t &pos) const {
  ...
  for(unsigned i = 0; i != tempEvent.size(); ++i) tempEvent[i] = m_data(p, i).at(q);
...
void EventListSIMD::gather(const std::array<Event, real_v::size> &data, unsigned pos) {
  for(unsigned field = 0; field != m_data.nFields(); ++field) m_data(pos, field) = utils::gather<real_v>(data, [field](auto &event) { return event[field]; });
