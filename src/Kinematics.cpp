...
double MomentumTransfer::Q2(const double &s, const double &s1, const double &s2) const { return s / 4. - (s1 + s2) / 2. + (s1 - s2) * (s1 - s2) / (4 * s); }

HelicityCosine::HelicityCosine(const std::vector<unsigned> &p1, const std::vector<unsigned> &p2, const std::vector<unsigned> &pR) : _i(p1), _j(p2), _pR(pR) {}
