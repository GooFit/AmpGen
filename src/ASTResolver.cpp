...
ASTResolver::ASTResolver(const std::map<std::string, unsigned> &evtMap, const MinuitParameterSet *mps) : m_evtMap(evtMap), m_mps(mps) {}
...
{
  if(m_resolvedParameters.count(&spline) != 0)
    return;
...
template <> void ASTResolver::resolve<Parameter>(const Parameter &parameter)
{
  ...
  if(m_resolvedParameters.count(&parameter) != 0 || parameter.isResolved())
    return;
  ...
  if(!parameter.isResolved())
    m_unresolvedParameters.push_back(parameter);
}
...
template <> void ASTResolver::resolve<MinuitParameterLink>(const MinuitParameterLink &parameter)
{
  ...
  if(m_resolvedParameters.count(&parameter) != 0)
    return;
  if(m_mps == nullptr)
    return;
  auto it = m_mps->find(parameter.name());
...
{
  if(m_resolvedParameters.count(&obj) != 0)
    return;
...
void ASTResolver::addResolvedParameter(const IExpression *param, const std::string &thing) { m_resolvedParameters[param] = thing; }
