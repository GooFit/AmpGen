...
void ParticlePropertiesList::makeMappings() {
  for(auto &it : m_theList) {
    ...
    if(it.pdgID() != 0) {
      ...
      if(found != m_byID.end()) { WARNING("pdgID " << it.pdgID() << " used twice, here: " << (found->second)->name() << ", and here: " << it.name()); }
    }
    m_byID[it.pdgID()] = &it;
  }
}
...
void ParticlePropertiesList::addParticle(const std::vector<std::string> &tokens) {
  ...
  for(int i = 2; i != tokens.size(); i += 2) { pp->setProperty(tokens[i], tokens[i + 1]); }
  m_byName[name] = pp;
}
