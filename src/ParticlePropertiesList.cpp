...
const ParticlePropertiesList *ParticlePropertiesList::getMe()
{
  ...
  if(nullptr == ptr)
    FATAL("Couldn't get ParticlePropertiesList (i.e. myself)");
  return ptr;
}
...
ParticlePropertiesList::ParticlePropertiesList(const std::string &fname_in)
{
  ...
  if(!status)
    {
      WARNING("Failed to load full PDG configuration, beware of unexpected "
              "behaviour");
    }
  makeMappings();
}
      ...
      if(it.pdgID() != 0)
        {
          ...
          if(found != m_byID.end())
            {
              WARNING("pdgID " << it.pdgID() << " used twice, here: " << (found->second)->name() << ", and here: " << it.name());
            }
        }
      m_byID[it.pdgID()] = &it;
    }
...
const ParticleProperties *ParticlePropertiesList::find(int id, bool quiet) const
{
  ...
  if(it == m_byID.end())
    {
      if(!quiet)
        ERROR("Particle with id: " << id << " not recognised");
      return nullptr;
    }
  return it->second;
}
...
void ParticlePropertiesList::addParticle(const std::vector<std::string> &tokens)
{
  ...
  auto name = tokens[1];
  if(tokens.size() % 2 != 0)
    ERROR("Expecting properties as set of key value pairs");
