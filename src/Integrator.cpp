...
Bilinears::Bilinears(const size_t &r, const size_t &c) : rows(r), cols(c), norms(c * r), markAsZero(c * r, 0), calculate(c * r, 1) {}

complex_t Bilinears::get(const size_t &x, const size_t &y) const { return norms[x * cols + y]; }
void Bilinears::set(const size_t &x, const size_t &y, const complex_t &f)
{
  norms[x * cols + y] = f;
  calculate[x * cols + y] = false;
}
...
template <> complex_v Integrator::get(const unsigned &index, const unsigned &evt) const { return m_cache(evt, index); }
