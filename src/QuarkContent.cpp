...
char QuarkState::nameFromPosition(int i) const { return (i < 0 || i >= 6) ? 'X' : gNames[i]; }
...
bool QuarkState::operator!=(const QuarkState &rhs) const { return !(*this == rhs); }
...
bool QuarkContent::operator!=(const QuarkContent &rhs) const { return !(*this == rhs); }

QuarkState QuarkContent::operator[](const size_t &index) const { return m_quarks[index]; }

size_t QuarkContent::size() const { return m_quarks.size(); }

const std::vector<QuarkState> &QuarkContent::quarks() const { return m_quarks; }
