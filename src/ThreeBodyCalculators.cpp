      ...
      for(const auto &[particle, coupling] : all_decay_paths)
        {
          ...
          if(std::find(finalStates.begin(), finalStates.end(), type) == finalStates.end())
            finalStates.push_back(type);
        }
    }
  if(finalStates.size() == 0)
    FATAL("Particle: " << head << " has no integrable decay path");
  for(auto &type : finalStates)
    m_widths.emplace_back(type, mps);
  if(nKnots != 999)
    setAxis(nKnots, min, max);
}
  ...
  for(size_t c = 0; c < m_nKnots; ++c)
    {
      ...
      if(mps.find(knotName) != nullptr)
        mps[knotName]->setCurrentFitVal(I);
      INFO(knotName << " = " << I);
    }
  pc.stop();
