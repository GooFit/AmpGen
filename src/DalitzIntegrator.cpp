...
DalitzIntegrator::DalitzIntegrator(const double &s0, const double &s1, const double &s2, const double &s3) : m_s0(s0), m_s1(s1), m_s2(s2), m_s3(s3) {
...
real_v DalitzIntegrator::getMAB(const sqCo &coords) const { return m_min + (m_max - m_min) * (cos(M_PI * coords.first) + 1) / 2.; }
...
void DalitzIntegrator::debug() const {
  ...
  setEvent( pos, event );
  INFO( "s0 = " << m_s0 << " " << sqrt(m_s0) <<  ", x = " << pos.first << " "
  << pos.second << " r = " << m_min << "  " << m_max ); double mAB =
  getMAB(pos); double pA  = safe_sqrt( mAB * mAB / 4. - ( m_s1 + m_s2 ) / 2. +
  ( m_s1 - m_s2 ) * ( m_s1 - m_s2 ) / ( 4 * mAB * mAB ) ); std::cout << "pA = "
  << pA << " mAB = " << mAB << std::endl; for ( unsigned int i = 0; i < 12; ++i
  ) INFO( "Evt[" << i << "] = " << event[i] );
  */
}

/*
TH1D* DalitzIntegrator::makePlot( const std::function<double(const double*)>&
fcn, const Projection& projection, const std::string& name, const size_t&
nSamples )
{
...
TH2D* DalitzIntegrator::makePlot( const std::function<double(const double*)>&
fcn, const Projection2D& projection, const std::string& name, const size_t&
nSamples )
{
  auto plot = projection.plot();
  Event event( 12 );
...
TGraph *DalitzIntegrator::makeBoundaryGraph(const Projection2D &projection) const {
  ...
  auto setEventScalar = [this, &event](const double &f, const double &g) mutable {
    ...
    auto pA = this->safe_sqrt(0.25 * mAB * mAB - 0.5 * (this->m_s1 + this->m_s2) + (this->m_s1 - this->m_s2) * (this->m_s1 - this->m_s2) / (4. * mAB * mAB));
    auto eA = SQRT(pA * pA + m_s1);
    auto eB = SQRT(pA * pA + m_s2);
