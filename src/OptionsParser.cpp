...
void OptionsParser::import(const std::string &fName) {
  ...
  processFile(fName, [this, &currentTokens, &braceDepth](const auto &line) mutable {
    ...
    if(this->m_parsedLines.find(key) != this->m_parsedLines.end()) { WARNING("Overwriting parameter: " << key); }
...
std::vector<std::string> OptionsParser::makeParsedStrings(const std::string &line, int &braceDepth) const {
  ...
  if(insideQuotes) { WARNING("Unbalanced quotes in string : " << line); }
  if(braceDepth != 0) { DEBUG("Opened braces in line: " << line); }
  return fillThisList;
}
