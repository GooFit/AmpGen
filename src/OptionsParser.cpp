...
{
  m_keywords["Import"] = [this](const auto &tokens) {
    if(tokens.size() != 2)
      return;
  ...
  m_keywords["ParticlePropertiesList::Alias"] = [this](const auto &tokens) {
    if(tokens.size() != 3)
      return;
    ParticlePropertiesList::getMutable()->makeAlias(tokens[1], tokens[2]);
  };
  m_keywords["ParticlePropertiesList::Import"] = [this](const auto &tokens) {
    if(tokens.size() != 2)
      return;
    ParticlePropertiesList::getMutable()->readFile(tokens[1]);
  };
}

OptionsParser *OptionsParser::getMe()
{
  if(gOptionsParser == nullptr)
    gOptionsParser = new OptionsParser();
  return gOptionsParser;
}

void OptionsParser::setQuiet() { m_quiet = true; }
...
bool OptionsParser::ignoreThisLine(const std::string &line)
{
  ...
  for(int i = 0; ignoreLinesStartingWith[i] != '\0'; i++)
    {
      if(line[0] == ignoreLinesStartingWith[i])
        return true;
    }
  return false;
}
...
std::vector<std::vector<std::string>> OptionsParser::getInputOrdered() const
{
  ...
  for(const auto &key : m_orderedKeys)
    rt.push_back(m_parsedLines.find(key)->second);
  return rt;
}
...
void OptionsParser::setCommandLineArgs(int argc, char **argv, const std::string &description)
{
  ...
  if(m_printHelp)
    {
      std::cout << bold_on << "Usage: " << bold_off << argv[0] << italic_on
                << " options_file1.opt options_file2.opt --key1=value1 "
                   "--key2=value2 ..."
                << italic_off << std::endl;
      if(description != "")
...
void OptionsParser::addArg(const std::vector<std::string> &tokens)
{
  ...
  if(m_keywords.count(key) != 0)
    m_keywords[key](tokens);
  else
    {
...
std::vector<std::string> OptionsParser::makeParsedStrings(const std::string &line, int &braceDepth) const
{
  ...
  for(std::string::const_iterator it = s.begin(); it != s.end(); ++it)
    {
      if(!insideQuotes && *it == '#')
        break; /// indicates a comment, except in speech marks ///
