...
EventType::EventType(const std::vector<std::string> &particleNames, const bool &isTD) : m_timeDependent(isTD)
{
  ...
  if(particleNames.size() < 3)
    { // Mother plus two daughters minimum required
  ...
  auto dimOfParticle = [](auto &name) { return name == "gamma0" ? 2 : ParticlePropertiesList::get(name)->twoSpin() + 1; };
  m_dim.first = dimOfParticle(m_mother);
  m_dim.second = 1;
  for(auto &p : m_particleNames)
    m_dim.second *= dimOfParticle(p);
}
  ...
  for(unsigned int ip = 0; ip < size(); ++ip)
    {
      ...
      if(m_includeEnergy)
        returnValue[stub + "_E"] = s * ip + 3;
      returnValue[stub + "_Px"] = s * ip + 0;
      returnValue[stub + "_Py"] = s * ip + 1;
      returnValue[stub + "_Pz"] = s * ip + 2;
    }
  if(m_timeDependent)
    returnValue[m_mother + "_decayTime"] = 4 * size();
  for(auto &extend : m_eventTypeExtensions)
    returnValue[extend] = returnValue.size();
  return returnValue;
}
void EventType::extendEventType(const std::string &branch) { m_eventTypeExtensions.push_back(branch); }
...
std::string EventType::label(const unsigned &index, bool isRoot) const { return ParticlePropertiesList::get(m_particleNames[index])->label(); }
...
Projection EventType::projection(const unsigned &nBins, const std::vector<unsigned> &indices, const std::string &observable) const
{
  ...
  if(observable == "mass2")
    ...
    {
      return Projection([indices](const Event &evt) { return sqrt(evt.s(indices)); }, "m" + vectorToString(indices), "m_{" + label(indices) + "}", nBins,
...
{
  if(m_mother != other.mother() || size() != other.size())
    return false;
  for(unsigned i = 0; i < m_particleNames.size(); ++i)
    {
      if(m_particleNames[i] != other[i])
        return false;
    }
  return true;
}
...
std::function<void(Event &)> EventType::symmetriser(unsigned seed) const
{
  ...
  for(auto &im : particleOrdering)
    if(im.second.size() != 1)
      shuffles.push_back(im.second);
  std::mt19937 rng(seed);
  for(auto &shuffle : shuffles)
    {
      std::string shuffle_string = "";
      for(auto &s : shuffle)
        shuffle_string += std::to_string(s) + " ";
...
std::function<bool(Event &, const std::vector<int> &)> EventType::automaticOrdering() const
{
  std::vector<int> ids;
  for(unsigned i = 0; i != m_particleNames.size(); ++i)
    ids.push_back(ParticleProperties::get(m_particleNames[i])->pdgID());
  ...
  return [ids, matches](auto &event, const auto &actual_ids) -> bool {
    ...
    int sgn = +1;
    if(matches(ids, actual_ids))
      sgn = +1;
    else if(matches(ids, actual_ids, -1))
      sgn = -1;
    else
      {
        ERROR("Ids: " << vectorToString(actual_ids, " ") << " do not match either particle or antiparticle [" << vectorToString(ids, " ") << "]");
        return false;
      }
    ...
    for(unsigned i = 0; i != ids.size(); ++i)
      {
        for(unsigned j = 0; j != actual_ids.size(); ++j)
          {
            if(actual_ids[j] == sgn * ids[i] && new_addresses[j] == 999)
              {
                new_addresses[j] = i;
                break;
              }
...
std::string EventType::decayDescriptor() const { return mother() + "{" + vectorToString(m_particleNames, ",") + "}"; }

Event EventType::makeEvent() const { return PhaseSpace(*this).makeEvent(); }

std::string EventType::constructor_string() const { return mother() + " " + vectorToString(m_particleNames, " "); }
