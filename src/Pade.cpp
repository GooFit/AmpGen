...
std::vector<double> AmpGen::detail::solve_pade(const std::function<double(const double &)> &fcn, const double &min, const double &max, const unsigned &N,
  ...
  for(unsigned eq = 0; eq < 2 * N + 1; ++eq)
    {
      ...
      for(unsigned i = 0; i <= N; ++i)
        gsl_matrix_set(solver, eq, i, std::pow(samples[eq], i));
      for(unsigned i = 1; i <= N; ++i)
        gsl_matrix_set(solver, eq, i + N, -rest[eq] * std::pow(samples[eq], i));
    }
  ...
  for(unsigned i = 0; i < 2 * N + 1; ++i)
    {
      for(unsigned j = 0; j != 2 * N + 1; ++j)
        rv[i] += gsl_matrix_get(inverse, i, j) * rest[j];
    }
