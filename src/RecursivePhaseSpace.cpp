...
RecursivePhaseSpace::RecursivePhaseSpace(const Particle &decayChain, const EventType &type, TRandom *rndm) : m_name(decayChain.name()), m_eventType(type)
{
...
void RecursivePhaseSpace::setRandom(TRandom *rand)
{
  ...
  for(auto &node : m_nodes)
    if(node.decayProds != nullptr)
      node.decayProds->setRandom(rand);
}
