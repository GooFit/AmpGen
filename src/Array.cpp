...
std::string Array::to_string(const ASTResolver *resolver) const {
  ...
  std::string offset = "";
  if(m_size != -1)
    offset = Ternary(m_address >= 0 && m_address < m_size, m_address, 0.).to_string(resolver);
  else if(is<Constant>(m_address))
    offset = std::to_string(int(m_address().real()));
  else
...
Expression Array::operator[](const Expression &address) const { return Array(m_top, m_size, address); }
