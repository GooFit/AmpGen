...
MatrixElement::MatrixElement(const Particle &dt, const TotalCoupling &coupling, const amp_type &amp) : amp_type(amp), decayTree(dt), coupling(coupling)
{
...
MatrixElement::MatrixElement(const Particle &dt, const TotalCoupling &coupling, const MinuitParameterSet &mps, const std::map<std::string, unsigned> &evtFormat,
                             const bool &debugThis)
    : amp_type(dt.decayDescriptor()), decayTree(dt), coupling(coupling)
{
...
std::vector<size_t> AmpGen::processIndex(const std::vector<MatrixElement> &tm, const std::string &label)
{
  ...
  for(size_t i = 0; i < tm.size(); ++i)
    {
      if(tm[i].coupling.contains(label))
        indices.push_back(i);
    }
  return indices;
}
...
size_t AmpGen::findIndex(const std::vector<MatrixElement> &tm, const std::string &decayDescriptor)
{
  for(size_t i = 0; i < tm.size(); ++i)
    {
      if(tm[i].decayDescriptor() == decayDescriptor)
        return i;
    }
...
std::vector<size_t> AmpGen::findIndices(const std::vector<MatrixElement> &tm, const std::string &decayDescriptor)
{
  ...
  for(size_t i = 0; i < tm.size(); ++i)
    {
      if(tm[i].decayDescriptor().find(decayDescriptor) != std::string::npos)
        rt.push_back(i);
    }
  return rt;
}
