...
PhaseSpace::PhaseSpace(const Particle &particle, TRandom *rand) : PhaseSpace(particle.eventType(), rand) {}
...
void PhaseSpace::fill(double *output, unsigned width)
{
  ...
  do
    {
      ...
      rno[0] = 0;
      for(n = 1; n < m_nt - 1; n++)
        rno[n] = m_rand->Rndm(); // m_nt-2 random numbers
      rno[m_nt - 1] = 1;
  ...
  if(m_type.isTimeDependent())
    output[4 * m_nt * width] = m_rand->Exp(m_decayTime);
}
