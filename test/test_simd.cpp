...
#define test_simd_complex(avx_function, scalar_function, data)                                                                                                 \
  {                                                                                                                                                            \
    ...
    for(int i = 0; i != real_v::size; ++i) {                                                                                                                   \
      BOOST_TEST(std::real(utils::at(v, i)) == std::real(scalar_function(utils::at(data, i))));                                                                \
...
BOOST_AUTO_TEST_CASE(test_fmod, *utf::tolerance(scalar_t(precision))) {
  ...
  for(int i = 0; i != real_v::size; ++i) { BOOST_TEST(mod[i] == fmod(a[i], b[i])); }
}
...
BOOST_AUTO_TEST_CASE(test_double_to_int) {
  ...
  auto t = simd::store(f);
  for(int i = 0; i != real_v::size; ++i) {
    std::cout << "test[" << i << "] : " << t[i] << " " << a[i] << std::endl;
    BOOST_TEST(t[i] == int(a[i]));
  }
}
...
#else
BOOST_AUTO_TEST_CASE(test_dummy) { BOOST_TEST(1 == 1); }
#endif
